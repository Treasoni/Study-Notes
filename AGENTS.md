...
- 交接或提交前运行 `.codex/scripts/workflow-health-check.sh`：它是上述各条校验的统一入口，聚合 routing 表校准、manifest 注册校验、跨机器可移植性检查，以及「workflow 定义只能调用 `todo-state.sh` 真实实现的动作」守卫。
...
- 每次工作流新增、修改、重命名或删除后，必须运行 `.codex/scripts/sync-workflow-routing.sh`，并确保 `.codex/scripts/sync-workflow-routing.sh --check` 通过；随后用 `.codex/scripts/workflow-health-check.sh` 收口，它会探测 workflow 定义里调用的每个 `todo-state.sh` 动作是否真实存在。
