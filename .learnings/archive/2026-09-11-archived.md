...
本文件收纳 2026-09-11 之前已**完全蒸馏进 `RULES.md`** 的详细条目。
保留全文仅供追溯，日常只需读 `RULES.md`。

归档理由：
- 2026-09-11 `digest` 时 `LEARNINGS.md` 已达 145 行（超过 100 行阈值），属旧记录堆积，按 digest Step 1 压缩归档。
- 其中 `LRN-20260816-004` 含一条**已失效事实**（"python/python3 是商店存根不可用"）——2026-09-11 复核本机 `python3` 为原生 CPython 3.14.7（MSC 版本号，非商店存根），可正常处理中文。该条已按新事实改写进 `RULES.md`，故原文归档而不删除，保留判断依据。
