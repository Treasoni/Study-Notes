...
## [ERR-20260911-007] chapter-writer：父 agent 在 prompt 里改写来源论断，章节因此出现伪引证

### 错误：第二章 2.9.1 写成「官方口径是：`Run` / `RunOnce` 这两个键本身不执行任何操作，是系统在用户登录时去读取它们 [S19]」——官方原文没有"键本身不执行任何操作"这层意思

**错误**：回查缓存官方页（S19）确认原文是「使用 `Run` 或 `RunOnce` 注册表项**使程序在用户登录时运行**」。伪论断落到两处：2.9.1 的 callout、以及本章小结。

**触发场景**：父 agent 给 chapter-writer 写 prompt 时，用自己的话概括了一条来源论断，冠以「官方口径是」并附上来源 ID。

**根因**：**转述（paraphrase）和引证（citation）混写**。子 agent 无法区分 prompt 里哪句是原文、哪句是父 agent 的概括，于是照抄措辞并保留 `[S19]`，把我的话包装成了官方原话。子 agent 执行无误，错误 100% 来自上游 prompt——**引用链条断在派发方，不在写作方**。

**修复**：
- 两处均改回官方原文口径「使程序在用户登录时运行」，并补上原文可推出的触发时机「用户登录」。
- 复查第二章其余「官方口径是」句式，无同类问题。

**预防措施**：
- 给子 agent 派活时，**不要把来源论断改写成陈述句塞进 prompt**；只传来源 ID 与检索位置，要求「先回原文核对再落笔」。
- 确需在 prompt 里带结论时，必须成对标注：原文引文加引号，非原文加「（概述，待核对）」。
- 章节验收时，对所有「官方口径是 / 官方说明 / 原文」句式**逐条回源比对**，不能只看有没有挂来源 ID。
