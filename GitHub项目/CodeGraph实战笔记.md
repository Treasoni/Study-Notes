...
> [!info] 工具简介
> CodeGraph 是为 AI 编程助手设计的代码语义分析工具，通过预索引知识图谱提供符号关系、调用图谱和代码结构查询。
...
- 命令行终端操作
- [[待创建: MCP]] (Model Context Protocol) 概念
...
> [!info] 测试条件
> Claude Opus 4.7, Claude Code v2.1.145，`claude -p` 模式，4 次运行中位数
...
> [!success] 平均效果
> **35% 更便宜 · 59% 更少 Token · 49% 更快 · 70% 更少工具调用**
...
flowchart TB
    ...
    subgraph CGS["CodeGraph MCP Server"]
        SEARCH["Search"]
        CALLERS["Callers"]
        CTX["Context"]
        DB[(SQLite<br/>FTS5)]
    ...
    class CC internal-link
    class CGS internal-link
```
...
- 下一步可以学什么：[[待创建: Claude Code 进阶使用]]
- 相关实战：[[待创建: CodeGraph 实战案例]]
