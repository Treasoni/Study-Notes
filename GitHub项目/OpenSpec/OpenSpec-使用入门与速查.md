---
topic: OpenSpec
title: OpenSpec 使用入门与速查
type: concept + cheat-sheet
...
> [!quote] 核心哲学
> SDD 的关键转变：从"告诉 AI 做什么"到"和 AI 一起定义做什么"。
...
> [!tip] 棕地优先的意义
> 如果你正在维护一个大型现有项目，Delta Specs 可以让你逐步引入规范，而不需要一次性为整个系统编写完整规范。
...
flowchart TD
    ...
    changes --> change["<change-name>/"]
    change --> proposal["proposal.md\n(为什么做)"]
    change --> design["design.md\n(怎么做)"]
    change --> tasks["tasks.md\n(检查清单)"]
    change --> deltaSpecs["specs/\n(增量规范)"]

    deltaSpecs -- 归档时合并 --> specs

    style specs fill:#4a9,stroke:#2a7,color:#fff
    style change fill:#67b,stroke:#458,color:#fff
```
...
flowchart LR
    ...
    style A fill:#e8a,stroke:#c6a,color:#fff
    style B fill:#fa0,stroke:#d80,color:#fff
    style C fill:#4a9,stroke:#2a7,color:#fff
    style D fill:#67b,stroke:#458,color:#fff
    style E fill:#888,stroke:#666,color:#fff
```
...
> [!warning] 审查不可跳过
> 仔细阅读 proposal.md 和 design.md，确保 AI 的方案和你的想法一致。跳过这个验证步骤是常见的踩坑点。[来源: R15]
...
> [!tip] 实现中的调整
> 发现设计问题不要直接改代码，先更新 design.md 再继续 apply。这保持了规范的完整性。
...
> [!warning] 旧版语法兼容性
> 如果看到 `/openspec-proposal` 这样的旧版语法，说明是旧版 OpenSpec。新版统一使用 `/opsx:` 前缀。示例中的厨房计时器教程 [来源: R12] 就是使用的旧版语法。
...
> [!tip] Context vs Rules
> Context 注入到所有工件，适合描述项目全局信息（技术栈、API 风格等）。Rules 只影响特定工件类型，适合精细化控制。
...
> [!note] 笔记说明
> 本文档是 OpenSpec 的使用入门与速查笔记，综合了官方文档和社区资源编写而成。涉及 OpenSpec 内部架构、CI/CD 集成、大规模项目性能等话题，当前资料覆盖不足，标注为 [待补充]。如有新的研究发现，欢迎扩展相应章节。
