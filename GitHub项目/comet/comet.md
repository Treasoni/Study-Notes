...
2. **Superpowers 产出细致的 Spec 文档**，但这些文档 ==没有状态化设计==——做完需求后只在文档上打勾（甚至 Agent 还会忘记打勾），导致断点续传时 Agent 要重新读代码猜进度，浪费大量 Token [来源: 2]
...
> OpenSpec 决定"做什么"，Superpowers 决定"怎么做"，Comet 把它们串起来自动跑。 [来源: 1][来源: 2]
...
> [!tip] 关键特性
> 主入口 `/comet` 支持状态检测。关闭会话后回来只需再次 ==`/comet`==，它会自动读取活跃 Spec，动态识别当前执行到哪个阶段，继续往下执行。 [来源: 1][来源: 2]
...
sequenceDiagram
    ...
    User->>Comet: /comet
    Comet->>Comet: 检测活跃 Spec
    Comet->>OS: Open: propose
    OS->>User: 提案/规格/设计/任务
    User->>Comet: 确认方案

    Comet->>SP: Design: brainstorming
    SP->>User: Design Doc
    User->>Comet: 批准设计

    Comet->>SP: Build: TDD 执行
    SP->>SP: Implementer → Reviewer
    SP->>User: 完成代码

    Comet->>Comet: Verify: Guard 脚本
    Comet->>OS: Archive: 归档变更
    OS->>User: 变更已归档
```
...
> [!info] 与传统方案的本质区别
> 传统方案的 Spec 只是静态 Markdown，AI 每次重启都得从头理解上下文。Comet 的状态文件让 AI 知道"我上次做到哪了，接下来该做什么"。 [来源: 1][来源: 2]
...
- **双星架构核心分工**：OpenSpec 管 WHAT，Superpowers 管 HOW，Comet 管串联 [来源: 1][来源: 2]
...
> [!info] 延伸阅读
> - [[Superpowers]] — 了解 HOW 层的工作原理
> - `[待创建: OpenSpec]` — 了解 WHAT 层的规格管理
> - [Comet GitHub 仓库](https://github.com/rpamis/comet) — 源码和最新更新
