...
本笔记系统拆解 Matt Pocock 的 skills 仓库——一个 184K+ Stars 的开源项目，专门存放高质量 [[Agent Skill]]（skills）。全书共 8 章，从前 7 章对仓库的逐层拆解，到第 8 章的综合应用指南，回答一个核心问题：**如何用工程纪律驯服 Agent，使其输出从"随机"变为"可预测"？**
...
> [!quote] 核心洞察
> 在 Agent 框架中，每一次不确定性都是有代价的——要么消耗用户的认知，要么消耗模型的上下文。
...
共享的精准术语比冗长的解释更有价值。[[Leading Word]] 是这一原则的极致体现。
...
> [!note] 宪法文件的功能分工
> - **CLAUDE.md**：结构规则 + 调用规则 + 维护纪律
> - **AGENTS.md**：跨平台契约（与 CLAUDE.md 内容一致）
> - **CONTEXT.md**：核心术语词典 + Avoid 指令
...
**[[Cognitive Load]]（认知负载）**：用户必须记住 skill 存在的负担。每个 user-invoked skill 都在用户的"技能清单"上占据一个位置。当用户记不住有哪些 skill 时，就需要 [[Router]]。

**[[Context Load]]（上下文负载）**：模型每次处理 description 消耗的 token。每个 model-invoked skill 的 description 都在每轮对话中被读取。当 model-invoked skills 太多时，上下文被大量 description 占据。
...
[[Predictability]]（可预测性）不是指每次都产生相同输出，而是指 Agent 每次运行都遵循**相同的过程**。
...
每个 step 都必须有一个完成标准，防止 Agent **[[Premature Completion]]（过早完成）**。
...
>
> 如果你编一个新词，需要花 token 解释——抵消了 leading word 的优势。优先从模型的预训练词汇中寻找。
...
| 5 | **[[No-Op Test|No-Op]]** | 指令不改变行为 | 删除（大多数救不了） |
...
> [!note] 三层抽象的关键启示
> **核心逻辑应该与状态管理分离**。grilling 只负责"盘问"这个行为，`grill-with-docs` 才负责"盘问并保存"。如果你的 skill 也有"有状态"和"无状态"两个版本，考虑用同样的分层。
...
> [!note] 第 1 句：`relentlessly` 是 leading word
> 不是普通的 interview，是不放弃、不遗漏、穷尽的 interview。

> [!tip] 第 5 句：推荐答案是最关键的设计决策
> Agent 给出自己的推荐答案，用户只需要确认/修正/拒绝。这把"开放式问题"变成了"选择题"。

> [!example] 第 9 句：事实/决策的边界划分
> "Facts → look up, Decisions → ask me"。不浪费用户时间去回答 Agent 能从环境找到的信息。
...
> [!tip] Context Hygiene 的核心逻辑
> - **前 3 步同一窗口**：因为它们是互相依赖的认知工作。盘问→规格→任务拆分，每个步骤的产出都是下一步的输入。
> - **每 implement 新会话**：因为实现是执行性的工作，只需要规格，不需要盘问的完整细节。新会话减少 token 污染。
...
| 规则 | 设计意图 |
|------|---------|
| 保存到临时目录 | 交接文档是临时的——读一次后无价值。保存到工作区会污染项目。 |
| 包含 "suggested skills" | 最巧妙的规则——不仅告诉新 Agent 之前发生了什么，还告诉它接下来该用什么 skill。 |
| 不重复已有内容 | 交接文档是**索引**，不是仓库。已有文件通过路径引用，大幅压缩文档大小。 |
| 过滤敏感信息 | 防止 API keys、密码等在传递过程中泄露。 |
| 参数定制 | 支持 `argument-hint`——只生成下一个 session 需要的部分。 |
...
> [!tip] 实用原则
> **不想丢失任何细节时用 handoff，愿意接受一定程度摘要时用 compact。**
...
Implement 是"从想法到代码"的最终执行者。它是 user-invoked skill，内部委托给 model-invoked skills：
...
> [!note] Implement 不做决策，只执行
> 它是"手，不是头"——头已经在 grilling + to-spec 阶段完成了思考。

## 7.3 词汇表即架构 — codebase-design

codebase-design 是一个不包含步骤、全部是参考的 model-invoked skill。它是其他 skill（如 tdd、improve-codebase-architecture）的共享词汇表。
...
| [[No-Op Test\|No-Op]] | 无效指令 | 不改变行为的指令 |
| Negation | 否定陷阱 | 负面指令反而强化目标行为 |
| [[Handoff\|Smart Zone]] | 智能区间 | 模型能稳定推理的上下文大小（~12 万 token） |
