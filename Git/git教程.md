...
👉 优点：保持历史是线性的，没有多余的 merge commit

### 场景 2：分支合并前，让历史变直

假设你在 feature-login 分支上开发：
...
这会把 feature-login 上的提交，全部"挪"到 main 的最新提交后面。
...
## rebase vs merge（怎么选？）
  ...
  **情况**                            **推荐**
  ...
  个人整理提交记录                  rebase
  拉取远程代码                      rebase
  功能分支合并到 main              merge（保留分支历史）
  团队协作已 push 的提交            merge（安全）
  ------------------------------------------------------------
...
| 解决冲突后继续 | `git add <file>; git commit` |
| 撤销合并 | `git merge --abort` |

---

**学习建议：**
1. 先掌握基本操作：init, add, commit, push, pull
2. 理解 4 个区域和 4 种状态
3. 学会分支操作
4. 学会解决冲突
5. 再进阶学习 rebase、reset 高级用法
