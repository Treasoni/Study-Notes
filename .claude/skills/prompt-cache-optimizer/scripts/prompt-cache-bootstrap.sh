...
Options:
  ...
  --agent SPEC         Add a custom agent profile: name,agent_dir,entry_file[,rule_path].
                       Example: generic,.agent,CLAUDE.md
...
Examples:
  ...
  bash prompt-cache-bootstrap.sh --apply --platform none --agent myagent,.agent,CLAUDE.md --target ../my-project
USAGE
}
...
emit_profile_spec() {
  ...
  case "$name" in
    codex) printf '%s\n' $'codex\t.codex\tCLAUDE.md\t.claude/rules/common/prompt-cache.md\t.claude/skills' ;;
    claude) printf '%s\n' $'claude\t.claude\tCLAUDE.md\t.claude/rules/common/prompt-cache.md\t.claude/skills' ;;
    generic) printf '%s\n' $'generic\t.agent\tCLAUDE.md\t.agent/rules/common/prompt-cache.md\t.agent/skills' ;;
    *)
