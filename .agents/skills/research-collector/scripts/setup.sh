...
# ---------------------------------------------------------------------------
# 1. 定位 conda 可执行文件（同时覆盖 Unix/macOS 与 Windows/Git Bash 布局）
# ---------------------------------------------------------------------------
resolve_conda() {
...
# ---------------------------------------------------------------------------
# 2. 解析 conda 根目录（Windows/Git Bash 下把 Windows 路径转成 unix 风格）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. 定位 env 内 python（Windows: python.exe；Unix/macOS: bin/python）
# ---------------------------------------------------------------------------
if [ -f "$ENV_DIR/python.exe" ]; then
  ...
  echo "错误: 创建后未在 $ENV_DIR 找到 python 可执行文件。" >&2
  exit 1
fi
