...
aliases:
  - Subagent 的两种启动模式
  - Subagent General-purpose vs Fork
---
...
flowchart LR
    A["主会话 (已授权 Read/Edit)"]
    A -->|启动 general-purpose subagent| B["新会话, 零权限"]
    B -->|尝试 Read/Edit| C[权限弹窗出现]
    C -->|后台运行, 弹窗被隐藏| D[卡住等待审批]
    D -->|超时| E[失败]
```

这正是在研究系统中不断重复的现象。==General-purpose 模式适合完全独立的任务==——比如一个只读分析任务，不需要操作代码库。 [来源: 个人经验]
...
flowchart LR
    A["主会话 (已授权 Read/Edit)"]
    A -->|启动 fork subagent| B["继承全部权限和上下文"]
    B -->|直接执行 Read/Edit| C[无需审批]
```

切换到 Fork 模式后，后台 Subagent 再没有因为权限弹窗而卡死。 [来源: 个人经验]

> [!tip] 核心经验
> 凡是需要在后台执行读写代码操作的 Subagent，优先使用 Fork 模式。General-purpose 模式只适用于完全独立、不需要操作代码库的只读任务。
...
> [!note] Subagent 上下文边界
> Subagent 的上下文边界由 AgentDefinition 的字段显式控制。以下清单列出了哪些内容会传递到 Subagent、哪些不会。
...
- 父代理的工具执行结果
- 父代理的系统提示
- 预加载的 skill 具体内容（除非在 `skills` 字段中明确列出）
...
==第 3 步（ask）和第 6 步（canUseTool 回调）是用户交互瓶颈==。当 Subagent 在后台运行时，这些权限提示被隐藏，Subagent 就卡死了。 [来源: 个人经验]
...
flowchart TD
    A[主会话]
    A -->|启动 research subagent| B["Fork 模式"]
    B -->|读取 50 个文件| C[独立上下文]
    C -->|返回摘要| D["仅摘要进入主对话"]
    D -->|主对话保持清洁| E[可继续高效工作]
```
...
> [!tip] 上下文隔离的最佳实践
> 对于研究密集型任务（如读取大量文件进行分析），使用 Fork 模式启动 Subagent 来承载"脏活"，可以有效控制主会话的 Token 消耗，延长单次会话的有效工作时间。
...
- 相关笔记：[待创建: Claude Code Subagent 与 Skill 调度机制]——关于 Subagent 调度与 Skill 编排

## 思考题

> [!question] 深化理解
> 以下问题帮助你检验对 Subagent 启动模式的理解深度：
