...
在使用 Claude Code 构建自动化工作流时，遇到了一个典型的架构设计问题：**当一个 [[Agent]] 和一个 [[Skills]] 实现了相同的功能时，它们之间是层级调用关系还是权限争夺关系？**
...
> [!warning] `[待验证]` 问题根源
> 当面对两个都能完成任务的选项时，大模型的选择行为缺乏官方文档明确说明。根据社区经验观察，可能出现：
...
```mermaid
flowchart TB
    subgraph 主流程["主流程（指挥者）"]
        A[发出任务]
    end

    subgraph Subagent["Subagent（规划者）"]
        B[接收任务]
        C[复杂推理]
        D[决策：何时调用什么]
    end

    subgraph Skills["Skills（执行者）"]
        E[Skill 1]
        F[Skill 2]
        G[Skill N]
    end
