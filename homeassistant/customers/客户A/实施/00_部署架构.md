...
graph TB
  ...
  subgraph ENDDEV["终端设备（多品牌）"]
    Z["Aqara 开关/传感器 + 杜亚窗帘（Zigbee→中枢）"]
    W["Yeelight 灯 + 德施曼门锁 + 石头扫地机"]
    C["萤石摄像头 RTSP"]
    AC["美的/格力空调 + 海尔白电"]
    HUE["飞利浦 Hue（Hue Bridge）"]
    T["涂鸦插座/温湿度计"]
  end
  BE --- NAS
  BE --- GW
...
### ADR-004 · 安防 NVR：Frigate（Docker） + 萤石摄像头 RTSP

- **决策**：萤石（海康威视系）摄像头开 RTSP/ONVIF，Frigate 跑在 NAS Docker，录像存 NAS。
- **理由**：样板房必须展示专业安防品牌（萤石为必露品牌）；RTSP/ONVIF 接入 Frigate，Frigate 是 HA 标准 NVR 方案，支持人形检测（OpenVINO on iGPU）。HA 内 `frigate` 集成出事件/抓拍。
- **注意**：确认萤石型号支持 RTSP（萤石本地设置开启；ONVIF 兜底）；萤石本地账号密码需在配置中保留。
...
### ADR-008 · 设备矩阵：全面打散多品牌（样板房展示）

- **决策**：设备**每品类用不同品牌**，全屋 12+ 品牌一个平台统一控制。品牌分布：Yeelight（灯）/ 飞利浦 Hue（氛围）/ Aqara（开关传感器）/ 杜亚（窗帘）/ 德施曼（门锁）/ 萤石（摄像头）/ 美的+格力+海尔（白电）/ 石头（扫地）/ 涂鸦（插座温控）/ 小米（中枢音箱网络安全）。
- **理由**：本项目兼作样板房，「跨品牌兼容」是核心卖点；全面打散比「小米全家桶」说服力强得多。所有品牌均落在既有接入矩阵内（xiaomi_home/tuya+localtuya/midea_ac_lan/gree/hon/yeelight/hue/roborock/RTSP），不引入新技术栈。
- **集成要点**：杜亚走涂鸦 localtuya（窗帘有 localKey，锁/摄像头无——本方案摄像头用萤石 RTSP 不走涂鸦）；德施曼选**米家版**走 xiaomi_home；石头选支持 `roborock` 集成或米家版；萤石走 RTSP/ONVIF→Frigate。
- **风险**：多品牌 = 更多集成点，演示稳定性需逐一验证；海尔 hon 云较脆弱，标注「进阶展示」不设为核心依赖。
...
| 萤石摄像头 RTSP 支持 | 中 | 采购前确认型号支持 RTSP/ONVIF |
| 多品牌集成点演示稳定性 | 中 | 逐一验证各品牌集成；演示前跑一遍巡检脚本 |
| 海尔 hon 云脆弱 | 低 | 标注进阶展示，不设为核心依赖 |
| 客户暖通未确认 | 低 | 方案分叉，报价两档 |
