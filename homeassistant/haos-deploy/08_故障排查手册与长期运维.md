...
**③ ghcr.io 拉不动 → 控制台提前配 4.2 国内镜像（核心）**

DNS 通了仍死循环，就是 ghcr.io 瓶颈。此时还没过初始化、SSH 开不了，但 **VM 控制台可以 `login` 进 root shell**，提前把 [[04_手动配置国内源|4.2]] 的镜像映射配好：
