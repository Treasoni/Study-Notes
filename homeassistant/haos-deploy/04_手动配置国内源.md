...
>
> 进入 HAOS 底层 Linux shell 的两条路：
> - **宿主 SSH**（改系统文件的入口）：`ssh root@<IP> -p 22222`。启用入口随版本不同（新版一般在「设置 → 系统」的硬件/调试区有 SSH 开关；或接显示器在 VM 控制台登录）。
...
> [!note] 常见疑问：必须先进 Home Assistant 才能用宿主 SSH 吗？
> 分两种情况：
> - **走宿主 SSH（22222）**：是的。SSH 服务默认关闭，开关在 Web 界面里。最小路径：烧录 → 开机 → 浏览器打开 `http://<IP>:8123` 完成首次引导（创建管理员账号）→ 「设置 → 系统」硬件/调试区打开 SSH 开关 → `ssh root@<IP> -p 22222`。这里只需完成引导、能进设置页即可，不必先把 HA 全部配好。
> - **两条绕开 Web 界面的路**：① 接显示器 / VM 控制台直接 `login` 进 root shell，或从控制台开启 SSH；② 首启前预置——把 4.4 的 `timesyncd.conf` 写进烧录后的 `hassio-boot/CONFIG/`（权限 644），开机前就完成。
>
> > [!warning] 鸡生蛋陷阱
> > 很多人以为的"SSH"是 **Advanced SSH & Web Terminal 加载项**，它要从 Add-on 商店安装，而商店拉镜像正是本章要解决的 ghcr 瓶颈。**不要**指望先装加载项再改源，应优先用内置 22222 宿主 SSH。
>
> > [!tip] 一句话流程
> > 烧录 → 开机 → `http://<IP>:8123` 完成引导 → 设置里开 SSH → `ssh root@IP -p 22222` 改文件。若连首次引导都卡在"拉 700MB 组件"，先按 4.4 预置时间源，或换第 5 章 [[05_HAOS-CN极速版]]。
