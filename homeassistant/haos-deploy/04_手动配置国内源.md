...
> - **终端加载项**（日常 `ha` 命令）：加载项商店装 **Advanced SSH & Web Terminal**，打开 Web UI 即是终端。
>
> **关于 4.1 的 `daemon.json`**：官方 HAOS 根文件系统**只读**，`/etc/docker/daemon.json` 改不了。那段 12 源配置适用于普通 Linux + Docker、Supervised 安装，或已内置该配置的 [[HAOS-CN]]。在官方 HAOS 上真正要改的是 **4.2 的 `/usr/share/hassio/docker.json`** 和 **4.4 的 `/etc/systemd/timesyncd.conf`**——两者都走宿主 SSH。
...
> - **③ 加载项终端（日常 `ha` 命令）**：商店装 **Advanced SSH & Web Terminal**，默认端口 22，Web 界面里即是终端。
>
> > [!warning] 鸡生蛋陷阱
> > 很多人以为的"SSH"是 **Advanced SSH & Web Terminal 加载项**，它要从 Add-on 商店安装，而商店拉镜像正是本章要解决的 ghcr 瓶颈。**不要**指望先装加载项再改源；改系统文件优先用 22222 宿主 SSH（①）或控制台 `login`（②）。
>
...
> [!warning] 动手前先确认：文件不存在是正常的 + 官方 HAOS 不要改这个
> ① `/etc/docker/daemon.json` 不是 Docker 默认自带文件，**不存在是正常的**。普通 Linux + Docker 或 Supervised 安装需要先手动创建：`mkdir -p /etc/docker` 再新建该文件。
> ② 如果你在**官方 HAOS**（本教程主角）：该文件位于**只读根文件系统**，改了也无效，**请跳过本节**。官方原版真正要改的是 **4.2 的 `/usr/share/hassio/docker.json`**——那才是解决 ghcr 拉取瓶颈的正路。

配置写入 `/etc/docker/daemon.json`（完整 12 源示例，仅适用于普通 Linux + Docker / Supervised）：
...
> 1. **没有配置入口**：OS 更新下载地址在 Supervisor 内部写死，指向 GitHub 官方 Releases，不是 4.2 的 `docker.json` 能改的；
