---
title: "第五章：HA Supervised 详解（含弃用现状）"
tags:
  - 智能家居/HomeAssistant
  - 学习笔记
  - 部署方式
...
**定义**：Supervised = 在你自己管理的 Linux 上，用安装器部署 Supervisor + Core，从而获得「完整 HA 组件（除 HAOS 之外）」。它保留了宿主 OS 的控制权——apt、systemd、自定义服务都可以继续用——同时拥有 HAOS 才有的 Supervisor 与 Add-on 商店。正因如此，历史上官方定位是 **「only for advanced users」**，要求精通 Linux / Docker / 网络，维护难度列为 Expert。

**ADR-0014 与 revert**：Supervised 的官方支持曾由一份架构决策记录（ADR）正式定义，编号 ADR-0014。但该 ADR 状态已标记 **reverted（已撤销）**，官方不再承认 Supervised 是受支持的安装方式。网上大量 Supervised 教程写于 ADR 有效期内，内容基于这份已撤销的正式支持——阅读这类教程前，先给它打个问号。
...
官方唯一支持的宿主操作系统是 **Debian 12 Bookworm**，且**不接受任何衍生版**——Raspberry Pi OS、Ubuntu 等都会被安装器直接拦截。注意这是硬性检查，不是「强烈建议」：树莓派用户想跑 Supervised，必须先刷纯 Debian 12，而不是常见的 Raspberry Pi OS，否则安装器第一步就过不去。
...
比依赖清单更苛刻的是一条隐含约束：**宿主必须专用于 HA，不得安装额外软件**。因为几乎任何对系统状态的改动，都可能让 Supervised 的自检从 Healthy 变成 Unsupported / Unhealthy。这带来一个反直觉的结论：Supervised 虽然跑在你的 Linux 上，但你想在它旁边装别的服务，恰恰是最容易触发 Unsupported 的行为之一。

## 5.3 安装原理与脚本步骤

官方曾维护安装脚本仓库 `supervised-installer`，负责把一套 Debian 12 系统变成 Supervised 宿主。安装分四步：
...
- **数据目录**：默认 `/var/lib/homeassistant`，可用环境变量 `DATA_SHARE` 自定义。
- **支持机型**：列表有限——generic-x86-64、qemux86-64、qemuarm-64、odroid-c2 / c4 / n2、khadas-vim3、raspberrypi3-64 / 4-64 / 5-64 等。注意树莓派条目只认 64 位，且必须跑纯 Debian。

> [!warning] 弃用后的安装器
> `supervised-installer` 仓库顶部已标注「unsupported with HA OS 2025.12.0」——即该安装器对 2025.12 之后的 HA 版本不再提供支持。照抄上面命令之前，先确认你接受「无人维护」的现实。

## 5.4 弃用时间线与现状

官方对 Supervised（连同 Core、32 位系统）的弃用，有一条清晰的时间线：
...
早在弃用之前，社区对 Supervised 的评价就是 **「less supported (and liked)」**——处于受支持边缘地带，选它等于自负维护责任。弃用公告之后这个立场更明确：多数老用户的实际选择是「Proxmox 上跑 HAOS VM，杂项服务用独立 LXC/VM 跑，不塞进 HA」。社区并不是讨厌 Supervised 的功能，而是厌倦了它与宿主系统之间脆弱的耦合。
...
> BYPASS_OS_CHECK 的记载主要来自社区指南，**官方文档不背书、不保证、不负责**。目前能确认的事实只有两点：一是这个变量确实被社区多次提及；二是即使绕过成功，系统也几乎必然被判 Unsupported / Unhealthy，官方支持随之失效。至于它在某个具体版本怎么生效、会不会被新版本移除，**没有官方口径**——任何声称「详细教程」的帖子都应视为社区经验，需自行验证。
...
理论上，Supervised 是「功能完整性」与「灵活性」两条权衡轴上的理想折中：它有 HAOS 的 Supervisor / Add-on / Thread / Z-Wave，又有 Container 那样的宿主控制权。但它的成立前提——宿主必须专用于 HA、仅支持纯 Debian、任何改动都可能触发 Unsupported——让这个折中名存实亡：真把宿主「专用」了，与独占一台 HAOS 没有本质区别；真在共享宿主上跑，又必然触碰 Unsupported 红线。
...
三种方式的机制都已拆完，下一章把它们收拢成一张选型决策树。

---

> [[Home Assistant 三种部署方式对比与选型|⬅ 返回索引]]　·　[[04_DockerContainer详解|上一篇]]　·　[[06_选型决策树与建议|下一篇]]
