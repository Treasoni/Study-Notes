...
直接把 CLAUDE.md 放在项目根目录，子文件夹不放 CLAUDE.md。

**运作方式**：
1. Claude 启动时读取根目录的 CLAUDE.md
2. 在根目录 CLAUDE.md 中定义所有 rules、phases 和路径配置
3. 在指令中注明：skills 和 agent 在 `./core/` 目录下，生成内容输出到根目录

**改造建议**：把子文件夹里的 CLAUDE.md 移出来放到根目录，并在文档里补充一句：
