...
在使用 Claude Code 执行多阶段工作流时，模型有时会跳过人工确认环节，直接推进到下一步。这不是架构设计问题，而是 LLM 底层的 "Prompt Drift"（提示词漂移）现象。
...
**原理**：模型在生成这段文本时，实际上是在对自己进行心理暗示，强化"暂停"状态。
...
**原理**：每次调用技能时重新读取刹车指令，确保防线不因上下文稀释而失效。
