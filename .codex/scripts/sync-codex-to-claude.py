...
def transform_text(path: Path) -> None:
    ...
    try:
        ...
        return
    transformed = original.replace(".codex", ".codex").replace("Codex", "Codex")
...
def expected_mirror(root: Path, temporary_root: Path) -> Path:
    expected_root = temporary_root / ".codex"
    actual_root = root / ".codex"
...
def main() -> int:
    ...
    try:
        if not args.check:
            apply_sync(root)
            print("Synced portable Codex config to Codex config.")
            return 0
        ...
        with tempfile.TemporaryDirectory(prefix="study-system-claude-check.") as temporary_dir:
            ...
            differences = differing_paths(expected, root / ".codex")
