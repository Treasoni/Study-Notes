#!/usr/bin/env bash
# Install and audit prompt-cache conventions for Codex and Codex projects.
...
scan_prompts() {
  ...
  for prompt_dir in "$root/prompts" "$root/.co""dex/prompts" "$root/.codex/prompts"; do
...
if [ "$MODE" = "apply" ]; then
  ...
  if platform_enabled claude; then
    install_rule ".codex" "CLAUDE.md" ".codex/rules/common/prompt-cache.md"
  fi
  install_observability
