...
.codex/scripts/todo-state.sh "$WORKFLOW_STATE_FILE" start P4
```

**After Each Chapter Completion:**
- Do NOT edit the workflow state file yourself. Parallel writers editing the same file race and corrupt the chapter checklist order; the orchestrator (parent) collects completion receipts and updates the checklist centrally via `.codex/scripts/todo-state.sh`.
...
.codex/scripts/todo-state.sh "$WORKFLOW_STATE_FILE" complete P4
```
