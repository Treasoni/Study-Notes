...
- **检查项**:
  ...
  - [ ] 第 3 章已写完并确认
  - [ ] ...（根据实际章节数添加）
  - [ ] 并行写作 ≥2 章时：已冻结跨章口径表并写进每个 dispatch（见下「并行写作的跨章口径」）
  - [ ] 全部章节交付后：已对「可照抄的配置块」做跨章 grep 比对
- **输出文件**: `chapters/{N}_{章节名}.md`
- **状态**: [P4] ⬜ 未开始

#### 并行写作的跨章口径（阶段 4）

并行写作只隔离了「不读其他章的文件」，**没有**隔离「跨章共享的术语与字段名」；各章都忠于自己手边的来源，合起来却可能互相矛盾（同一设置在新旧来源里有两种写法，例如 `method` 与 `network`）。这是并行换来的代价，必须显式补偿：

1. **派发前**产出一份跨章口径表（字段名、术语、命名风格），写进**每个** chapter-writer dispatch 的 Parameters；口径表缺失就不并发。
2. **交付后**对所有「可照抄的配置块」做一次跨章比对，例如：
   ```bash
   # 同一配置键在各章出现了几种取值：命中多行同名键就要人工统一
   grep -rhoE '^[[:space:]]+[a-z_]+:.*$' "${WORKSPACE_PATH:-./workspace}/${PROJECT_SLUG}/chapters/" \
     | sort | uniq -c | sort -rn | head -20
   ```
3. 统一时**不要发明等价关系**：「两者指同一设置」这类无来源可引的断言必须显式标为**推断**。
...
状态通过 `[PN]` 阶段标记和 YAML frontmatter 共同记录。每次阶段切换都调用 `.codex/scripts/todo-state.sh`，避免各 skill 手写 `sed` 导致状态漂移：
...
```bash
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" start P2
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" mode P2 freeform "用户选择随性模式"
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" confirm P2 "用户确认研究方向"
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" complete P2
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" start P3
.codex/scripts/todo-state.sh "$RUN_STATE_FILE" skip P3 "随性模式不生成大纲"
```
...
2. 调用状态脚本将运行模式持久化为 `freeform`，再确认并完成阶段 2：
   ```bash
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" mode P2 freeform "用户选择随性模式"
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" confirm P2 "用户确认直接组装"
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" complete P2
   ```
3. 依次启动并跳过阶段 3 和阶段 4；脚本会把选择写入方向调整记录和跳过记录：
   ...
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" start P3
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" skip P3 "随性模式不生成大纲"
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" start P4
   .codex/scripts/todo-state.sh "$RUN_STATE_FILE" skip P4 "随性模式不逐章写作"
   ```
