---
url: "https://hermes-agent.nousresearch.com/docs/user-guide/docker"
title: "Hermes Docker Setup | Hermes Agent"
...
See the [Where the logs go](https://hermes-agent.nousresearch.com/docs/user-guide/docker#where-the-logs-go) section below for the full routing map (per-profile gateways, dashboard, boot reconciler, container-wide `docker logs`).
...
-eAPI_SERVER_ENABLED=true \




-eAPI_SERVER_HOST=0.0.0.0 \




-eAPI_SERVER_KEY="$(openssl rand -hex32)"\




-eAPI_SERVER_CORS_ORIGINS='*'\
...
The dashboard is supervised by s6 — if it crashes, `s6-supervise` restarts it automatically after a short backoff. Dashboard stdout/stderr is forwarded to `docker logs <container>` (no prefix; the gateway's own output now lives in a per-profile s6-log file — see [Where the logs go](https://hermes-agent.nousresearch.com/docs/user-guide/docker#where-the-logs-go) below — so the two streams don't clash).  
| Environment variable  | Description  | Default  |  
| --- | --- | --- |  
| `HERMES_DASHBOARD`  | Set to `1` (or `true` / `yes`) to enable the supervised dashboard service  | _(unset — service is registered but stays down)_  |  
...
| `HERMES_DASHBOARD_INSECURE`  |  **Deprecated / no-op.** Formerly bypassed the auth gate; as of the June 2026 hardening it no longer disables authentication. A non-loopback bind always requires an auth provider  | _(ignored — configure a provider instead)_  |  
...
### Immutable install tree[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#immutable-install-tree "Direct link to Immutable install tree")
...
dockerexec hermes hermes profile create coder
...
dockerexec hermes hermes -p coder gateway start




dockerexec hermes hermes -p coder gateway stop




dockerexec hermes hermes -p coder gateway restart
...
dockerexec hermes hermes -p coder gateway status
...
dockerexec hermes hermes profile delete coder
...
### Reaching more than one profile from outside the container[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#reaching-more-than-one-profile-from-outside-the-container "Direct link to Reaching more than one profile from outside the container")
...
dockerexec hermes hermes profile create work
...
dockerexec hermes hermes -p work gateway restart
...
### Why one container with many profiles, not many containers[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#why-one-container-with-many-profiles-not-many-containers "Direct link to Why one container with many profiles, not many containers")
...
### When you DO want a separate container[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#when-you-do-want-a-separate-container "Direct link to When you DO want a separate container")
...
The warning from [Persistent volumes](https://hermes-agent.nousresearch.com/docs/user-guide/docker#persistent-volumes) still applies: never point two containers at the same `~/.hermes` directory simultaneously. The s6 supervisor inside each container manages its own profile set; cross-container sharing of a data volume corrupts session files and memory stores.
## Where the logs go[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#where-the-logs-go "Direct link to Where the logs go")
...
Start with `docker compose up -d` and view logs with `docker compose logs -f`. The supervised gateway's stdout is also tee'd to `${HERMES_HOME}/logs/gateways/<profile>/current` on the volume — see [Where the logs go](https://hermes-agent.nousresearch.com/docs/user-guide/docker#where-the-logs-go) for the full routing map.
...
exportHERMES_UID="$(id-u)"




exportHERMES_GID="$(id-g)"
...
dockerexec hermes /opt/hermes/.venv/bin/python -c"import sounddevice as sd; print(sd.query_devices())"
...
The official image is based on `debian:13.4` and includes:
  ...
  * as PID 1 (replaces the older `tini`) — supervises the dashboard and per-profile gateways with auto-restart on crash, reaps zombie subprocesses, and forwards signals.
...
###  `docker exec` automatically drops to the `hermes` user[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#docker-exec-automatically-drops-to-the-hermes-user "Direct link to docker-exec-automatically-drops-to-the-hermes-user")
...
dockerexec-eHERMES_DOCKER_EXEC_AS_ROOT=1 hermes cmd
...
Each profile created with `hermes profile create <name>` automatically gets an s6-supervised gateway service registered at `/run/service/gateway-<name>/`, with state-persistent auto-restart across container restarts. See [Multi-profile support](https://hermes-agent.nousresearch.com/docs/user-guide/docker#multi-profile-support) above for the user-facing workflow and the lifecycle commands.
**Supervision benefits over the pre-s6 image:**
  ...
  * Per-profile gateway logs persist under `$HERMES_HOME/logs/gateways/<profile>/current` (rotated by `s6-log`), and the reconciler's actions are appended to `$HERMES_HOME/logs/container-boot.log` per boot. See [Where the logs go](https://hermes-agent.nousresearch.com/docs/user-guide/docker#where-the-logs-go) for the full routing map.
...
`docker exec hermes <cmd>` automatically drops to UID 10000 too — see [`docker exec` automatically drops to the `hermes` user](https://hermes-agent.nousresearch.com/docs/user-guide/docker#docker-exec-automatically-drops-to-the-hermes-user) for details and the per-invocation opt-out.
### "Permission denied" on every `docker exec` (install dir locked to 0700)[​](https://hermes-agent.nousresearch.com/docs/user-guide/docker#permission-denied-on-every-docker-exec-install-dir-locked-to-0700 "Direct link to permission-denied-on-every-docker-exec-install-dir-locked-to-0700")
...
dockerexec-u root hermes chmod 0755 /opt/hermes
...
docker logs --tail50 hermes          # Recent logs
...
```
  ...
  * [Persistent volumes](https://hermes-agent.nousresearch.com/docs/user-guide/docker#persistent-volumes)
    * [Immutable install tree](https://hermes-agent.nousresearch.com/docs/user-guide/docker#immutable-install-tree)
  * [Multi-profile support](https://hermes-agent.nousresearch.com/docs/user-guide/docker#multi-profile-support)
    * [Reaching more than one profile from outside the container](https://hermes-agent.nousresearch.com/docs/user-guide/docker#reaching-more-than-one-profile-from-outside-the-container)
    * [Why one container with many profiles, not many containers](https://hermes-agent.nousresearch.com/docs/user-guide/docker#why-one-container-with-many-profiles-not-many-containers)
    * [When you DO want a separate container](https://hermes-agent.nousresearch.com/docs/user-guide/docker#when-you-do-want-a-separate-container)
  ...
  * [What the Dockerfile does](https://hermes-agent.nousresearch.com/docs/user-guide/docker#what-the-dockerfile-does)
    * [`docker exec` automatically drops to the `hermes` user](https://hermes-agent.nousresearch.com/docs/user-guide/docker#docker-exec-automatically-drops-to-the-hermes-user)
  ...
  * [Troubleshooting](https://hermes-agent.nousresearch.com/docs/user-guide/docker#troubleshooting)
    ...
    * ["Permission denied" on every `docker exec` (install dir locked to 0700)](https://hermes-agent.nousresearch.com/docs/user-guide/docker#permission-denied-on-every-docker-exec-install-dir-locked-to-0700)
