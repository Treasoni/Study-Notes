...
[Skip to main content](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#__docusaurus_skipToContent_fallback)
On this page
...
Don't point two agent processes at the same Hermes home directory. Memory writes are automatic and load back into the system prompt at session start, so two writers sharing one home will compound each other's entries into state neither of them (nor you) authored. Memory is scoped per [profile](https://hermes-agent.nousresearch.com/docs/user-guide/profiles) by design — give a second agent its own profile, and if they need shared memory, use an [external memory provider](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers) instead.
info
...
### Substring Matching[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#substring-matching "Direct link to Substring Matching")
...
## Two Targets Explained[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#two-targets-explained "Direct link to Two Targets Explained")
###  `memory` — Agent's Personal Notes[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#memory--agents-personal-notes "Direct link to memory--agents-personal-notes")
...
###  `user` — User Profile[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#user--user-profile "Direct link to user--user-profile")
...
## What to Save vs Skip[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#what-to-save-vs-skip "Direct link to What to Save vs Skip")
### Save These (Proactively)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#save-these-proactively "Direct link to Save These \(Proactively\)")
...
### Skip These[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#skip-these "Direct link to Skip These")
...
## Capacity Management[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#capacity-management "Direct link to Capacity Management")
...
### What Happens When Memory is Full[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#what-happens-when-memory-is-full "Direct link to What Happens When Memory is Full")
...
### Practical Examples of Good Memory Entries[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#practical-examples-of-good-memory-entries "Direct link to Practical Examples of Good Memory Entries")
...
## Duplicate Prevention[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#duplicate-prevention "Direct link to Duplicate Prevention")
...
## Security Scanning[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#security-scanning "Direct link to Security Scanning")
...
## Session Search[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#session-search "Direct link to Session Search")
...
See [Session Search Tool](https://hermes-agent.nousresearch.com/docs/user-guide/sessions#session-search-tool) for the three calling shapes (discovery / scroll / browse) and the response format.
### session_search vs memory[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#session_search-vs-memory "Direct link to session_search vs memory")  
...
## Learning Journey (`/journey`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#learning-journey-journey "Direct link to learning-journey-journey")
...
## Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#configuration "Direct link to Configuration")
...
## Controlling memory writes (`write_approval`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#controlling-memory-writes-write_approval "Direct link to controlling-memory-writes-write_approval")
...
## Background review notifications (`display.memory_notifications`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#background-review-notifications-displaymemory_notifications "Direct link to background-review-notifications-displaymemory_notifications")
...
## Running the review on a cheaper model (`auxiliary.background_review`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#running-the-review-on-a-cheaper-model-auxiliarybackground_review "Direct link to running-the-review-on-a-cheaper-model-auxiliarybackground_review")
...
### Disabling automatic reviews (`enabled`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#disabling-automatic-reviews-enabled "Direct link to disabling-automatic-reviews-enabled")
...
## Controlling skill writes (`skills.write_approval`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#controlling-skill-writes-skillswrite_approval "Direct link to controlling-skill-writes-skillswrite_approval")
...
On a messaging platform, approve a skill from its gist + metadata, or open `/skills diff` on the CLI / dashboard / the staged file under `~/.hermes/pending/skills/<id>.json` when you want to read the whole change. Full details in [Gating agent skill writes](https://hermes-agent.nousresearch.com/docs/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval).
## External Memory Providers[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#external-memory-providers "Direct link to External Memory Providers")
...
See the [Memory Providers](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers) guide for full details on each provider, setup instructions, and comparison.
  * [How Memory Appears in the System Prompt](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#how-memory-appears-in-the-system-prompt)
  * [Memory Tool Actions](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#memory-tool-actions)
  * [Two Targets Explained](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#two-targets-explained)
    * [`memory` — Agent's Personal Notes](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#memory--agents-personal-notes)
    * [`user` — User Profile](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#user--user-profile)
  * [What to Save vs Skip](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#what-to-save-vs-skip)
    * [Save These (Proactively)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#save-these-proactively)
  * [Capacity Management](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#capacity-management)
    * [What Happens When Memory is Full](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#what-happens-when-memory-is-full)
    * [Practical Examples of Good Memory Entries](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#practical-examples-of-good-memory-entries)
  * [Duplicate Prevention](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#duplicate-prevention)
  * [Session Search](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#session-search)
    * [session_search vs memory](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#session_search-vs-memory)
  * [Learning Journey (`/journey`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#learning-journey-journey)
  * [Controlling memory writes (`write_approval`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#controlling-memory-writes-write_approval)
  * [Background review notifications (`display.memory_notifications`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#background-review-notifications-displaymemory_notifications)
  * [Running the review on a cheaper model (`auxiliary.background_review`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#running-the-review-on-a-cheaper-model-auxiliarybackground_review)
    * [Disabling automatic reviews (`enabled`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#disabling-automatic-reviews-enabled)
  * [Controlling skill writes (`skills.write_approval`)](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#controlling-skill-writes-skillswrite_approval)
  * [External Memory Providers](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#external-memory-providers)
