...
The auto-injected dialectic also scales its reasoning level by query length (longer query → deeper reasoning, capped at `reasoningLevelCap`); see [Query-Adaptive Reasoning Level](https://hermes-agent.nousresearch.com/docs/user-guide/features/honcho#query-adaptive-reasoning-level).
**Setup Wizard:**
...
| `dialecticCadence`  | Minimum turns between `peer.chat()` LLM calls. Recommended 1–5. Only applies to `hybrid`/`context` modes  |  
| `dialecticDepth`  | Number of `.chat()` passes per dialectic invocation. Clamped 1–3. Pass 0: cold/warm prompt, pass 1: self-audit, pass 2: reconciliation  |  
...
### Gateway identity mapping[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers#gateway-identity-mapping "Direct link to Gateway identity mapping")
The peer model above covers CLI, TUI, and desktop sessions, where every conversation resolves to `peerName`. The [gateway](https://hermes-agent.nousresearch.com/docs/developer-guide/gateway-internals) adds a second axis: users arrive with platform-native runtime IDs (Telegram UID, Discord snowflake, Slack user), and three keys decide which peer each ID resolves to.  
| Key  | Effect  |  
| --- | --- |  
...
Off-gateway these keys do nothing. `hermes memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](https://hermes-agent.nousresearch.com/docs/user-guide/features/honcho#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
| `max_recall_results`  | Max recalled items to format into context  |  
| `profile_frequency`  | Include profile facts on first turn and every N turns  |  
...
**Support:** [Discord](https://supermemory.link/discord) · support@supermemory.com
### Memori[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers#memori "Direct link to Memori")
...
See the [Developer Guide: Memory Provider Plugins](https://hermes-agent.nousresearch.com/docs/developer-guide/memory-provider-plugin) for how to create your own.
  * [Available Providers](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers#available-providers)
    ...
    * [Gateway identity mapping](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers#gateway-identity-mapping)
