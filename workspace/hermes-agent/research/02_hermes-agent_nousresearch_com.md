...
Platform Support
For the full platform support matrix (which OSes, distribution methods, and platform-gated features are supported), see .
...
You don't need to rebuild your setup from scratch. Restore a full backup with `hermes import` (see [Exporting Hermes to another machine](https://hermes-agent.nousresearch.com/docs/reference/faq#exporting-hermes-to-another-machine)), or bring over a single agent with `hermes profile import` (see [Moving a single profile to another machine](https://hermes-agent.nousresearch.com/docs/reference/faq#moving-a-single-profile-to-another-machine)). Note that a profile export excludes credentials by design, so an export alone is not a full backup — [`hermes backup` vs `hermes profile export`](https://hermes-agent.nousresearch.com/docs/reference/faq#hermes-backup-vs-hermes-profile-export) explains which to use.
...
Nix users
Nix is **no longer an explicitly supported install path** (best-effort only). If you already use Nix (on NixOS, macOS, or Linux), there's a dedicated setup path with a Nix flake, declarative NixOS module, and optional container mode. See the guide.
...
The installer also pre-installs [`cua-driver`](https://hermes-agent.nousresearch.com/docs/user-guide/features/computer-use) so the Computer Use toolset works the moment you enable it; pass `--skip-computer-use` to opt out (it will then install on demand when you enable the tool).
...
sudoln-s /home/hermes/.hermes/hermes-agent/venv/bin/hermes /usr/local/bin/hermes
...
See [Messaging Gateway](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/) for the service setup itself.
