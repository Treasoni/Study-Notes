...
> [!tip] 大白话
> 把 MCP 当成「AI 的 USB 接口」：数据库、GitHub、Sentry 这些外部工具都按同一套协议插上就能用，Claude 不需要为每个工具单独学一套「方言」。
...
> [!tip] 大白话
> 本地工具用 **stdio**（Claude 直接拉起本地进程，最快）；云端服务用 **HTTP**（通过网址连接，当前主流）。一句话记住：**本地 stdio，云端 HTTP**；SSE 已废弃，新项目别再用。
...
> [!tip] macOS 密钥链超时导致 OAuth 401（v2.1.225+ 已修复）
> 早期版本在 macOS 上可能因密钥链（keychain）访问超时，导致 MCP OAuth 令牌读写失败，表现为连续突发的 401 授权错误。升级到 v2.1.225+ 即自动修复，无需手动处理。
...
> [!warning] 安全行为变化（2026-08）
> `claude mcp list` / `claude mcp get` 不再自动「自批准」项目 `.mcp.json` 中自带的服务器——列出或查询配置**不等于**放行使用，服务器首次实际调用时仍需明确批准。
...
> [!warning] 外部插件 MCP 需安装同意
> 外部插件提供的 MCP 服务器在加载时要求明确的安装同意；只有项目设置明确启用该插件时，其 MCP 服务器才会被加载并可用。
...
从 **v2.1.203** 起，会话工作目录会自动加入 MCP 的 `roots/list`，并在目录变化时发送 `notifications/roots/list_changed`，让 MCP 服务器始终知道当前会话的工作目录。

### Capability discovery 自动重试（v2.1.191+）

MCP 服务器的能力探测请求（`tools/list`、`prompts/list`、`resources/list`）对瞬时网络错误会自动重试，降低临时断连导致的连接/启动失败率。

### 长耗时工具调用自动转后台

超过 **2 分钟**的 MCP 工具调用会自动转为后台任务，避免阻塞当前会话。触发阈值可用环境变量 `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` 调整（单位：毫秒）。
...
- ❌ 令牌过期
- ❌ 权限不足
- ❌ macOS 连续 401（密钥链超时）→ 升级到 v2.1.225+ 已修复
...
[[人工智能重要的六大概念体系]] | [[Claude Code Subagents 完整指南]] | [[如何使用Claude code]]
...
| 日期 | 变更摘要 |
|------|----------|
| 2026-08-10 | 同步 2026-08 MCP 行为：capability discovery（`tools/list`、`prompts/list`、`resources/list`）对瞬时网络错误自动重试（v2.1.191+）；会话工作目录加入 MCP `roots/list` 并通知 `notifications/roots/list_changed`（v2.1.203+）；超过 2 分钟的 MCP 工具调用自动转后台（可用 `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` 调整）；macOS 密钥链超时导致的 OAuth 401 突发已修复（v2.1.225+）；`claude mcp list`/`get` 不再自批准 `.mcp.json` 服务器、外部插件 MCP 需安装同意；核心概念补充「大白话」callout；修正重复章节编号（CLI 命令速查 12→13）。 |
