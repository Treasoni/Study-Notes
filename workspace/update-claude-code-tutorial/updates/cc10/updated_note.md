...
> **通俗比喻**：就像一个项目经理（主 Agent）带领一群专业顾问（Subagents）—— 每个顾问有自己的专业领域、独立的工作空间，完成后向经理汇报结果。
...
> [!tip] 大白话
> Subagents 就是给 Claude 配的"专科医生"。主对话像全科门诊，能看大部分病，但每次会诊都会占用诊室（上下文）；遇到专科问题就派给眼科、骨科，各管各的，看完把检查报告拿回来。2026-08 起这些"会诊"默认在后台进行，主对话不用干等。
...
旧的 in-session 子代理（会话内直接派生的临时子代理）已改为 **`/subtask`** 命令（v2.1.212+）：

```
/subtask 让 test-runner 检查登录模块的失败用例
```

- `/subtask` 在会话内派生一个临时子代理，结果返回主对话。
- 已完成后台代理可通过 `/tasks` 查看。
- 与 `@-mention` 调用预设 Agent 不同，`/subtask` 更接近"就地开一个子代理干活"。

> [!tip] 大白话
> `/subtask` 就是"临时找个帮手，干完就走"。不想专门建一个长期 Subagent 文件，又想快速把一件事丢给独立上下文的子代理去办时，用它最顺手。

> [!info] 📚 来源
> - [GitHub - claude-howto Subagents Guide](https://github.com/luongnv89/claude-howto/tree/main/04-subagents) - Using Subagents
> - SB-06 / SB-08：`/subtask` 取代旧 in-session 子代理，`/tasks` 保留已完成代理
...
> [!tip] 2026 更新
> 自 Claude Code v2.1.198+，Subagents **默认在后台运行**，无需显式设置 `background: true`；新版安装后会自动把已有 Subagent 升级为后台代理。
...
Subagents 默认最多 **20 个**并发运行，可用环境变量调整：
...
预算与文本透传（CLI 标志，v2.1.208+）：

```bash
# 达到预算上限时自动停止后台子代理
claude --max-budget-usd 10

# 把子代理产生的文本透传到 stream-json 输出，便于外部脚本消费
claude --output-format stream-json --forward-subagent-text
```

> [!tip] 大白话
> "默认后台运行"的意思是：Claude 把子任务扔到后台去干，主对话不用干等，你可以继续聊天或派新任务。并发上限 20 是防止一次开太多子代理把资源占满；`--max-budget-usd` 是给"烧钱"设的保险丝，到顶自动踩刹车。

> [!info] 📚 来源
> - [GitHub - claude-howto Subagents Guide](https://github.com/luongnv89/claude-howto/tree/main/04-subagents) - Background Subagents
> - SB-05 / SB-06：CLI 标志与环境变量、默认后台运行与并发规则
...
worktree 隔离的 Subagent **不能对主 checkout 执行破坏性 git 命令**（v2.1.216+），包括通过 `git -C`、`--git-dir` 或 `GIT_DIR` 环境变量把命令重定向到主仓库的尝试。另外，`EnterWorktree` 若要进入 `.claude/worktrees/` 之外的目录，会先要求确认（v2.1.222+）。

> [!warning] 安全边界
> 隔离不是完整沙箱：它阻止的是对主仓库的 git 破坏性操作，Subagent 在其 worktree 范围内仍可正常读写。不要把需要访问主仓库敏感内容的 hooks 配置进隔离 worktree。

> [!info] 📚 来源
> - [GitHub - claude-howto Subagents Guide](https://github.com/luongnv89/claude-howto/tree/main/04-subagents) - Worktree Isolation
> - SB-07：子代理文件隔离与安全（v2.1.216 / v2.1.222）
...
默认情况下，Subagents 可以再生成 Subagents，最多嵌套 **3 层**（v2.1.219 起恢复；v2.1.217 曾默认禁用嵌套）。
...
> [!note] 每会话 spawn 上限已移除
> v2.1.224 起，旧的"每会话最多 200 个 spawn"上限已被移除，子代理数量按并发上限与嵌套深度自然约束。

> [!tip] 大白话
> "嵌套深度"就是"子代理还能往下派几层"。默认 3 层 ≈ 主管→经理→员工，够用也不失控。设成 `1` 等于"子代理干完就汇报，不许再往下派活"，能防止递归失控。
...
> - SB-06：Subagents 嵌套规则（v2.1.217 / v2.1.219 / v2.1.224）
...
- **嵌套生成**：Subagents 默认可嵌套最多 **3 层**（v2.1.219 恢复；v2.1.217 曾默认禁用嵌套）。用 `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1` 禁用嵌套；用 `Agent(agent_type)` 语法限制可生成的 Subagent 类型
...
默认可以，最多嵌套 **3 层**（v2.1.219+；v2.1.217 曾默认禁用嵌套）。如需完全禁止嵌套，设置 `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1`；如需限制只能调用某些类型，使用 `Agent(agent_name)` 语法在 `tools` 字段中明确允许。
...
> [!note] 模型默认值提示
> 表中 Tier 1 的 Opus 4.8 为历史分层示例；2026-08 起默认 Opus 模型已更新（v2.1.219），按当前项目可用模型调整即可。
...
- **2026-08-10**：同步到 2026-08 现状（覆盖 v2.1.193–v2.1.226，来源库 SB-05/SB-06/SB-07）。
  - **默认后台运行**：Subagents 默认在后台运行（v2.1.198+），无需 `background: true`。
  - **并发上限**：默认 20 个并发，`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 调整。
  - **嵌套规则**：v2.1.217 默认禁用嵌套 → v2.1.219 恢复嵌套深度 3；`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1` 禁用嵌套；v2.1.224 移除每会话 200 spawn 上限。
  - **/subtask**：旧 in-session 子代理改为 `/subtask`（v2.1.212+），`/tasks` 保留已完成后台代理。
  - **CLI 与预算**：`--forward-subagent-text` 透传子代理文本到 stream-json；`--max-budget-usd` 达上限自动停止后台子代理。
  - **Worktree 安全**：worktree 隔离子代理不能对主 checkout 执行破坏性 git 命令（含 `git -C`/`--git-dir`/`GIT_DIR` 重定向）。
