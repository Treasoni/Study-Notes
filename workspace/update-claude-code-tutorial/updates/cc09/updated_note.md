...
- **Project Memory** = 团队的"编码规范文档"
- **Auto Memory** = Claude 自动记录的"学习笔记"

> [!tip] 大白话：三层记忆体系
> Claude Code 的记忆可以想成一个"三层抽屉"：
> 1. **CLAUDE.md** = 你自己手写的"项目说明书"——编码规范、架构约定、常用命令，白纸黑字，明码写清楚；
> 2. **Auto Memory** = Claude 自己攒的"工作笔记本"——它发现你的纠错、调试经验、偏好习惯后自动记下来，不用你动手；
> 3. **参考文档** = 用 `@` 引用的"参考资料柜"——README、接口文档、架构图不用复制粘贴，直接引用即可。
> 三层各有分工：说明书保证"按你的规矩办"，笔记本保证"经验不丢"，资料柜保证"信息不重复"。
...
| `/memory` | 列出/编辑记忆文件，开关 Auto Memory | 大量更新、重组内容、开关自动记忆 |
| `/context` | 查看当前会话已加载的记忆文件 | 调试记忆是否生效 |
| `/doctor` | CLAUDE.md 健康体检（v2.1.206+） | 定期维护：裁剪/去重/标记慢 hooks |
...
**功能**：
- 列出用户/项目范围内的 CLAUDE.md、CLAUDE.local.md 与 Auto Memory 文件位置（含尚未创建的文件）
- 在系统默认编辑器中打开选中的记忆文件（GUI 编辑器如 VS Code 会开独立窗口，v2.1.216+ 无需等待关闭即可继续会话）
- 提供对层级中所有记忆文件的访问
- 提供 Auto Memory 开关，并可直接打开 Auto Memory 文件夹
...
- **位置**：`~/.claude/projects/<project>/memory/`（项目目录由 git 仓库路径推导，同一仓库的所有 worktree/子目录共享一个记忆目录）
- **入口点**：`MEMORY.md` 作为记忆目录的**索引**
- **主题文件**：可选的特定主题文件（如 `debugging.md`），从 `MEMORY.md` 链接
- **加载行为**：启动时加载 `MEMORY.md` 前 200 行**或前 25KB**（取先到者）；主题文件不随启动加载，Claude 需要时按需读取

### 目录结构

```
~/.claude/projects/<project>/memory/
├── MEMORY.md              # 入口点/索引（启动时加载前 200 行或 25KB）
...
Auto Memory 需要 **Claude Code v2.1.59 或更高版本**；记忆文件写入时带 YAML frontmatter 的话，会在下次写入时记录 `modified` 时间戳字段（ISO 8601，需 v2.1.214+）。

### 自定义 Auto Memory 目录

```json
// 可从任意 settings 层级读取：用户 / 项目 / 本地 / 策略 / --settings
{
...
> [!note] 注意
> 值必须是**绝对路径**或以 `~/` 开头。`autoMemoryDirectory` 现在可从任意 settings 层级读取（用户、项目、本地、托管策略或 `--settings`）；写在项目级 `.claude/settings.json` / `.claude/settings.local.json` 时，需先接受该文件夹的**工作区信任对话框**才生效。

### 控制 Auto Memory

Auto Memory **默认开启**。官方现行有三种控制方式：

| 方式 | 说明 |
|------|------|
| `/memory` 会话内开关 | 在会话内切换，选择结果写入 `~/.claude/settings.json` 的 `autoMemoryEnabled` |
| 设置项 `autoMemoryEnabled` | 设为 `false` 关闭；用户设置全局关闭，项目设置仅对该项目关闭 |
| 环境变量 `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` | 会话级关闭 |

```bash
# 通过环境变量关闭 Auto Memory
...
> [!warning] 说明
> 官方文档只明确 `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` 用于关闭；旧写法 `=0 强制开启` 未见于官方文档，社区报告它只停用自动记忆写入而非完整功能，建议以 `autoMemoryEnabled` 设置为准。

> [!info] 📚 来源
> - [GitHub - claude-howto Memory Guide](https://github.com/luongnv89/claude-howto/blob/main/02-memory/README.md) - Auto Memory
> - [Memory - Claude Code Docs](https://code.claude.com/docs/en/memory) - Auto Memory 官方文档（2026-08 核对）
...
| **太长** | 单个 CLAUDE.md 目标 200 行以内；过长消耗上下文、降低遵循度 |
| **过度组织** | 策略性使用层级 |
| **忘记更新** | 过时的记忆会导致困惑 |

> [!tip] 用 `/doctor` 做 CLAUDE.md 健康体检（v2.1.206+）
> `/doctor`（别名 `/checkup`）会提议**裁剪已提交的 CLAUDE.md**：删除可由代码库推导的内容（目录结构、依赖清单、架构总览），保留踩坑记录、设计理由和与工具默认不同的约定；还会**合并重复的记忆文件**，并**标记慢 hooks**。CLAUDE.md 编写原则因此多了一条——**保持精简、可由工具推导的内容不写**。定期跑一次 `/doctor`，让记忆文件只留"Claude 自己发现不了的信息"。
...
| **加载时机** | 启动时完整加载 | MEMORY.md 前 200 行或 25KB + 按需加载 |
...
最大支持 **4 层递归导入**（官方现行；旧版文档曾写 5 层）。

### Q: 如何调试 Memory 加载问题？

1. 运行 `/context`，核对 **Memory files** 列表确认记忆文件是否真的加载
2. 使用 `/memory` 打开、编辑或浏览记忆文件
3. 检查文件路径是否正确
4. 验证 YAML frontmatter 格式
5. 查看 Claude Code 日志

### Q: CLAUDE.md 太长或重复怎么办？

- 目标控制在 **200 行以内**；过长会消耗上下文并降低遵循度。
- 可由工具推导的内容（目录结构、依赖清单、架构总览）不必写，`/doctor`（v2.1.206+）会自动提议裁剪。
- 按路径拆分到 `.claude/rules/` 的 `paths:` 规则，触及时才加载；重复的记忆文件交给 `/doctor` 合并。
...
- **2026-08-10**：同步到 2026-08 现状（对照官方 Memory 文档与 2026-07/08 changelog）。
  - CLAUDE.md 最佳实践：单个 CLAUDE.md 目标由"500 行以内"改为官方现行"**200 行以内**"；新增 `/doctor`（v2.1.206+）健康体检说明——裁剪已提交 CLAUDE.md、去重、删除可由代码库推导的内容、合并重复记忆文件、标记慢 hooks。
  - Auto Memory 加载行为：`MEMORY.md` 前 200 行补充 **或 25KB（取先到者）**；明确 `MEMORY.md` 是记忆目录索引。
  - Auto Memory 开关：改为官方现行三种方式——`/memory` 会话内开关、`autoMemoryEnabled` 设置、`CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` 环境变量；删除旧 `=0 强制开启` 描述（未见于官方文档）。
  - `autoMemoryDirectory`：更正为可从任意 settings 层级读取，项目级需接受工作区信任对话框。
  - 导入深度：最大 **5 层 → 4 层**。
  - `/memory` 命令：补充列出记忆位置、开关 Auto Memory、打开 Auto Memory 文件夹；GUI 编辑器 v2.1.216+ 行为。
  - 命令参考新增 `/context`（核对已加载记忆文件）与 `/doctor`；FAQ 调试步骤改用 `/context`。
  - 核心概念新增 `[!tip] 大白话`：三层记忆体系（CLAUDE.md 说明书 + Auto Memory 工作笔记本 + 参考文档资料柜）比喻。
  - 顺带修复 3 处原文件乱码（编码规范/编码规范文档/始终记录错误）。
