...
> [!tip] 大白话
> 斜杠命令就是输入 `/` 开头的「快捷按钮」—— 把高频操作（提交代码、清空对话、审查代码）变成一条短命令，省得反复敲长指令。
...
| **自定义 Skills** | 用户创建 | `/commit`, `/optimize` | 存放在 `.claude/skills/` |
...
| 命令 | 用途 | 说明 |
|------|------|------|
| `/branch [name]` | 分支对话到新会话 | 创建独立分支会话 |
| `/clear` | 清除对话 | 别名：`/reset`, `/new` |
| `/compact [instructions]` | 压缩对话 | 可指定焦点指令 |
| `/export [filename]` | 导出对话 | 导出到文件或剪贴板 |
| `/fork` | 复制当前对话到新后台会话 | 原为 `/branch` 别名，现为独立命令【SB-08】 |
| `/rename [name]` | 重命名会话 | 便于会话管理 |
| `/resume [session]` | 恢复对话 | 别名：`/continue`；agent 视图打开历史会话选择器，以后台会话恢复【SB-08】 |
| `/rewind` | 回滚对话/代码 | 别名：`/checkpoint`；可恢复到 `/clear` 之前的对话【SB-08】 |

> [!tip] 大白话
> `/fork` 就像给当前对话拍一张「快照分身」—— 原对话继续，新对话在后台独立跑，适合想并行推进几个方向时用。
...
| 命令 | 用途 | 说明 |
|------|------|------|
| `/code-review [scope]` | 审查变更代码 | 作为后台子代理运行；复用上次 effort 等级；需手动调用【SB-09】 |
| `/review` | `/code-review` 的别名 | 不再自动运行【SB-09】 |
...
| `/doctor` | 全量环境体检 | 别名：`/checkup`；诊断并修复安装健康、未用 skills/MCP/插件、CLAUDE.md 去重与裁剪、慢 hooks【SB-10】 |
| `/help` | 显示帮助 | 命令帮助信息 |
| `/insights` | 生成会话分析报告 | 使用洞察 |
| `/release-notes` | 查看更新日志 | 版本更新信息 |
| `/stats` | 可视化使用统计 | 每日使用、会话、连续天数 |
| `/status` | 显示状态 | 版本、模型、账户，以及会话类型（interactive / attached / unattended）【SB-10】 |
...
| `/subtask` | 启动 in-session 子代理 | 取代旧逻辑；在会话内开一个子代理处理子任务【SB-08】 |
| `/tasks` | 列出/管理后台任务 | 后台任务管理 |

> [!tip] 大白话
> `/subtask` 就像在对话里临时「叫一个帮手」去独立处理子任务（如并行查资料），不污染主对话的上下文。
...
> [!tip] 交互细节（2026-08）
> - **Slash/Skill 可叠加调用**：可以写 `/skill-a /skill-b do XYZ`，一次最多连续加载 5 个前置 skill【SB-11】。
> - **emoji 短码自动补全**：输入 `:thumbsup:` 这类短码会自动补全为 emoji【SB-11】；由 `emojiCompletionEnabled` 设置控制【SB-12】。
...
| `model` | 指定模型 | 继承 |
| `disable-model-invocation` | 仅用户调用（禁止模型自动调用，此时 Claude 会请你手动运行） | `false` |
...
> [!tip] Slash/Skill 叠加调用
> 可以一次写多个 `/skill` 叠加执行，最多 5 个前置 skill【SB-11】。例如 `/simplify /code-review` 组合使用。
...
| 版本 | 变更 |
|------|------|
| v2.1.212 | `/fork` 复制当前对话到新后台会话；旧 in-session 子代理改为 `/subtask`；`/resume` agent 视图打开历史会话选择器并以后台会话恢复；`/rewind` 可恢复到 `/clear` 之前【SB-08】 |
| v2.1.205/206/210 | `/doctor` 全量环境体检，`/checkup` 为别名；`/status` 显示会话类型（interactive / attached / unattended）【SB-10】 |
| v2.1.199 | Slash/Skill 可叠加调用（最多 5 个前置 skill）；emoji 短码自动补全【SB-11】 |
| v2.1.77 | `/fork` 重命名为 `/branch`，保留 `/fork` 作为别名（后 `/fork` 又恢复为独立命令） |
| v2.1.73 | `/output-style` 废弃 |
| - | `/review` 现在是 `/code-review` 的别名；Claude 不再自行运行 `/verify`、`/code-review`，需手动调用；`/code-review` 作为后台子代理运行并复用上次 effort 等级【SB-09】 |
...
| [[Claude Code Subagents 完整指南]] | Subagent 独立上下文，Skill 共享上下文；`/subtask` 在会话内启动子代理 |
...
| 日期 | 变更摘要 |
|------|----------|
| 2026-08-10 | 同步 2026-08 Slash Commands：新增 `/fork`（复制对话到新后台会话）、`/subtask`（in-session 子代理）、`/code-review`（后台子代理运行，`/review` 为其别名且不再自动运行）、`/doctor`（全量体检，`/checkup` 为别名）；`/status` 显示会话类型；`/rewind` 可恢复到 `/clear` 之前；内置命令表按字母序整理；补充 Slash/Skill 叠加（最多 5 个）与 emoji 短码自动补全；修正 `/review` 自定义示例。 |
