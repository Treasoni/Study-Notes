...
> [!note] 默认模型（2026-08）
> - **Claude Sonnet 5** 为 Claude Code 默认模型（原生 1M 上下文，促销至 2026-08-31）。
> - **Claude Opus 5**（`claude-opus-5`）为默认 Opus 模型（1M 上下文）。
> - **Bedrock / Vertex / Claude Platform on AWS** 默认模型仍为 **Opus 4.8**。

> [!tip] 大白话
> 现在默认"员工"是 Sonnet 5（1M 长记忆），需要最强推理时切到 Opus 5；如果通过 AWS Bedrock / Vertex 接入，默认仍是 Opus 4.8。

| 模型 | 思考模式 |
|------|----------|
| **Opus 5**（默认 Opus） | 自适应推理，1M 上下文，支持 effort 级别：`standard`, `high`, `xhigh`, `max` |
| **Sonnet 5**（默认模型） | 自适应推理，1M 上下文，支持 effort 级别：`standard`, `high`, `xhigh`, `max` |
...
> [!note] 正式发布
> Auto Mode 于 2026 年 Q2 正式发布（GA），Pro 计划即可使用。2026 年 7 月起已支持 Bedrock、Vertex AI 和 Foundry，且第三方平台**无需 opt-in**（不再需要 `CLAUDE_CODE_ENABLE_AUTO_MODE`；如需关闭可用 `disableAutoMode` 设置）。
...
| 篡改会话 transcript | 修改或删除会话记录文件 |
| 命令替换中的灾难性删除 | `$(rm -rf ~)`、反引号、`<(…)` 中的破坏性命令（即使 `--dangerously-skip-permissions` 也会提示） |
...
> [!warning] 行为变化（2026-08）
> - Auto Mode 会**阻止篡改会话 transcript 文件**。
> - 对 `$(…)` / 反引号 / `<(…)` 中的灾难性删除（如 `rm -rf ~`），**即使使用 `--dangerously-skip-permissions` 也会提示确认**。
> - **AskUserQuestion 对话框默认不再自动继续**（可在 `/config` 中设置 idle timeout）。
...
> [!tip] 🎯 一句话定义
> **`/fork` 把当前对话复制到一个新的后台会话，方便从同一上下文并行探索不同方案。**
...
- 新会话在**后台**运行，原会话不受影响
- 适合从同一上下文分叉出多个实验方向（例如分别尝试不同实现方案）

> [!tip] 大白话
> 就像把一份文档复制出多个副本，每个副本可以各自往下写，互不干扰。
...
> [!tip] 🎯 一句话定义
> **子代理是并行执行子任务的后台 Agent，默认在后台运行，把大任务拆解给多个独立上下文的 Agent。**

### 默认行为（2026-08）

- **默认后台运行**：子代理不再阻塞当前对话，完成后通知你
- **并发上限**：默认 **20**（可用 `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` 调整）
- **嵌套深度**：默认 **3** 层；`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1` 可禁用嵌套
- **Spawn 上限**：v2.1.224 起移除每会话 200 个 spawn 上限

### 与旧版差异

- **`/subtask`** 取代旧的 in-session 子代理
- **`/tasks`** 保留已完成后台代理，可查看/恢复
- **`--forward-subagent-text`**：stream-json 模式下透传子代理文本

### 隔离与安全

- **Worktree 隔离**：`EnterWorktree` 创建隔离工作树；隔离的子代理**不能对主 checkout 执行破坏性 git 命令**（含 `git -C` / `--git-dir` 重定向）
- 对 `.claude/worktrees/` 之外的工作树，`EnterWorktree` 会先要求确认

> [!tip] 大白话
> 子代理就像同时雇了 20 个实习生（默认并发上限），各自独立干活。他们只能在各自的"工位"（worktree）上改东西，不能跑回主线仓库乱删分支。
...
| 模式 | 行为 |
|------|------|
| `manual` | 只读文件；其他操作需提示确认（原 `default`，v2.1.200 改名） |
...
> [!tip] 🎯 一句话定义
> **屏幕阅读器模式把终端界面转成线性纯文本，供 VoiceOver / NVDA 等读屏工具使用。**
...
- 终端界面转成**线性纯文本**，适配 VoiceOver / NVDA
- 支持**删除操作的屏幕播报**

> [!tip] 大白话
> 就是给视障用户开"朗读模式"——把密密麻麻的终端界面变成一行行能读出来的文字。
...
> [!note] 新增能力（2026-08）
> - **`sandbox.filesystem.disabled: true`**：跳过文件系统隔离，但仍保留网络出口控制。
> - **凭据 mask 模式**：`credentialMasking.mode: "mask"` 对凭据文件做掩码处理。
...
# 无障碍
export CLAUDE_AX_SCREEN_READER=0
...
| 日期 | 变更摘要 |
|------|----------|
| 2026-08-10 | 模型默认值更新（Sonnet 5 默认、Opus 5 默认 Opus、Bedrock/Vertex/AWS 默认 Opus 4.8）；Auto Mode 第三方平台免 opt-in + transcript/灾难性删除保护 + AskUserQuestion 行为；新增 Subagents、Accessibility、/fork（Session Management）章节；沙盒新增 `filesystem.disabled` 与凭据 `mode: "mask"`；桌面端内置浏览器；权限模式 `default`→`manual`；Mermaid 图补 Accessibility 节点。 |
