...
[Skip to main content](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#__docusaurus_skipToContent_fallback)
On this page
Hermes has four hook systems that run custom code at key lifecycle points:  
| System  | Registered via  | Runs in  | Use case  |  
| --- | --- | --- | --- |  
|   |  `HOOK.yaml` + `handler.py` in `~/.hermes/hooks/`  | Gateway only  | Logging, alerts, webhooks  |  
|   |  `ctx.register_hook()` in a [plugin](https://hermes-agent.nousresearch.com/docs/user-guide/features/plugins)  | CLI + Gateway  | Tool interception, metrics, guardrails  |  
|   |  `hooks:` block in `~/.hermes/config.yaml` pointing at shell scripts  | CLI + Gateway  | Drop-in scripts for blocking, auto-formatting, context injection  |  
| **[Outbound webhooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#outbound-webhooks)**  |  `hooks.outbound:` list in `~/.hermes/config.yaml`  | CLI + Gateway  | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents  |  
...
asyncdefhandle(event_type:str, context:dict):
...
### Available Events[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#available-events "Direct link to Available Events")  
...
#### Wildcard Matching[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#wildcard-matching "Direct link to Wildcard Matching")
...
### Examples[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#examples "Direct link to Examples")
#### Telegram Alert on Long Tasks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#telegram-alert-on-long-tasks "Direct link to Telegram Alert on Long Tasks")
...
#### Command Usage Logger[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#command-usage-logger "Direct link to Command Usage Logger")
...
defhandle(event_type:str, context:dict):
...
#### Session Start Webhook[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#session-start-webhook "Direct link to Session Start Webhook")
...
### Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot "Direct link to Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot")
...
#### What we're building[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#what-were-building "Direct link to What we're building")
...
#### Step 1: Write your checklist[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#step-1-write-your-checklist "Direct link to Step 1: Write your checklist")
...
#### Step 2: Create the hook[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#step-2-create-the-hook "Direct link to Step 2: Create the hook")
...
def_build_prompt(content:str)-str:
...
def_run_boot_agent(content:str)-None:
...
if response.upper()notin{"[SILENT]","SILENT","NO_REPLY","NO REPLY"}:
...
asyncdefhandle(event_type:str, context:dict)-None:




ifnot BOOT_FILE.exists():
...
#### Step 3: Test it[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#step-3-test-it "Direct link to Step 3: Test it")
Restart the gateway:
...
#### Extending the pattern[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#extending-the-pattern "Direct link to Extending the pattern")
...
#### Why this isn't a built-in[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#why-this-isnt-a-built-in "Direct link to Why this isn't a built-in")
...
### How It Works[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#how-it-works "Direct link to How It Works")
...
info
Gateway hooks only fire in the **gateway** (Telegram, Discord, Slack, WhatsApp, Teams). The CLI does not load gateway hooks. For hooks that work everywhere, use [plugin hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#plugin-hooks).
## Plugin Hooks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#plugin-hooks "Direct link to Plugin Hooks")
[Plugins](https://hermes-agent.nousresearch.com/docs/user-guide/features/plugins) can register hooks that fire in **both CLI and gateway** sessions. These are registered programmatically via `ctx.register_hook()` in your plugin's `register()` function.
For plugin packaging and registration details, see the [Plugins guide](https://hermes-agent.nousresearch.com/docs/user-guide/features/plugins).
...
### Cache-safe system prompt sections[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#cache-safe-system-prompt-sections "Direct link to Cache-safe system prompt sections")
...
defboard_rules(session_info):




returnf"Apply the worker rules for profile {session_info['profile_name']}."
...
### Shipped plugin-hook catalog[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#shipped-plugin-hook-catalog "Direct link to Shipped plugin-hook catalog")
...
| --- | --- | --- | --- | --- |  
| Directive/control  | Once before execution; first valid `block` or `approve` directive wins, and `modify` returns are shallow-merged into the tool arguments.  |  `tool_name`, `args`, `task_id`, `session_id`, `tool_call_id`, `turn_id`, `api_request_id`, `middleware_trace`  | Raw arguments may contain user content, paths, commands, or secrets.  |  
...
### Streaming output hooks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#streaming-output-hooks "Direct link to Streaming output hooks")
...
defon_delta(delta, kind, model, provider,**kwargs):
...
###  `pre_tool_call`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_tool_call "Direct link to pre_tool_call")
...
defmy_callback(tool_name:str, args:dict, task_id:str,**kwargs):
...
defaudit_tool_call(tool_name, args, task_id,**kwargs):
...
defwarn_dangerous(tool_name,**kwargs):
...
###  `post_tool_call`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_tool_call "Direct link to post_tool_call")
...
defmy_callback(tool_name:str, args:dict, result:str, task_id:str,
...
deftrack_metrics(tool_name, result, duration_ms=0,**kwargs):
...
###  `pre_llm_call`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_llm_call "Direct link to pre_llm_call")
...
defmy_callback(session_id:str, user_message:str, conversation_history:list,
...
defrecall(session_id, user_message, is_first_turn,**kwargs):
...
###  `post_llm_call`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_llm_call "Direct link to post_llm_call")
...
defmy_callback(session_id:str, user_message:str, assistant_response:str,
...
defsync_memory(session_id, user_message, assistant_response,**kwargs):
...
deflog_response_length(session_id, assistant_response, model,**kwargs):
...
###  `pre_verify`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_verify "Direct link to pre_verify")
...
defmy_callback(session_id:str, platform:str, model:str, coding:bool,
...
defdefer_ui_checks(coding, attempt, changed_paths,**kwargs):




if attempt ornot coding:




returnNone# one-shot, coding only




ifnotall(p.endswith(UI)forin changed_paths):




returnNone# only pure-UI edits
...
###  `transform_api_error_classification`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_api_error_classification "Direct link to transform_api_error_classification")
...
###  `on_session_start`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_start "Direct link to on_session_start")
...
defmy_callback(session_id:str, model:str, platform:str,**kwargs):
...
definit_session(session_id, model, platform,**kwargs):
...
###  `on_session_end`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_end "Direct link to on_session_end")
...
defmy_callback(session_id:str, completed:bool, interrupted:bool,
...
defcleanup_session(session_id, completed, interrupted,**kwargs):
...
defon_end(session_id, completed, interrupted,**kwargs):
...
###  `on_session_finalize`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_finalize "Direct link to on_session_finalize")
...
defmy_callback(session_id:str|None, platform:str,**kwargs):
...
###  `on_session_reset`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_session_reset "Direct link to on_session_reset")
...
defmy_callback(session_id:str, platform:str,**kwargs):
...
###  `subagent_start`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#subagent_start "Direct link to subagent_start")
...
defmy_callback(parent_session_id:str|None,
...
info
`subagent_start` is useful for delegation observability, but it is not a blocking policy hook. To block delegation before a child is built, use [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_tool_call) to block the `delegate_task` tool call.
###  `subagent_stop`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#subagent_stop "Direct link to subagent_stop")
...
defmy_callback(parent_session_id:str, child_role:str|None,
...
deflog_subagent(parent_session_id, child_role, child_status, duration_ms,**kwargs):
...
###  `pre_gateway_dispatch`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_gateway_dispatch "Direct link to pre_gateway_dispatch")
...
defmy_callback(event, gateway, session_store,**kwargs):
...
defdeny_unauthorized_dms(event,**kwargs):
...
if src.chat_type =="dm"andnot _is_approved_user(src.user_id):
...
defbuffer_or_rewrite(event,**kwargs):
...
###  `gateway_platform_event`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#gateway_platform_event "Direct link to gateway_platform_event")
...
defon_platform_event(platform, event_type, payload,**kwargs):
...
**Return value:** ignored. Hooks here are observer-only; they cannot veto or pre-answer the approval. Use [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_tool_call) to block a tool before it reaches the approval system.
...
defnotify_approval(command, description, session_key,**kwargs):
...
###  `post_approval_response`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_approval_response "Direct link to post_approval_response")
...
deflog_decision(command, choice, session_key,**kwargs):
...
###  `pre_transcription`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_transcription "Direct link to pre_transcription")
...
| `prompt`  | `str | None`  | The static [`stt.prompt`](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#transcription-prompt-vocabulary-hints) value, or `None`.  |  
...
defadd_vocab(provider, prompt, source,**kwargs):
...
Not every backend accepts a prompt. `local` maps it to faster-whisper's `initial_prompt`; `openai`, `groq`, `mistral`, and `deepinfra` send it as `prompt`; `xai`, `elevenlabs`, `local_command`, and `type: command` providers log at DEBUG and transcribe without it. See the [provider support table](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#transcription-prompt-vocabulary-hints) for the full matrix and the privacy boundary. Hook-plumbing errors are fail-open: the dispatch continues with the unmodified request.
###  `transform_tool_result`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_tool_result "Direct link to transform_tool_result")
...
defmy_callback(tool_name:str, args:dict, result:str, task_id:str,**kwargs)-str|None:
...
defredact_secrets(tool_name, result,**kwargs):
...
###  `transform_terminal_output`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_terminal_output "Direct link to transform_terminal_output")
...
defsummarize_find(command, output,**kwargs):




if command.startswith("find ")andlen(output)50_000:
...
###  `transform_llm_output`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_llm_output "Direct link to transform_llm_output")
...
defspongebob(response_text,**kwargs):
...
returnNone# pass through unchanged
...
### API-request observer hooks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#api-request-observer-hooks "Direct link to API-request observer hooks")
####  `pre_api_request`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_api_request "Direct link to pre_api_request")
...
####  `post_api_request`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_api_request "Direct link to post_api_request")
...
####  `api_request_error`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#api_request_error "Direct link to api_request_error")
...
###  `on_skill_lifecycle`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#on_skill_lifecycle "Direct link to on_skill_lifecycle")
...
### Kanban lifecycle observers[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban-lifecycle-observers "Direct link to Kanban lifecycle observers")
####  `kanban_task_claimed`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban_task_claimed "Direct link to kanban_task_claimed")
Fires after the claim commit in the dispatcher process, immediately before worker spawn.
####  `kanban_task_completed`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban_task_completed "Direct link to kanban_task_completed")
Fires after completion and cleanup, usually in the worker process. Its `summary` can contain project or user content.
####  `kanban_task_blocked`[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban_task_blocked "Direct link to kanban_task_blocked")
...
### Kanban worker-lifecycle, task-mutation, and dispatch observers[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban-worker-lifecycle-task-mutation-and-dispatch-observers "Direct link to Kanban worker-lifecycle, task-mutation, and dispatch observers")
...
## Shell Hooks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#shell-hooks "Direct link to Shell Hooks")
...
Use shell hooks when you want a drop-in, single-file script (Bash, Python, anything with a shebang) to:
  ...
  * **Inject context into the next LLM turn** — prepend `git status` output, the current weekday, or retrieved documents to the user message (see [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#pre_llm_call)).
...
### Comparison at a glance[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#comparison-at-a-glance "Direct link to Comparison at a glance")  
| Dimension  | Shell hooks  |  
| --- | --- |  
...
### Configuration schema[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#configuration-schema "Direct link to Configuration schema")
...
### Exit code 2 = block (Claude Code / Cursor compatible)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#exit-code-2--block-claude-code--cursor-compatible "Direct link to Exit code 2 = block \(Claude Code / Cursor compatible\)")
...
### Fail-open vs fail-closed[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#fail-open-vs-fail-closed "Direct link to Fail-open vs fail-closed")
...
### Worked examples[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#worked-examples "Direct link to Worked examples")
#### 1. Auto-format Python files after every write[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#1-auto-format-python-files-after-every-write "Direct link to 1. Auto-format Python files after every write")
...
#### 2. Block destructive `terminal` commands[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#2-block-destructive-terminal-commands "Direct link to 2-block-destructive-terminal-commands")
...
ifecho"$cmd"|grep-qE'rm[[:space:]]+-rf?[[:space:]]+/';then
...
#### 3. Inject `git status` into every turn (Claude-Code `UserPromptSubmit` equivalent)[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent "Direct link to 3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent")
...
ifstatus=$(git status --porcelain2/dev/null)[[-n"$status"]];then




  jq --null-input --arg"$status"\
...
#### 4. Log every subagent completion[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#4-log-every-subagent-completion "Direct link to 4. Log every subagent completion")
...
### Consent model[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#consent-model "Direct link to Consent model")
...
#### Manual allowlisting[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#manual-allowlisting "Direct link to Manual allowlisting")
...
### The `hermes hooks` CLI[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#the-hermes-hooks-cli "Direct link to the-hermes-hooks-cli")  
...
### Security[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#security "Direct link to Security")
...
### Ordering and precedence[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#ordering-and-precedence "Direct link to Ordering and precedence")
...
## Outbound Webhooks[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#outbound-webhooks "Direct link to Outbound Webhooks")
Outbound webhooks are the push-side mirror of the [inbound webhook platform](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/webhooks): inbound webhooks wake Hermes when the world changes; outbound webhooks tell the world when Hermes does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Hermes POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
Typical uses:
...
### Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#configuration "Direct link to Configuration")
...
### Wire format[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#wire-format "Direct link to Wire format")
...
defverify(body:bytes, header:str, secret:str)-bool:
...
### Delivery semantics[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#delivery-semantics "Direct link to Delivery semantics")
...
`hermes hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
  * [Gateway Event Hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#gateway-event-hooks)
  ...
  * [Plugin Hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#plugin-hooks)
    * [Cache-safe system prompt sections](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#cache-safe-system-prompt-sections)
    * [Shipped plugin-hook catalog](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#shipped-plugin-hook-catalog)
    * [Streaming output hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#streaming-output-hooks)
    * [`transform_api_error_classification`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_api_error_classification)
    * [`gateway_platform_event`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#gateway_platform_event)
    * [`post_approval_response`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#post_approval_response)
    * [`transform_tool_result`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_tool_result)
    * [`transform_terminal_output`](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#transform_terminal_output)
    * [API-request observer hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#api-request-observer-hooks)
    * [Kanban lifecycle observers](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban-lifecycle-observers)
    * [Kanban worker-lifecycle, task-mutation, and dispatch observers](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#kanban-worker-lifecycle-task-mutation-and-dispatch-observers)
  * [Shell Hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#shell-hooks)
    * [Comparison at a glance](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#comparison-at-a-glance)
    * [Configuration schema](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#configuration-schema)
    * [Exit code 2 = block (Claude Code / Cursor compatible)](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#exit-code-2--block-claude-code--cursor-compatible)
    * [Fail-open vs fail-closed](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#fail-open-vs-fail-closed)
    * [The `hermes hooks` CLI](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#the-hermes-hooks-cli)
    * [Ordering and precedence](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#ordering-and-precedence)
  * [Outbound Webhooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks#outbound-webhooks)
