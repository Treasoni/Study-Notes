...
上一章我们学习了 Bridge、Host、Container、None 四种单机模式。它们的共同局限：**只能在一台宿主机上工作**。
...
| 模式 | 解决什么问题 | 本质方案 |
|------|-----------|---------|
| **Overlay** | 跨主机通信，零修改现网 | VXLAN 隧道封装 |
...
- **Overlay 模式**通过 VXLAN 隧道实现跨主机容器通信，需要 Docker Swarm 集群支持。数据包每跳增加约 1.1ms 延迟，吞吐损耗 5-15%（大包场景可达 60%），使用时必须调整 MTU 为 1450。
