...
| 模式 | 一句话定位 | 适用场景 |
|------|-----------|---------|
| **Bridge**（默认） | 通过虚拟网桥 + iptables 实现隔离和端口映射 | 单机开发测试、默认选择 |
...
在第五章我们亲手搭建过这样的场景：用 `ip netns` 创建隔离的网络命名空间，用 `veth pair` 连接两个空间，再挂到 `br0` 网桥上。Docker Bridge 模式的本质，就是这个方案的自动化版本。
...
> 注：如果系统没有 `brctl`，可以通过 `ip link show type bridge` 或 `bridge link` 查看。
...
技术上说，Docker 的默认 Bridge 模式对外通信依赖 **iptables MASQUERADE（源地址伪装）** 。让我们剖析一次完整的对外访问：
...
> 经验法则：每个并发连接消耗约 300 字节内核内存。计算需要的最大并发量，再乘以 1.5 的余量来设置 `nf_conntrack_max`。

### 7.2.6 默认 Bridge 模式的局限

默认 Bridge 模式虽然即开即用，但有两个重要局限：
...
> 注意：Host 模式**仅支持 Linux**。在 macOS 和 Windows 上，Docker 通过虚拟机运行，`--network host` 的实际行为与 Linux 不同。
...
- **Bridge 模式**是 Docker 的默认网络模式，通过 docker0 网桥 + veth pair + iptables NAT 实现容器网络隔离和对外通信。数据包出宿主机时经过 MASQUERADE（源地址伪装），入宿主机时经过 DNAT（目的地址改写）。
...
本章所有模式都局限于**单台主机**。跨主机容器如何通信？下一章介绍的 Overlay 模式通过 VXLAN 隧道解决这个问题，同时 Macvlan 和 IPvlan 模式提供了近乎物理网卡性能的高性能方案。
