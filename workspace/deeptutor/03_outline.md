...
- **篇幅**：长篇
- **覆盖要点**：Knowledge Center 与多引擎 RAG 知识库、带引用问答、Ask Questions/Quiz 出题、Visualize 可视化、Solve 解题、Research 深研、Mastery Path 掌握度路径
- **素材引用**：[S1]、[S2]、[S4]、[S6]
...
### 3.6 掌握度路径（Mastery Path）与 Course Study
### 3.7 本节小结：一个完整学习闭环示例

---

## 第 4 章：进阶玩法 —— Living Book、Partners 与 Agent 驱动

- **篇幅**：中篇
- **覆盖要点**：Living Book（Book Engine 14 种块类型）、Partners/TutorBot（SOUL/模型策略/资料库/记忆 + 15+ IM 渠道）、My Agents 调度外部 agent、CLI 与 SDK 驱动、术语演进说明
- **素材引用**：[S1]、[S4]
- **代码示例**：有（deeptutor run 驱动、Partner 创建与渠道接入、Living Book 编译与导出）

### 4.1 Living Book：把 KB / 笔记 / 题库 / 聊天历史编译成交互"活书"
### 4.2 Partners / TutorBot：持久陪伴体与 15+ IM 渠道
#### 4.2.1 术语演进：TutorBot → Partners（写作时统一并说明）
#### 4.2.2 创建 Partner 并接入一个 IM 渠道（Feishu/Telegram/微信等）
### 4.3 My Agents：调度外部 agent（Claude Code / Codex 等）与导入历史会话
### 4.4 CLI 与 SDK：单轮 run、给其他 agent 驱动 DeepTutor
### 4.5 EduHub 技能生态（可选拓展）

---

## 第 5 章：架构与原理 —— 它为什么这么设计
...
### 5.1 Agent-Native 架构重写：为什么是"单一 Agent 循环"
#### 5.1.1 ChatOrchestrator：Web / CLI / SDK / Partner 所有入口汇聚
#### 5.1.2 工具模型：可开关工具 vs 上下文相关工具（自动挂载）
#### 5.1.3 两类上下文：sticky session context vs one-time references
### 5.2 三层持久记忆：可读、可审计、可追溯
#### 5.2.1 L1 事件追踪 → L2 单面整理 → L3 跨面综合
#### 5.2.2 Memory Graph：从综合主张回溯原始事件
#### 5.2.3 read_memory / write_memory 的工作机制
### 5.3 多引擎 RAG：为什么不是一套检索打天下
### 5.4 论文设计：混合个性化引擎与 TutorBench
#### 5.4.1 引文锚定问题辅导 + 难度校准题目生成
#### 5.4.2 评测方法与结论（个性化指标 +10.8%、通用 agentic reasoning +29.4%）
### 5.5 论文设计与当前产品的版本差异（术语与功能演进）
### 5.6 开放生态与许可：Agent-Skills 格式、EduHub、Apache-2.0
...
### 6.1 排错工具链：deeptutor doctor、日志 tail、端口检查
### 6.2 启动类问题：端口被占、容器立刻退出、前端空白页
### 6.3 模型接入类问题：provider probe 401、host.docker.internal 解析、embedding endpoint
### 6.4 知识库与索引问题：Embedding dimension mismatch、KB 卡 indexing
### 6.5 Windows 环境专项：VS Build Tools、pip 慢
### 6.6 Partner 渠道与认证：missing SDK、allow_from 显式 opt-in、auth_secret 丢失
### 6.7 高频问题速查表汇总
...
- 不需要预先掌握 Agent / RAG 概念 —— 笔记会在使用时同步讲解，原理放在第 5 章集中展开。
...
### 建议学习顺序
- 顺序：第 1 章 → 第 2 章 → 第 3 章 → 第 4 章 → 第 5 章 → 第 6 章。
- 第 6 章是速查表，建议第一次通读，之后按需查阅；第 5 章可先通读建立整体认知，部署后再精读。
- 时间估计（仅供参考）：第 1 章约 30 分钟；第 2 章部署 + 配置约 1–2 小时；第 3 章核心玩法约 3–4 小时；第 4 章进阶约 2 小时；第 5 章原理约 2–3 小时；第 6 章随用随查。
