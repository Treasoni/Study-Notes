...
title: "GitHub - HKUDS/DeepTutor: DeepTutor: Lifelong Personalized Tutoring. https://deeptutor.info/. · GitHub"
scraped_at: 2026-09-01T15:15:32+00:00
---

[Skip to content](https://github.com/HKUDS/DeepTutor#start-of-content)
You signed in with another tab or window. [Reload](https://github.com/HKUDS/DeepTutor) to refresh your session. You signed out in another tab or window. [Reload](https://github.com/HKUDS/DeepTutor) to refresh your session. You switched accounts on another tab or window. [Reload](https://github.com/HKUDS/DeepTutor) to refresh your session. Dismiss alert
###  Uh oh! 
There was an error while loading. [Please reload this page](https://github.com/HKUDS/DeepTutor).
/ Public
  * [ Notifications ](https://github.com/login?return_to=%2FHKUDS%2FDeepTutor) You must be signed in to change notification settings
  * [ Fork 4.8k ](https://github.com/login?return_to=%2FHKUDS%2FDeepTutor)
  * [ Star  38.2k ](https://github.com/login?return_to=%2FHKUDS%2FDeepTutor)
...
## Latest commit
## History
[1,550 Commits](https://github.com/HKUDS/DeepTutor/commits/main/)
1,550 Commits
## Folders and files  
...
|   |   |  
|   |   |  
| [docs/plans](https://github.com/HKUDS/DeepTutor/tree/main/docs/plans "This path skips through empty directories")  | [docs/plans](https://github.com/HKUDS/DeepTutor/tree/main/docs/plans "This path skips through empty directories")  |  
| [packaging/deeptutor-cli](https://github.com/HKUDS/DeepTutor/tree/main/packaging/deeptutor-cli "This path skips through empty directories")  | [packaging/deeptutor-cli](https://github.com/HKUDS/DeepTutor/tree/main/packaging/deeptutor-cli "This path skips through empty directories")  |  
|   |   |  
|   |   |  
|   |   |  
 |  
...
|   |   |  
|   |   |  
| [.pre-commit-config.yaml](https://github.com/HKUDS/DeepTutor/blob/main/.pre-commit-config.yaml ".pre-commit-config.yaml")  | [.pre-commit-config.yaml](https://github.com/HKUDS/DeepTutor/blob/main/.pre-commit-config.yaml ".pre-commit-config.yaml")  |  
|   |   |  
|   |   |  
|   |   |  
|   |   |  
...
|   |   |  
|   |   |  
| [compose.codex-oauth.yaml](https://github.com/HKUDS/DeepTutor/blob/main/compose.codex-oauth.yaml "compose.codex-oauth.yaml")  | [compose.codex-oauth.yaml](https://github.com/HKUDS/DeepTutor/blob/main/compose.codex-oauth.yaml "compose.codex-oauth.yaml")  |  
|   |   |  
|   |   |  
| [docker-compose.ghcr.yml](https://github.com/HKUDS/DeepTutor/blob/main/docker-compose.ghcr.yml "docker-compose.ghcr.yml")  | [docker-compose.ghcr.yml](https://github.com/HKUDS/DeepTutor/blob/main/docker-compose.ghcr.yml "docker-compose.ghcr.yml")  |  
|   |   |  
|   |   |  
|   |   |  
| View all files |  
...
[Features](https://github.com/HKUDS/DeepTutor#-key-features) · [Get Started](https://github.com/HKUDS/DeepTutor#-get-started) · [Explore](https://github.com/HKUDS/DeepTutor#-explore-deeptutor) · [CLI](https://github.com/HKUDS/DeepTutor#%EF%B8%8F-deeptutor-cli--agent-native-interface) · [Ecosystem](https://github.com/HKUDS/DeepTutor#-ecosystem--eduhub--the-skills-community) · [Community](https://github.com/HKUDS/DeepTutor#-community)
> 🤝 **We welcome any kinds of contributing!** Vote on roadmap items or propose new ones at [`Roadmap`](https://github.com/HKUDS/DeepTutor/issues/498), and see our [Contributing Guide](https://github.com/HKUDS/DeepTutor/blob/main/CONTRIBUTING.md) for branching strategy, coding standards, and how to get started.
### 📦 Releases
...
> See [CONTAINERIZATION.md](https://github.com/HKUDS/DeepTutor/blob/main/CONTAINERIZATION.md) for podman/rootless/read-only-rootfs deployments and the full per-installation guide.
...
**🏗️ System architecture** **💬 Chat — The Agent Loop You Actually Use**
...
Home keeps **Chat** , **Ask Questions** , **Quiz** , **Visualize** , and **Immersive Watching** one click away; **Research** for cited reports and **Solve** for worked reasoning sit under _More Capabilities_. **Mastery Path** and **Immersive Reading** are dedicated sidebar workspaces, while Course Study keeps its own course-bound context.
...
You don't have to write every skill yourself — **Import from EduHub** browses the community catalog and downloads a skill straight into your library through a security gate (see [Ecosystem](https://github.com/HKUDS/DeepTutor#-ecosystem--eduhub--the-skills-community)).
**🧠 Memory — Inspectable Personalization**
Memory is a file-backed, three-layer system you can read, curate, and audit — deliberately _not_ a hidden vector store. **L1** is the workspace mirror plus an append-only event trace (`trace/<surface>/<date>.jsonl`); **L2** is per-surface curated facts (`L2/<surface>.md`); **L3** is cross-surface synthesis (`L3/<profile|recent|scope|preferences>.md`). Because L2 cites L1 and L3 cites L2, nothing in your profile is unaccountable.
...
Default local Docker and Podman deployments use separate loopback networks and need a temporary bridge during sign-in. Follow the [temporary local Codex OAuth bridge guide](https://github.com/HKUDS/DeepTutor/blob/main/CONTAINERIZATION.md#temporary-local-codex-oauth-bridge) for the exact Docker, Compose, Podman, and teardown commands.
...
DeepTutor is built to be _operated by another agent_. Add `--format json` to any `run` and each turn streams **NDJSON — one event per line** (`content`, `tool_call`, `tool_result`, `done`, …), every line tagged with its `session_id`. Runs are headless-safe: an `ask_user` pause with no TTY auto-resolves with an empty reply instead of hanging.
...
The repo ships a root [`SKILL.md`](https://github.com/HKUDS/DeepTutor/blob/main/SKILL.md) — a ~200-line handover doc that teaches any tool-using LLM the whole surface in one read. Hand it to Claude Code, Codex, or OpenCode (they pick up `SKILL.md` automatically), or wrap `deeptutor run` as a tool in a LangChain / AutoGen loop. Full recipes: [Agent Handoff](https://deeptutor.info/docs/cli/agent-handoff/).
**Command reference**  
| Command  | Description  |  
| --- | --- |  
...
Because DeepTutor speaks the open Agent-Skills format, works as a first-class source too — it's built in alongside EduHub. Pick it with the hub prefix:
...
When several publishers share the same slug, search shows each publisher and a fully scoped install ref (`clawhub:<ownerHandle>/<slug>`).
...
Using code: **`DEEPTUTOR20`**— get $20 off for your first[PageIndex subscription](https://developer.pageindex.ai/)! 
## 🌐 Community
### 🔗 Maintainers  
| [**Bingxi Zhao**](https://github.com/pancacake)  | [**Xingyu Hou**](https://github.com/TyrionH-is-coding)  | [**Jiahao Zhang**](https://github.com/zzhtx258)  |  
| --- | --- | --- |  
### 📮 Contact
...
| Ultra-lightweight agent engine that powered the original TutorBot _(HKUDS)_  |  
| Simple & fast RAG _(HKUDS)_  |  
| Zero-code agent framework _(HKUDS)_  |  
| Automated research pipeline _(HKUDS)_  |  
...
### Topics
[ai-agents](https://github.com/topics/ai-agents)[ai-tutor](https://github.com/topics/ai-tutor)[clawdbot](https://github.com/topics/clawdbot)[cli-tool](https://github.com/topics/cli-tool)[deepresearch](https://github.com/topics/deepresearch)[interactive-learning](https://github.com/topics/interactive-learning)[large-language-models](https://github.com/topics/large-language-models)[multi-agent-systems](https://github.com/topics/multi-agent-systems)[rag](https://github.com/topics/rag)
### Resources
### Contributing
...
**189** watching
### Forks
[**4.8k** forks](https://github.com/HKUDS/DeepTutor/forks)
## Releases
## Packages
## Contributors
## Languages
