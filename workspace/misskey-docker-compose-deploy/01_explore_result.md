...
| 决策项 | 结果 |
| --- | --- |
| P2 精读范围 | **A（最小可运行闭环）+ C（NAS 落地与运维）** |
| B 对外服务链路 | 不纳入本期 |
| D 全文检索配置 | 不纳入本期 |
| 对象存储（S3） | **移除** |
| 邮件发送 | **移除** |

**由此产生的范围约束（供 P2 与后续阶段遵守）**：

1. 全篇主张应尽量落在官方一手来源（S1、S4、S5、S7、S8、S9、S10）上，避免引入无来源的配置细节。
2. 反向代理与 TLS 不进正文；仅在解释 `url` 字段不可更改、以及实例对外可达性时作为一句话背景提及，不展开配置。
3. 全文检索不进正文的独立章节；仅在解释 `default.yml` 必改字段时说明默认值是 `sqlLike`（不需扩展），不展开三种 provider 对比。
4. C 部分需重点落实：构建 vs 预构建镜像的取舍、内存门槛的多方口径冲突、卷权限（UID/GID 991）、镜像标签与升级流程、近期破坏性变更（Node 版本、sharp 需 SSE4.2、NSFW 判定改外部服务）。
5. 需明确标注为「本文档推断、非官方」的内容：仅内网 / 无公网域名的变体方案、NAS 专属实操步骤。
