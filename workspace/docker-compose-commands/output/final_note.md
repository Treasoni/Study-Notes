...
> 笔记类型：概念速查（concept + cheat_sheet 混合），面向「入门 + 随手查」。
> 内容涵盖：Docker 核心概念（镜像 vs 容器）、Docker CLI 常用命令速查（镜像 / 容器 / 网络 / 卷 / 系统清理）、Docker Compose 编排与配置语法、常见坑与排错速查表。零基础可通读一遍建立心智模型，之后当作随身速查手册随查随用。
