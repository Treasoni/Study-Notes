...
## Install [​](https://doc.oplist.org/guide/installation/docker#install)
## 安装 [​](https://doc.oplist.org/guide/installation/docker#%E5%AE%89%E8%A3%85)
### Docker CLI [​](https://doc.oplist.org/guide/installation/docker#docker-cli)
Install Docker. And run the command below:
安装好 Docker 后，执行以下命令：
#### v4.1.0 以后版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E4%BB%A5%E5%90%8E%E7%89%88%E6%9C%AC)
#### For version after v4.1.0 [​](https://doc.oplist.org/guide/installation/docker#for-version-after-v4-1-0)
WARNING
...
#### v4.1.0 及以前版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E5%8F%8A%E4%BB%A5%E5%89%8D%E7%89%88%E6%9C%AC)
#### For version v4.1.0 and earlier [​](https://doc.oplist.org/guide/installation/docker#for-version-v4-1-0-and-earlier)
bash
```
...
### Docker Compose [​](https://doc.oplist.org/guide/installation/docker#docker-compose)
...
写入以下内容，然后保存并退出：
#### For version after v4.1.0 [​](https://doc.oplist.org/guide/installation/docker#for-version-after-v4-1-0-1)
#### v4.1.0 以后版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E4%BB%A5%E5%90%8E%E7%89%88%E6%9C%AC-1)
yaml
```
# docker-compose.yml
services:
...
#### For version v4.1.0 and earlier [​](https://doc.oplist.org/guide/installation/docker#for-version-v4-1-0-and-earlier-1)
#### v4.1.0 及以前版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E5%8F%8A%E4%BB%A5%E5%89%8D%E7%89%88%E6%9C%AC-1)
yaml
```
# docker-compose.yml
services:
...
## Env [​](https://doc.oplist.org/guide/installation/docker#env)
## 环境变量 [​](https://doc.oplist.org/guide/installation/docker#%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F)  
| Name  | Default  | Desc  |  
| --- | --- | --- |  
| `PUID`  | User UID, Deprecated in v4.1.0 later versions  |  
| `PGID`  | User GID, Deprecated in v4.1.0 later versions  |  
| `UMASK`  | `022`  | <https://en.wikipedia.org/wiki/Umask>  |  
| `UTC`  | Default is the UTC time zone. If you want to specify a time zone, you can set this variable, for example: `Asia/Shanghai`.  |  
...
| 名称  | 默认值  | 说明  |  
| --- | --- | --- |  
| `PUID`  | 运行身份 UID，在 v4.1.0 以后的版本中废弃  |  
| `PGID`  | 运行身份 GID，在 v4.1.0 以后的版本中废弃  |  
| `UMASK`  | `022`  | <https://en.wikipedia.org/wiki/Umask>  |  
...
Additionally, OpenList supports passing [configuration](https://doc.oplist.org/configuration/configuration) through environment variables. The OpenList in the Docker image runs by default with the `--no-prefix` flag, so you don’t need to add the `OPENLIST_` prefix.
You can view all available environment variables online in Go Packages.
此外，OpenList 支持将 [配置](https://doc.oplist.org/configuration/configuration) 通过环境变量的方式传入。Docker 镜像中的 OpenList 默认使用 `--no-prefix` 参数运行，因此您无需添加 `OPENLIST_` 前缀。
您可以在 Go Packages 中在线查看所有可以使用的环境变量。
<https://pkg.go.dev/github.com/OpenListTeam/OpenList/v4/internal/conf#Config>
## Image Versions [​](https://doc.oplist.org/guide/installation/docker#image-versions)
## 镜像版本 [​](https://doc.oplist.org/guide/installation/docker#%E9%95%9C%E5%83%8F%E7%89%88%E6%9C%AC)
...
## See the admin’s info [​](https://doc.oplist.org/guide/installation/docker#see-the-admin-s-info)
## 查看管理员信息 [​](https://doc.oplist.org/guide/installation/docker#%E6%9F%A5%E7%9C%8B%E7%AE%A1%E7%90%86%E5%91%98%E4%BF%A1%E6%81%AF)
### First run [​](https://doc.oplist.org/guide/installation/docker#first-run)
### 首次运行 [​](https://doc.oplist.org/guide/installation/docker#%E9%A6%96%E6%AC%A1%E8%BF%90%E8%A1%8C)
bash
```
...
### Not first run [​](https://doc.oplist.org/guide/installation/docker#not-first-run)
### 非首次运行 [​](https://doc.oplist.org/guide/installation/docker#%E9%9D%9E%E9%A6%96%E6%AC%A1%E8%BF%90%E8%A1%8C)
...
## Update [​](https://doc.oplist.org/guide/installation/docker#update)
## 更新 [​](https://doc.oplist.org/guide/installation/docker#%E6%9B%B4%E6%96%B0)
### Watchtower [​](https://doc.oplist.org/guide/installation/docker#watchtower)
...
### Docker CLI [​](https://doc.oplist.org/guide/installation/docker#docker-cli-1)
bash
```
...
#### 升级到 v4.1.0 以后版本 （不包括v4.1.0） [​](https://doc.oplist.org/guide/installation/docker#%E5%8D%87%E7%BA%A7%E5%88%B0-v4-1-0-%E4%BB%A5%E5%90%8E%E7%89%88%E6%9C%AC-%E4%B8%8D%E5%8C%85%E6%8B%ACv4-1-0)
#### Upgrade to version v4.1.0 and later (excluding v4.1.0) [​](https://doc.oplist.org/guide/installation/docker#upgrade-to-version-v4-1-0-and-later-excluding-v4-1-0)
TIP
If you want to run and manage OpenList and its configuration directory using the default OpenList user (1001) inside the container, please use the following command:
TIP
如果您希望使用 1001，即容器内置的默认 `openlist` 用户运行和管理 OpenList 及其配置目录，请使用以下命令：
...
#### 升级到 v4.1.0 以前的版本（包括 v4.1.0） [​](https://doc.oplist.org/guide/installation/docker#%E5%8D%87%E7%BA%A7%E5%88%B0-v4-1-0-%E4%BB%A5%E5%89%8D%E7%9A%84%E7%89%88%E6%9C%AC-%E5%8C%85%E6%8B%AC-v4-1-0)
#### Upgrade to version v4.1.0 and earlier (including v4.1.0) [​](https://doc.oplist.org/guide/installation/docker#upgrade-to-version-v4-1-0-and-earlier-including-v4-1-0)
bash
```
docker run -d --restart=unless-stopped -v /etc/openlist:/opt/openlist/data -p 5244:5244 -e PUID=0 -e PGID=0 -e UMASK=022 --name="openlist" openlistteam/openlist:latest
```

### Docker Compose [​](https://doc.oplist.org/guide/installation/docker#docker-compose-1)
...
## Advanced Docker Compose [​](https://doc.oplist.org/guide/installation/docker#advanced-docker-compose)
## 增强版 Docker Compose [​](https://doc.oplist.org/guide/installation/docker#%E5%A2%9E%E5%BC%BA%E7%89%88-docker-compose)
Create `docker-compose.yml` file.
创建 `docker-compose.yml` 文件。
bash
```
...
Write the content below. Then save and exit.
写入以下内容，然后保存并退出：
#### For version after v4.1.0 [​](https://doc.oplist.org/guide/installation/docker#for-version-after-v4-1-0-2)
#### v4.1.0 以后版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E4%BB%A5%E5%90%8E%E7%89%88%E6%9C%AC-2)
yaml
```
# docker-compose.yml
services:
...
#### For version v4.1.0 and earlier [​](https://doc.oplist.org/guide/installation/docker#for-version-v4-1-0-and-earlier-2)
#### v4.1.0 及以前版本 [​](https://doc.oplist.org/guide/installation/docker#v4-1-0-%E5%8F%8A%E4%BB%A5%E5%89%8D%E7%89%88%E6%9C%AC-2)
yaml
```
# docker-compose.yml
services:
...
### Env [​](https://doc.oplist.org/guide/installation/docker#env-1)
### 环境变量 [​](https://doc.oplist.org/guide/installation/docker#%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F-1)  
| Name  | Default  | Desc  |  
| --- | --- | --- |  
| `OPLISTDX_PUID`  | User UID  |  
| `OPLISTDX_PGID`  | User GID  |  
...
| 名称  | 默认值  | 说明  |  
| --- | --- | --- |  
| `OPLISTDX_PUID`  | 运行身份 UID  |  
| `OPLISTDX_PGID`  | 运行身份 GID  |  
...
## Contributors
[Edit this page on GitHub](https://github.com/OpenListTeam/OpenList-Docs/edit/main/pages/guide/installation/docker.md)
Last updated: 
