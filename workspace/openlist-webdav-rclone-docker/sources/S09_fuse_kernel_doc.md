---
url: "https://docs.kernel.org/filesystems/fuse/fuse.html"
title: "1. FUSE Overview — The Linux Kernel  documentation"
...
Internal API manuals
  * [Subsystems](https://docs.kernel.org/subsystem-apis.html)
    ...
    * [Storage interfaces](https://docs.kernel.org/subsystem-apis.html#storage-interfaces)
      ...
      * [TCM Virtual Device](https://docs.kernel.org/target/index.html)
...
#  1. FUSE Overview[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#fuse-overview "Permalink to this heading")
##  1.1. Definitions[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#definitions "Permalink to this heading") 
...
##  1.2. What is FUSE?[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#what-is-fuse "Permalink to this heading")
...
##  1.3. Filesystem type[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#filesystem-type "Permalink to this heading")
...
##  1.4. Mount options[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#mount-options "Permalink to this heading") 
...
##  1.5. Control filesystem[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#control-filesystem "Permalink to this heading")
...
> The maximum number of background requests that can be outstanding at a time. When the number of background requests reaches this limit, further requests will be blocked until some are completed, potentially causing I/O operations to stall. 

congestion_threshold
    
> The threshold of background requests at which the kernel considers the filesystem to be congested. When the number of background requests exceeds this value, the kernel will skip asynchronous readahead operations, reducing read-ahead optimizations but preserving essential I/O, as well as suspending non-synchronous writeback operations (WB_SYNC_NONE), delaying page cache flushing to the filesystem.
Only the owner of the mount may read or write these files.
###  1.5.1. Interrupting filesystem operations[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#interrupting-filesystem-operations "Permalink to this heading")
...
The userspace filesystem may ignore the INTERRUPT requests entirely, or may honor them by sending a reply to the _original_ request, with the error set to EINTR.
...
##  1.6. Aborting a filesystem connection[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#aborting-a-filesystem-connection "Permalink to this heading")
...
##  1.7. How do non-privileged mounts work?[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#how-do-non-privileged-mounts-work "Permalink to this heading")
...
##  1.8. How are requirements fulfilled?[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#how-are-requirements-fulfilled "Permalink to this heading")
...
>>>       2. It can present files or directories of unlimited length, or directory structures of unlimited depth, possibly causing a system process to eat up diskspace, memory or other resources, again causing _DoS_.
...
>> Note that the _ptrace_ check is not strictly necessary to prevent C/2/i, it is enough to check if mount owner has enough privilege to send signal to the process accessing the filesystem, since _SIGSTOP_ can be used to get a similar effect.
> 

##  1.9. I think these limitations are unacceptable?[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#i-think-these-limitations-are-unacceptable "Permalink to this heading")
...
Note that both of these relaxations expose the system to potential information leak or _DoS_ as described in points B and C/2/i-ii in the preceding section.
##  1.10. Kernel - userspace interface[¶](https://docs.kernel.org/filesystems/fuse/fuse.html#kernel-userspace-interface "Permalink to this heading")
...
©The kernel development community. | Powered by [Sphinx 5.3.0](https://www.sphinx-doc.org/) & [Alabaster 0.7.16](https://alabaster.readthedocs.io) | [Page source](https://docs.kernel.org/_sources/filesystems/fuse/fuse.rst.txt)
