...
title: "Subnet routers · Tailscale Docs"
...
Close navigation
# Subnet routers
Last validated: Jan 12, 2026|Copy as Markdown|[View as Markdown](https://tailscale.com/docs/features/subnet-routers.md)
...
You can skip this step if you use [`autoApprovers`](https://tailscale.com/docs/reference/syntax/policy-file#autoapprovers).
  ...
  2. Locate the **Subnets** badge in the devices list or use the [`property:subnet`](https://console.tailscale.com/admin/machines?q=property%3Asubnet) filter to list all devices advertising subnet routes.
...
You can skip this step if you already have rules that allow access to your advertised subnet routes.
  1. Open the [Access controls](https://console.tailscale.com/admin/acls) page of the admin console to update your [tailnet policy file](https://tailscale.com/docs/features/access-control/acls).
...
Check that you can ping the Tailscale IP address of your new subnet routers from a tailnet device (such as a Linux, macOS, or Windows device). You can find the Tailscale IP in the [admin console](https://console.tailscale.com/admin) or by running the following command on the subnet router.
...
[Currently](https://github.com/tailscale/tailscale/issues/18725) setting `--snat-subnet-routes=false` on a node that's acting as both a subnet router and exit node can result in upstream drops. The recommended workaround for this is to separate the role of the subnet router and the exit node between two nodes if the subnet router must have SNAT disabled.
