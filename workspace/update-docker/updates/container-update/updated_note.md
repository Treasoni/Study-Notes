...
tags:
  - docker
status: 已更新
source_project: docker
---

我给你分 **两种情况** 讲清楚（你基本就在这两种里）：

> [!note] 版本背景（2026）
> 截至 2026 年，Docker Engine 当前稳定版为 **29.x**（如 29.6.2），Docker Compose 为 **v5.x**。文中的 `docker compose` 命令即 Compose v2 插件，是当前标准用法；旧版 `docker-compose`（v1，基于 Python）已废弃，不再推荐。
...
> [!tip] Docker Engine 29 相关改进
> Docker Engine 29 增强了 BuildKit 构建，并引入镜像层去重（image layer dedup）。频繁更新镜像时，重复的镜像层不再重复占用磁盘空间。
...
- **2026-08-04**：全面刷新到 2026 最新。补齐 frontmatter（`status`、`source_project`，`updated` 更新为 2026-08-04）；新增开头 `[!note]` 版本背景（Docker Engine 29.x / Compose v5.x，`docker compose` v2 插件为标准、`docker-compose` v1 已废弃）；§1.1 新增 `[!tip]` 说明 Docker Engine 29 的 BuildKit 改进与镜像层去重；核心更新流程不变。
