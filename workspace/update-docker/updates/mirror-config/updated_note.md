---
title: Docker Desktop Mac 镜像加速器配置
...
>
> 本文基于 **Docker Desktop 4.83 / Docker Engine 29.x**（2026-08 现状）核对。`registry-mirrors` 配置机制仍受支持，但社区镜像源变化频繁，请以「五、可用镜像源列表」的最新清单为准。
...
> [!tip] 配置入口在 Mac / Windows 上一致
> 本节操作是 Docker Desktop 通用界面，Mac 和 Windows 的 **Settings → Docker Engine** 入口相同，本笔记以 Mac 为例。
...
> `registry-mirrors` 数组在 daemon.json 中**仍受 Docker Engine 支持，未被移除**（截至 2026-08）。已弃用的只是 `dockerd --registry-mirror` 命令行 flag，**不要混用**。
...
> [!warning] 国内镜像站已大面积关停
> 2024-06 起，阿里云、腾讯云、中科大（USTC）、南大（NJU）、上海交大（SJTU）等 Docker Hub 加速器相继停服。本文以下清单为 **2025-2026 社区实测仍可用的个人/社区维护源，不保证长期有效**，且大多只支持 `docker pull`、不支持 `docker search`。**务必同时配置多个镜像源**，失效时及时更换。

### 国内镜像源

| 镜像源 | 地址 | 状态 |
|--------|------|------|
| **DaoCloud** | `https://docker.m.daocloud.io` | ⚠️ 白名单模式，需测试 |
| **1ms.run（毫秒镜像）** | `https://docker.1ms.run` | ⚠️ 需测试 |
| **轩辕镜像** | `https://docker.xuanyuan.me` | ⚠️ 需测试 |
| **阿里云 ACR（个人免费版）** | `https://<your-id>.mirror.aliyuncs.com` | ✅ 个人注册可配置 |

> [!tip] 阿里云 ACR 说明
> 登录阿里云容器镜像服务（ACR）控制台，在「镜像加速器」页面可查看属于你自己的加速地址 `https://<your-id>.mirror.aliyuncs.com`，相对更稳定，但同样需要自行测试。
...
**其他常见原因**：
- 镜像源本身已失效（社区源变化频繁，换一个试试）
- 拉取时使用了全限定名（如 `docker.io/nginx`）会**绕过**镜像源，请直接用 `docker pull nginx`
...
2. 尝试使用代理（Docker Desktop 需在 **Settings → Resources → Proxies** 配置）
3. 使用云厂商容器镜像服务（阿里云 ACR / 腾讯 TCR / 华为 SWR）
4. 使用一键换源工具（如 [Cmirror](https://github.com/ox01024/cmirror)，`sudo cmirror use docker --fastest`）

### Q4：GUI 配置和命令行配置冲突？

**说明**：两者编辑的是**同一个** `~/.docker/daemon.json` 文件，正常不会冲突；GUI 保存时只是把内容写回该文件。

**建议**：统一使用 GUI 配置，避免手写 JSON 时出现语法错误。
...
- [Docker Desktop release notes](https://docs.docker.com/desktop/release-notes/) - Docker 官方
- [Docker Engine 29 release notes](https://docs.docker.com/engine/release-notes/29/) - Docker 官方
...
- **2026-08-03**：替换失效镜像源（USTC/南大/SJTU → DaoCloud/1ms.run/轩辕 + 阿里云 ACR 个人版）；版本基线更新为 Docker Desktop 4.83 / Engine 29.x；澄清 `registry-mirrors` 机制仍受支持（仅 CLI flag 弃用）；补充社区源失效风险提示与 FAQ 修正；更新参考资料与 frontmatter。
