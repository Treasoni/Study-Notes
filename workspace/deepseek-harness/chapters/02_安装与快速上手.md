...
对熟悉 Claude Code 的用户，dsh 的安装并不复杂——不需要构建任何东西，`npx` 一行即可启动。本章带你走完安装三路径、Web UI 首次配置、跑通第一个会话，并覆盖 headless 一次性任务与常见坑。
...
> **大白话**：npx 像「借一辆现成的车」，源码构建像「买零件自己组装」，SDK 像「租一辆会开就行」。新手用 npm 最省事。
...
> **大白话**：把 API Key 想成保险箱里的钥匙——填进去后，页面只告诉你「钥匙在保险箱里」，不再让你看到钥匙本身。明文只存在本机 `$DSH_HOME/.credentials.yaml`。
...
涉及需审批的操作会按当前权限策略弹出确认——新会话默认 `workspace-write` 权限预设（第三章详解）。到这里，第一个会话就算跑通了。[1]
...
[1]: 素材来源：`workspace/deepseek-harness/02_deep_research.md` 第二部分（安装与快速上手）。
