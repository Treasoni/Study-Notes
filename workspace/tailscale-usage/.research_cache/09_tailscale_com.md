...
Close navigation
# Tailscale SSH
Last validated: Jan 5, 2026|Copy as Markdown|[View as Markdown](https://tailscale.com/docs/features/tailscale-ssh.md)
Tailscale SSH lets Tailscale manage the authentication and authorization of SSH connections in your tailnet.
Tailscale SSH is available for [all plans](https://tailscale.com/pricing).
The following related video provides additional context and examples.
## [Why it matters](https://tailscale.com/docs/features/tailscale-ssh#why-it-matters)
...
Your SSH configuration (`/etc/ssh/sshd_config`) and keys (`~/.ssh/authorized_keys`) files will not be modified, which means that other SSH connections to the same host, not made over Tailscale, will still work.
  ...
  * **Record SSH sessions with SSH recording** —enforce [SSH recording](https://tailscale.com/docs/features/tailscale-ssh/tailscale-ssh-session-recording) for audit, troubleshooting, and compliance requirements.


## [Use cases](https://tailscale.com/docs/features/tailscale-ssh#use-cases)
...
## [How is Tailscale SSH different?](https://tailscale.com/docs/features/tailscale-ssh#how-is-tailscale-ssh-different)
...
| Works with MagicDNS names.  | Works with MagicDNS names.  |  
...
| macOS  | Available.  | Requires use of [open source `tailscaled` variant](https://tailscale.com/docs/concepts/macos-variants#open-source-tailscaled-variant).  |  
For additional information, refer to [Limitations](https://tailscale.com/docs/features/tailscale-ssh#limitations).
## [How does it work?](https://tailscale.com/docs/features/tailscale-ssh#how-does-it-work)
...
### [Authentication and authorization](https://tailscale.com/docs/features/tailscale-ssh#authentication-and-authorization)
...
Tailscale will only authorize the two devices to connect if the access control policies in the tailnet permit it. Tailscale uses `netstack` port interception and just-in-time automatic configuration of the client `known_hosts` file to make `ssh myhost` work without any new binary or configuration file. Tailscale implements the SSH File Transfer Protocol (SFTP) which lets SCP and SFTP work for newer SSH clients.
...
### [Key management and distribution](https://tailscale.com/docs/features/tailscale-ssh#key-management-and-distribution)
...
## [Configure Tailscale SSH](https://tailscale.com/docs/features/tailscale-ssh#configure-tailscale-ssh)
### [Prerequisites](https://tailscale.com/docs/features/tailscale-ssh#prerequisites)
...
To enable Tailscale SSH, you must:
  ...
  * Ensure an access control policy exists that grants the source with access to the destination on port `22`. 
    * This is not necessary if you haven't modified the [access control policies](https://tailscale.com/docs/features/access-control/acls) in your account, as the [default access control policy](https://tailscale.com/docs/reference/examples/acls#allow-all-default-acl) grants access to all devices in the tailnet.
    * This may require modifying the tailnet policy file to include SSH. You need to be an [Admin or Network admin](https://tailscale.com/docs/reference/user-roles) to modify the tailnet policy file.
  * Ensure an access control policy exists that lets the source use SSH to connect to the destination machine using Tailscale SSH. 
    * This is not necessary if you haven't modified the [access control policies](https://tailscale.com/docs/features/access-control/acls) in your account, as the [SSH access rules in the default access control policy](https://tailscale.com/docs/features/tailscale-ssh#ssh-access-rules-in-default-acl) grant SSH access to all devices.
    * This will require modifying the tailnet policy file to include SSH policies. You need to be an [Admin or Network admin](https://tailscale.com/docs/reference/user-roles) to modify the tailnet policy file.


### [Advertise SSH on the host](https://tailscale.com/docs/features/tailscale-ssh#advertise-ssh-on-the-host)
...
### [Ensure Tailscale SSH is permitted in access control policies](https://tailscale.com/docs/features/tailscale-ssh#ensure-tailscale-ssh-is-permitted-in-access-control-policies)
...
SSH access rules are defined in the tailnet policy file, which you can edit through the [Access controls](https://console.tailscale.com/admin/acls) page of the admin console, or through the Tailscale [API](https://tailscale.com/docs/reference/tailscale-api).
...
The source where a connection originates from. This can be a user, group, tag, `user:*@<domain>`, or [`autogroup`](https://tailscale.com/docs/reference/syntax/policy-file#autogroups).
This cannot be a bare wildcard `*`.
Granting access to `autogroup:member` also grants access to [external invited users](https://tailscale.com/docs/features/sharing/how-to/invite-any-user) if the destination node is [shared](https://tailscale.com/docs/features/sharing) with them, even if they have no nodes in your tailnet.
...
##### [`acceptEnv` examples](https://tailscale.com/docs/features/tailscale-ssh#acceptenv-examples)  
...
#### [Order of evaluation](https://tailscale.com/docs/features/tailscale-ssh#order-of-evaluation)
...
The only kinds of connections that are allowed are:
  ...
  * From a user to a [shared](https://tailscale.com/docs/features/sharing) tagged device, as any user including `root`.
...
You can use the [visual policy editor](https://tailscale.com/docs/features/visual-editor) to manage your tailnet policy file. Refer to the [visual editor reference](https://tailscale.com/docs/reference/visual-editor) for guidance on using the visual editor.
#### [SSH access rules in default access control policy](https://tailscale.com/docs/features/tailscale-ssh#ssh-access-rules-in-default-access-control-policy)
By default, new tailnets or existing tailnets that have not modified their access control policies have a usable but conservative Tailscale SSH access rule. To facilitate connectivity, Tailscale SSH is configured with a [default policy](https://tailscale.com/docs/reference/examples/acls#allow-all-default-acl) that lets the user access their own devices using check mode, as either `root` or non-`root`:
...
#### [SSH access rule examples](https://tailscale.com/docs/features/tailscale-ssh#ssh-access-rule-examples)
...
You can use the [visual policy editor](https://tailscale.com/docs/features/visual-editor) to manage your tailnet policy file. Refer to the [visual editor reference](https://tailscale.com/docs/reference/visual-editor) for guidance on using the visual editor.
### [Connect over SSH](https://tailscale.com/docs/features/tailscale-ssh#connect-over-ssh)
...
This example uses the [MagicDNS](https://tailscale.com/docs/features/magicdns) hostname.
The MagicDNS hostname is automatically generated from the machine's name. You can [edit the machine name](https://tailscale.com/docs/concepts/machine-names#renaming-a-machine) if you want to use a different name.
...
You can also connect over SSH to a node that is tagged and has been [shared](https://tailscale.com/docs/features/sharing) with you, as long as the destination host has Tailscale configured with SSH and the destination's access control policy lets you connect over SSH.
...
### [Transition from your existing SSH client to Tailscale SSH](https://tailscale.com/docs/features/tailscale-ssh#transition-from-your-existing-ssh-client-to-tailscale-ssh)
When you enable Tailscale SSH, Tailscale claims port `22` for the Tailscale IP address (that is, only for traffic coming from your tailnet) on the devices for which you have enabled SSH, and routes SSH traffic for the device to an SSH server run by Tailscale, instead of your standard SSH server. Your SSH configuration (`/etc/ssh/sshd_config`) and keys (`~/.ssh/authorized_keys`) file will not be modified, which means that other SSH connections to the same host, not made over Tailscale, will still work.
### [Rotate keys](https://tailscale.com/docs/features/tailscale-ssh#rotate-keys)
...
## [Disable Tailscale SSH](https://tailscale.com/docs/features/tailscale-ssh#disable-tailscale-ssh)
### [Disable Tailscale SSH from a device](https://tailscale.com/docs/features/tailscale-ssh#disable-tailscale-ssh-from-a-device)
...
### [Block other devices from connecting to your device over SSH](https://tailscale.com/docs/features/tailscale-ssh#block-other-devices-from-connecting-to-your-device-over-ssh)
...
To [block incoming connections](https://tailscale.com/docs/features/client/manage-preferences#allow-incoming-connections) from the tailnet to your device, including Tailscale SSH connections:
...
### [Revoke a user's ability to SSH](https://tailscale.com/docs/features/tailscale-ssh#revoke-a-users-ability-to-ssh)
To remove a user's ability to SSH to a device, modify the [access rule](https://tailscale.com/docs/reference/syntax/policy-file#acls) specifying the user's ability to SSH to the device. An update to access control policies will be pushed to the device, and remove the user's access, almost instantaneously. You can still allow the user to connect to the device, but not use Tailscale SSH, if that is desired.
...
### [Disable Tailscale SSH from your tailnet](https://tailscale.com/docs/features/tailscale-ssh#disable-tailscale-ssh-from-your-tailnet)
...
You do not need to remove or modify other [access rules](https://tailscale.com/docs/reference/syntax/policy-file#acls) if you still want users to be able to access the specified devices, but not using Tailscale SSH.
### [Misconfigurations](https://tailscale.com/docs/features/tailscale-ssh#misconfigurations)
...
If the access control policy is missing, this can be changed in the tailnet policy file in the [Access controls](https://console.tailscale.com/admin/acls) page of the admin console. You need to be an [Admin or Network admin](https://tailscale.com/docs/reference/user-roles) to modify the tailnet policy file.
If SSH is not advertised on the device, connect to the device over Tailscale, then run `tailscale set --ssh`.
## [Configure Tailscale SSH with check mode](https://tailscale.com/docs/features/tailscale-ssh#configure-tailscale-ssh-with-check-mode)
SSH check mode lets you require SSH connections to be further verified before establishing the connection. Check mode requires re-authentication on the device initiating the connection. The SSH initialization provides a URL for signing in. The sign-in attempt may also trigger any identity provider [multifactor authentication](https://tailscale.com/docs/multifactor-auth) (MFA) or other risk-based challenges.
...
You can use the [visual policy editor](https://tailscale.com/docs/features/visual-editor) to manage your tailnet policy file. Refer to the [visual editor reference](https://tailscale.com/docs/reference/visual-editor) for guidance on using the visual editor.
## [Limitations](https://tailscale.com/docs/features/tailscale-ssh#limitations)
...
You can write [SSH tests](https://tailscale.com/docs/reference/syntax/policy-file#sshtests) to assert that your SSH access rules are working as expected.
...
### [OS user authentication on the client](https://tailscale.com/docs/features/tailscale-ssh#os-user-authentication-on-the-client)
...
You can mitigate this concern by using [check mode](https://tailscale.com/docs/features/tailscale-ssh#configure-tailscale-ssh-with-check-mode) with `checkPeriod` set to `always`, which will prompt for SSO-based approval on every new connection.
...
## [Additional information](https://tailscale.com/docs/features/tailscale-ssh#additional-information)
  * Tailscale CLI command: [`tailscale set --ssh`](https://tailscale.com/docs/reference/tailscale-cli#set)
  * Tailscale CLI command: [`tailscale up --ssh`](https://tailscale.com/docs/reference/tailscale-cli/up)
  * tailnet policy file section: [`ssh`](https://tailscale.com/docs/reference/syntax/policy-file#ssh)
