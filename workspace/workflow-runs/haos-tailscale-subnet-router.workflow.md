...
- [ ] 第 1 章 结论前置：同网段到底要不要开 SNAT（约 1000 字）
- [ ] 第 2 章 装插件并完成登录授权（约 800 字）
- [ ] 第 3 章 内网穿透：不开口、不做反代就远程访问（约 1000 字）
- [ ] 第 4 章 子路由：把 HAOS 变成 tailnet 的网关（约 1700 字）
- [ ] 第 5 章 在远程客户端上接收路由（约 700 字）
- [ ] 第 6 章 验收：你自己跑一遍六步（约 900 字）
- [ ] 第 7 章 排错：按顺序查，别猜（约 1400 字）
- [ ] 第 8 章 边界、未核验声明与下一步（约 600 字）
