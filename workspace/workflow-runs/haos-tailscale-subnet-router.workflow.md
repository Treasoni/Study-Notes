...
| 规模 | 767 行 / 11164 中文字（11145 为收尾修补前的数；3 个 warning 补标题 +18 字） |
...
**改写安全校验**：美化后把成品与 P5 成品做**归一化逐行比对**（剥掉 callout 前缀、把三种小结标记归一），差异行仅有「新增 frontmatter 13 行」+「**4 行形式替换**」（2 处 `### 本章小结` 标题 + 2 处 `**本章小结**` 粗体，各改为同文 `[!summary]` Callout；第 3 章那处本就是 Callout，无差异行）——**小结的格式改造没有产生任何内容差异行**，证明它纯属语法转换、未动一字。另有 3 处双链为本阶段改动，其中第 8 章那两处是**兑现原文自身的承诺**（原文写着「本篇只给链接，不展开」，此前给的却是项目代号而非链接）。

**收尾修补（用户授权「你来决定」后执行，1 处）：**

- **3 个无标题 `[!warning]` 补中文标题。** 原为裸 `> [!warning]`，Obsidian 会渲染成默认英文标题「Warning」，与另外 8 个中文标题的 warning 不一致。补为：3.2.1 节`别填 100.64.0.0/10`、3.4 节`这条命令有个已知后果`、7.4.3 节`别去 remount 根文件系统`。标题**只取该 callout 自身正文的首个要点作标签，未新增任何论断**。修后 `[!warning]` 11 处全部带标题，裸 callout = 0。
- 修补后复验：767 行 / 11164 中文字；H1×1、H2×9、H3×32、H4×9、H5+ 0；48 个围栏标记（24 块）全带语言标识；callout 后异常空行 0；3 条双链目标均存在。

**两项待定项已决（用户 2026-09-12 授权「你来决定」）：**

1. **目录锚点：保留 GitHub slug，不改纯文本。** 依据不是偏好而是既有事实——同目录 `Tailscale子网路由器部署教程.md` 用同一种 slug，且已在该 vault 长期使用；改成纯文本会让同一目录出现两种目录写法，且失去跳转。注意 **fallback 成本极低**：即便某条锚点不跳，点击也只是无响应，不会新建空笔记、不会丢内容，改回纯文本是一分钟的事。
2. **`[!summary]` Callout：保留，不切回标题。** 同 P6 理由（`note-system.md` 语义定义 + 避免 5 个同名标题导致目录消歧）。补一条新证据：组装器原本三种形态并存，说明这正是它没统一的地方；统一成 Callout 同时消除了这处不一致。
3. 两项均**保持现状、未改动成品**，因此上方 P6 改写安全校验结论继续有效。
...
| 2026-09-12 | P6 | 目录锚点能否在 Obsidian 内正常跳转，**无法从外部验证**（需 Obsidian 运行时） | 用户授权「你来决定」后决定**保留 slug 写法**，依据是既有笔记 `Tailscale子网路由器部署教程.md` 使用同一种 slug 且长期在用。**该跳转仍未经验证**，但失败代价明确且极小：点击无响应，不新建空笔记、不丢内容；若要改回纯文本列表，一分钟即可 |
