...
| `cron_mode`  | `deny`  | How [cron jobs](https://hermes-agent.nousresearch.com/docs/user-guide/features/cron) behave headlessly when they trigger a dangerous-command prompt. `deny` blocks the command (the agent must find another path); `approve` auto-approves everything in cron context.  |  
| `single_query_mode`  | `deny`  | How one-shot [`hermes chat -q`](https://hermes-agent.nousresearch.com/docs/user-guide/cli) sessions behave when they trigger a dangerous-command prompt. A `-q` session runs a single turn and exits with no user waiting to answer prompts; `deny` blocks the command (the agent must find another path), `approve` auto-approves everything in single-query context. Mirrors `cron_mode`.  |  
...
For destructive session slash commands (`/clear`, `/new` / `/reset`, `/undo`, `/quit --delete` — `/exit --delete` is an alias), the CLI also prompts for confirmation before running them. See [Slash Commands — Confirmation prompts for destructive commands](https://hermes-agent.nousresearch.com/docs/reference/slash-commands#confirmation-prompts-for-destructive-commands).
...
### User-Defined Deny Rules (`approvals.deny`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#user-defined-deny-rules-approvalsdeny "Direct link to user-defined-deny-rules-approvalsdeny")
...
### Mining Approval History (`hermes approvals suggest`)[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#mining-approval-history-hermes-approvals-suggest "Direct link to mining-approval-history-hermes-approvals-suggest")
...
hermes approvals suggest --apply1,3# merge picks into command_allowlist
...
## File Write Safety[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#file-write-safety "Direct link to File Write Safety")
Before `write_file` or `patch` touches disk, Hermes checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#file-mutation-verifier) over the assistant's closing summary.
### Protected paths (always blocked)[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#protected-paths-always-blocked "Direct link to Protected paths \(always blocked\)")
...
### HERMES_WRITE_SAFE_ROOT (optional sandbox)[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#hermes_write_safe_root-optional-sandbox "Direct link to HERMES_WRITE_SAFE_ROOT \(optional sandbox\)")
...
Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [HERMES_WRITE_SAFE_ROOT](https://hermes-agent.nousresearch.com/docs/reference/environment-variables#hermes_write_safe_root).
### Cron and other Hermes state[​](https://hermes-agent.nousresearch.com/docs/user-guide/security#cron-and-other-hermes-state "Direct link to Cron and other Hermes state")
Do not ask the agent to `patch` `~/.hermes/cron/jobs.json` directly. Use the `cronjob` tool, [`hermes cron`](https://hermes-agent.nousresearch.com/docs/user-guide/features/cron), or `/cron` — they update the job store through the supported API. The same applies to other Hermes control files when write safety blocks direct edits.
...
dockerexec-u hermes hermes-agent hermes pairing approve telegram ABC12DEF
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `hermes tools`.
  * [Dangerous Command Approval](https://hermes-agent.nousresearch.com/docs/user-guide/security#dangerous-command-approval)
    ...
    * [User-Defined Deny Rules (`approvals.deny`)](https://hermes-agent.nousresearch.com/docs/user-guide/security#user-defined-deny-rules-approvalsdeny)
    ...
    * [Mining Approval History (`hermes approvals suggest`)](https://hermes-agent.nousresearch.com/docs/user-guide/security#mining-approval-history-hermes-approvals-suggest)
  * [File Write Safety](https://hermes-agent.nousresearch.com/docs/user-guide/security#file-write-safety)
    * [Protected paths (always blocked)](https://hermes-agent.nousresearch.com/docs/user-guide/security#protected-paths-always-blocked)
    * [HERMES_WRITE_SAFE_ROOT (optional sandbox)](https://hermes-agent.nousresearch.com/docs/user-guide/security#hermes_write_safe_root-optional-sandbox)
    * [Cron and other Hermes state](https://hermes-agent.nousresearch.com/docs/user-guide/security#cron-and-other-hermes-state)
