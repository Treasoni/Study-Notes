...
本笔记是一份从零配置 Debian 最小化系统 sudo 权限的实战指南：先弄清「最小化安装为什么没有 sudo」以及 su/sudo 与 sudo 组机制的原理，再走一遍「安装 sudo → 加组授权 → 验证」三步标准流程，随后进阶到 visudo 与 sudoers 语法、NOPASSWD 免密，最后覆盖三类常见故障的排错与安全最佳实践。共八章，按顺序阅读即可完整掌握。
