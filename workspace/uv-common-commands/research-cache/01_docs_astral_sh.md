...
# [Caching](https://docs.astral.sh/uv/concepts/cache/#caching)
## [Dependency caching](https://docs.astral.sh/uv/concepts/cache/#dependency-caching)
...
## [Dynamic metadata](https://docs.astral.sh/uv/concepts/cache/#dynamic-metadata)
...
## [Cache safety](https://docs.astral.sh/uv/concepts/cache/#cache-safety)
...
## [Clearing the cache](https://docs.astral.sh/uv/concepts/cache/#clearing-the-cache)
uv provides a few different mechanisms for removing entries from the cache:
  ...
  * `uv cache prune` removes all _unused_ cache entries and all centralized project environments. For example, the cache directory may contain entries created in previous uv versions that are no longer necessary and can be safely removed. Centralized project environments are recreated as needed. `uv cache prune` is safe to run periodically, to keep the cache directory clean.


By default, cache cleanup estimates the disk space reclaimed. Enable the `cache-physical-space` [preview feature](https://docs.astral.sh/uv/concepts/preview/) for a more accurate estimate that accounts for hardlinks and copy-on-write clones:

```
$ uvcacheclean--preview-featurescache-physical-space
...
## [Cache directory](https://docs.astral.sh/uv/concepts/cache/#cache-directory)
...
## [Cache versioning](https://docs.astral.sh/uv/concepts/cache/#cache-versioning)
