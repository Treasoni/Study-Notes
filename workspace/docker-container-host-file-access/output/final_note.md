...
本笔记整理如何让 Docker 容器内部的服务控制/处理容器外或宿主机的文件。核心思路是：先用三种挂载方式（bind mount / named volume / tmpfs）做选型，再分别掌握命令行与 Compose 的挂载写法，接着解决容器内外文件的权限与属主问题，明确 docker.sock 与可写挂载的安全边界，最后结合 Docker Desktop 在 macOS/Windows 上的平台差异，走查一个 Web 服务读写宿主目录的完整实战案例。全篇共 7 章，按顺序阅读即可。

## 目录

1. [第 1 章：三种挂载方式怎么选（bind / volume / tmpfs）](#第-1-章三种挂载方式怎么选bind-volume-tmpfs)
2. [第 2 章：命令行挂载：docker run -v 与 --mount](#第-2-章命令行挂载docker-run--v-与---mount)
3. [第 3 章：Compose 挂载：volumes 长短语法与顶层声明](#第-3-章Compose-挂载volumes-长短语法与顶层声明)
4. [第 4 章：权限与属主：permission denied 的根因与修复](#第-4-章权限与属主permission-denied-的根因与修复)
5. [第 5 章：安全边界：只读、SELinux 与 docker.sock 风险](#第-5-章安全边界只读SELinux-与-dockersock-风险)
6. [第 6 章：Docker Desktop 的差异：macOS/Windows 挂载性能](#第-6-章Docker-Desktop-的差异macOSWindows-挂载性能)
7. [第 7 章：实战案例：Web 服务读写宿主目录的完整配置](#第-7-章实战案例Web-服务读写宿主目录的完整配置)
