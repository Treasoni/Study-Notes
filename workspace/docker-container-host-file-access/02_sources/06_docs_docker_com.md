...
title: "Define services in Docker Compose | Docker Docs"
...
```
services:image:nginx:latest- "8080:80"image:postgres:18environment:POSTGRES_USER:examplePOSTGRES_DB:exampledb
```
...
```
services:image:nginxvolumes:- type:bindsource:./proxy/nginx.conftarget:/etc/nginx/conf.d/default.confread_only:truedepends_on:- backendbackend:context:backendtarget:builder
```
...
```
services:image:busyboxblkio_config:weight:300weight_device:- path:/dev/sdaweight:400device_read_bps:- path:/dev/sdbrate:'12mb'device_read_iops:- path:/dev/sdbrate:120device_write_bps:- path:/dev/sdbrate:'1024k'device_write_iops:- path:/dev/sdbrate:30
```
...
```
services:image:redis:latestconfigs:- my_config- my_other_configconfigs:my_config:file:./my_config.txtmy_other_config:external:true
```
...
```
services:image:redis:latestconfigs:- source:my_configtarget:/redis_configuid:"103"gid:"103"mode:0440configs:my_config:external:truemy_other_config:external:true
```
...
```
services:myservice:image:myimage:latestcredential_spec:config:my_credential_specconfigs:my_credentials_spec:file:./my-credential-spec.json
```
...
```
services:build:.depends_on:image:redisimage:postgres:18
```
...
```
services:build:.depends_on:condition:service_healthyrestart:truecondition:service_startedimage:redisimage:postgres:18
```
...
```
services:networks:app_net:driver_opts:com.docker.network.bridge.host_binding_ipv4:"127.0.0.1"
```
...
```
entrypoint:- zend_extension=/usr/local/lib/php/extensions/no-debug-non-zts-20100525/xdebug.so- memory_limit=-1- vendor/bin/phpunit
```
...
The `format` attribute lets you use an alternative file format for the `env_file`. When not set, `env_file` is parsed according to the Compose rules outlined in [`Env_file` format](https://docs.docker.com/reference/compose-file/services/#env_file-format).
...
```
env_file:- path:./default.envformat:raw
```
...
#### [Restrictions](https://docs.docker.com/reference/compose-file/services/#restrictions)
...
```
services:common:image:busyboxenvironment:PORT:80extends:service:commonenvironment:PORT:8080
```

Produces the following configuration for the `cli` service. The same output is produced if array syntax is used.

```
environment:PORT:8080image:busybox
```
...
```
services:common:image:busyboxvolumes:- common-volume:/var/lib/backup/data:rwextends:service:commonvolumes:- cli-volume:/var/lib/backup/data:ro
```
...
```
image:busyboxvolumes:- cli-volume:/var/lib/backup/data:ro
```
...
```
services:image:busyboxuser:rootcommon:image:busyboxextends:service:baseextends:service:common
```
...
```
services:common:image:busyboxsecurity_opt:- label=role:ROLEextends:service:commonsecurity_opt:- label=user:USER
```
...
```
external_links:- database:mysql- database:postgresql
```
...
```
services:myservice:image:alpinegroup_add:
```
...
```
healthcheck:test:["CMD","curl","-f","http://localhost"]interval:1m30stimeout:10sretries:3start_period:40sstart_interval:5s
```
...
```
services:image:alpine:latestinit:true
```
...
```
services:label_file:./app.labelslabel_file:- ./app.labels- ./additional.labels
```

If a label is defined in both the `label_file` and the `labels` attribute, the value in [labels](https://docs.docker.com/reference/compose-file/services/#labels) takes precedence.
...
> Note
> Container runtimes might reject this value, for example Docker Engine >= v25.0. In that case, you should use [networks.mac_address](https://docs.docker.com/reference/compose-file/services/#mac_address) instead.
...
```
services:short_syntax:image:appmodels:- my_modellong_syntax:image:appmodels:my_model:endpoint_var:MODEL_URLmodel_var:MODEL
```
...
#### [Implicit default network](https://docs.docker.com/reference/compose-file/services/#implicit-default-network)
...
```
services:some-service:image:foonetworks:default:{}
```

If you want the service to not be connected a network, you must set [`network_mode: none`](https://docs.docker.com/reference/compose-file/services/#network_mode).
...
```
services:frontend:image:example/webappnetworks:- front-tier- back-tiermonitoring:image:example/monitoringnetworks:backend:image:example/backendnetworks:back-tier:aliases:- databasealiases:networks:front-tier:{}back-tier:{}admin:{}
```

#### [`interface_name`](https://docs.docker.com/reference/compose-file/services/#interface_name)
...
```
services:backend:image:alpinecommand:ip link shownetworks:back-tier:interface_name:eth0
```
...
```
services:frontend:image:example/webappnetworks:front-tier:ipv4_address:172.16.238.10ipv6_address:2001:3984:3989::10networks:front-tier:driver:defaultconfig:- subnet:"172.16.238.0/24"- subnet:"2001:3984:3989::/64"
```
...
```
services:image:busyboxcommand:topnetworks:app_net:link_local_ips:- 57.123.22.11- 57.123.22.13networks:app_net:driver:bridge
```
...
```
services:networks:app_net:driver_opts:foo:"bar"
```
...
```
services:image:busyboxcommand:topnetworks:app_net_1:app_net_2:gw_priority:1app_net_3:networks:app_net_1:app_net_2:app_net_3:
```
...
`priority` does not affect which network is selected as the default gateway. Use the [`gw_priority`](https://docs.docker.com/reference/compose-file/services/#gw_priority) attribute instead.
...
```
services:image:busyboxcommand:topnetworks:app_net_1:priority:1000app_net_2:app_net_3:priority:100networks:app_net_1:app_net_2:app_net_3:
```
...
```
platform:darwinplatform:windows/amd64platform:linux/arm64/v8
```
...
```
- "3000"- "3000-3005"- "8000:8000"- "9090-9091:8080-8081"- "49100:22"- "8000-9000:80"- "127.0.0.1:8001:8001"- "127.0.0.1:5000-5010:5000-5010"- "::1:6000:6000"- "[::1]:6001:6001"- "6060:6060/udp"
```
...
```
- name:webtarget:80host_ip:127.0.0.1published:"8080"protocol:tcpapp_protocol:httpmode:host- name:web-securedtarget:443host_ip:127.0.0.1published:"8083-9000"protocol:tcpapp_protocol:httpsmode:host
```
...
```
services:post_start:- command:./do_something_on_startup.shuser:rootprivileged:trueenvironment:- FOO=BAR
```
...
```
services:image:myapp:latestdepends_on:condition:service_healthypre_start:- command:["./manage.py","migrate"]- image:busyboxcommand:sh -c 'chown -R 1000:1000 /data'volumes:- data:/dataimage:postgres:16volumes:
```
...
Configuration is equivalent to [`post_start`](https://docs.docker.com/reference/compose-file/services/#post_start).
...
```
services:frontend:image:frontendprofiles:["frontend"]phpmyadmin:image:phpmyadmindepends_on:profiles:- debug
```
...
```
database:provider:type:awesomecloudoptions:type:mysqlfoo:barimage:myappdepends_on:- database
```
...
```
services:image:nginxpull_policy:every_12h
```
...
```
restart:"no"restart:alwaysrestart:on-failurerestart:on-failure:3restart:unless-stopped
```
...
```
image:busybox:latestcommand:trueruntime:runc
```
...
```
services:frontend:image:example/webappsecrets:- server-certificatesecrets:server-certificate:file:./server.cert
```
...
```
services:frontend:image:example/webappsecrets:- source:my-tokenuid:"103"gid:"103"mode:0o440secrets:my-token:environment:"MY_TOKEN"
```
...
```
services:- /data:mode=755,uid=1009,gid=1009
```
...
### [`use_api_socket`](https://docs.docker.com/reference/compose-file/services/#use_api_socket)
...
```
services:backend:image:example/backendvolumes:- type:volumesource:db-datatarget:/datavolume:nocopy:truesubpath:sub- type:bindsource:/var/run/postgres/postgres.socktarget:/var/run/postgres/postgres.sockvolumes:db-data:
```
