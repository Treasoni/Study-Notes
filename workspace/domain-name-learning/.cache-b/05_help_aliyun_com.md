...
title: "ICP备案域名核验要求与准备-备案-阿里云-备案(Filing Service)-阿里云帮助中心"
scraped_at: 2026-09-04T04:45:07+00:00
---

输入文档关键字查找
根据[《工业和信息化部关于规范互联网信息服务使用域名的通知》](https://beian.miit.gov.cn/#/Integrated/lawStatute)，阿里云ICP代备案管理系统会对域名注册信息进行核验，**如果您未能通过域名核验，阿里云将按照通信管理部门的要求停止为您提供接入服务** 。本文为您介绍域名核验的范围和要求。
## 域名核验的范围
...
| 首次备案  | 核验全部域名。  |  
| 新增互联网信息服务  | 核验新增的全部域名。  |  
| 变更备案  | 核验对应主体下2018年1月1日后备案成功的域名。  | 部分省份管局会核验您的域名后缀在2018年1月1日后是否获得工信部批复，未获得工信部批复的域名无法进行变更备案。  |  
| 注销主体  | 不进行域名核验。  | 注销备案后，如果重新提交域名备案，需通过备案域名核验。  |  
| 注销互联网信息服务 |  
| 取消接入  | 不进行域名核验。  | 取消接入后，再接入暂不进行域名核验。  |  
## **域名核验要求**
ICP备案的域名需同时满足以下要求，方可完成备案申请。若域名仅解析至非中国内地的服务器，则无需进行备案。
在逐项核查前，可使用备案首页的快捷工具批量检测域名。访问[阿里云备案首页](https://beian.aliyun.com/)，在**检测域名是否可备案** 快捷工具中输入域名并点击**检测** 。系统自动检查域名后缀是否获工信部批复、域名注册商是否为已批复机构，并返回检测结果。
### **要求一：域名后缀（顶级域名）获得工信部批复**
访问[工信部网站](https://domain.miit.gov.cn/)的**中国互联网域名体系** 页面，查看已批复的域名后缀（顶级域名），未列入[域名后缀列表](https://help.aliyun.com/zh/icp-filing/basic-icp-service/user-guide/prepare-and-check-the-domain-name#2b5205a99bd1c)中的域名无法进行ICP备案。
部分国际通用顶级域名（如.org）未获得工信部批复，无法在阿里云进行ICP备案。如持有此类域名，可将网站部署在中国内地以外的服务器上，或更换为已获工信部批复的域名后缀。
### **要求二：域名的注册商为已批复机构**
  1. 通过页面获取当前域名的注册商信息。
  2. 前往[工信部网站](https://domain.miit.gov.cn/)的**域名注册服务机构审批** 情况页面，查询该注册商的批复状态。
     * 当域名注册商未获批复时，需要将域名转移至已获批复的域名注册商，详细请参见[域名转入阿里云](https://help.aliyun.com/zh/dws/user-guide/transfer-a-domain-name-to-alibaba-cloud#task-1958371)。
     * 当域名注册商获批复时，可直接进行备案。
通过WHOIS查询域名的注册商信息（例如查询`taobao.com`，其注册商显示为**Alibaba Cloud Computing (Beijing) Co., Ltd.** ），然后在**域名注册服务机构审批情况** 页面中确认该注册商（对应**阿里云计算有限公司** ）已获得批复。
阿里云国际站域名服务（alibabacloud.com）为国际域名注册商。在此站点注册的域名需转入中国内地域名注册商，并完成域名实名认证才可以进行ICP备案。


### **要求三：域名持有人信息与备案主体信息一致**
  1. _域名实名认证或过户信息修改后，同步到工信部系统需要2～3天才可进行备案_ ：
     * 阿里云注册的域名：可先提交备案至阿里云初审，初审通过后，备案系统会自动延期并提交管局。域名认证步骤请参见。
     * 非阿里云注册的域名：需要等待2~3天，再提交备案至阿里云初审。
  2. _用于备案的域名有足够长的有效期_ ：
在页面查询域名的到期时间。各省对于域名的有效期有不同要求，但应至少确保域名距离到期日期大于45天。如果不满足要求请您先[域名续费](https://help.aliyun.com/zh/dws/user-guide/domain-name-renewal1#concept-k34-1xw-12b)。
  3. _域名实名信息和主办者信息一致_ ：
    1. 在的**域名列表** 页面，单击相应域名的名称后**操作** 列的**管理** ，查看域名的实名认证信息。
      1. 个人备案：域实名信息（持有人名称、证件类型、证件号码）需与主办者信息保持一致。
      2. 企业备案：域名实名信息（持有人名称、证件类型、证件号码）需与主办单位信息或主体负责人一致。且**实名认证证件类型** 必须与**主办单位证件类型** 吻合。
> 常见单位类型对应证件类型可参见[常见单位类型对应证件样式](https://help.aliyun.com/zh/account/unified-social-credit-code-certificates-examples)。
    2. 如果需要提供域名持有者信息截图（例如进行 App 备案时），请注意：阿里云域名控制台出于安全考虑，会对证件号码进行脱敏处理（例如显示为 `911**********0XW`），无法从控制台直接截图获取完整证件号码。
可以通过工业和信息化部（工信部）域名信息备案管理系统获取包含完整四要素（域名、单位名称、证件类型、证件号码）的截图，该截图是通信管理局认可的有效证明材料：
      1. 访问[工信部域名信息备案管理系统](https://domain.miit.gov.cn)，在查询框中输入域名并提交查询。
      2. 在查询结果页面确认以下信息均完整展示（无脱敏）：域名、单位名称、证件类型、证件号码。
      3. 对查询结果页面进行完整截图。
截图需在同一张图片中完整展示上述四要素，不可裁剪、拼接或 P 图。
    3. 仔细核对域名实名信息与备案填写信息中的主办者信息是否一致。
下面是常见的信息不一致情况，更多的不一致情况请参见[备案提示域名与主体信息不一致](https://help.aliyun.com/zh/dws/support/faq-about-domain-name-real-name-authentication#a49bcab74d8oo)。  
|   |   |  
| --- | --- |  
| **问题类型**  | **解决方法**  |  
| --- | --- |  
| 
      1. 域名认证信息为个人无法为企业备案性质进行备案。
      2. 域名认证信息没有证件类型和证件号码。
      3. 域名认证信息有证件类型，但和备案主体的证件类型不一致。
 | 
      1. 先[域名实名认证](https://help.aliyun.com/zh/dws/user-guide/how-to-complete-domain-name-authentication#section-klb-3bx-12b)。
      2. 将[域名持有者信息修改（过户）](https://help.aliyun.com/zh/dws/user-guide/modify-registrant-contact-information-or-transfer-domain-name-ownership#cce1d897d7j0i)后再进行备案申请。 认证信息修改后需等待2~3个工作日用于信息同步。
 |  
| 域名持有者（中文）的单位名称中没有正确使用中文括号。  |  中文全角括号（）与英文半角括号()视觉上相似，但备案系统会严格校验两者与营业执照的一致性。 请前往[阿里云域名控制台](https://dc.console.aliyun.com/#/domain-list/all)的**域名列表** 页面，修改域名持有者信息（过户），将单位名称中的括号更正为与营业执照一致的中文全角括号。修改完成后需重新完成域名实名认证，认证信息同步需等待 2~3 个工作日，完成后即可提交备案申请。  |  
| 企业备案时，域名信息模板类型为"个人"（即域名持有人为个人，而非企业）。  | 若该个人是企业的法定代表人，则持有者信息可与主体负责人信息一致，无需修改。否则，需将域名过户给企业或法人，并确保信息模板中的持有人名称、证件类型、证件号码与备案主体完全一致。修改实名认证信息后，需等待 2~3 个工作日信息同步完成后再提交备案。  |  
| 误以为域名实名认证中的"联系人"信息也必须与备案主体一致。  | 备案仅要求域名的"持有者"（持有人名称、证件类型、证件号码）与备案主体信息一致。"联系人"是域名管理员的联系方式，无需与备案主体一致，不影响备案核验。  |  
| 误以为阿里云账号的实名认证类型（个人实名）会影响企业备案资格，或认为账号为个人实名则只能进行个人备案。  | 阿里云账号的实名认证类型不影响企业备案资格。企业备案的关键在于：域名信息模板必须选择"企业"类型，并填写与备案主体一致的企业名称、证件类型和证件号码。  |  


在进行ICP备案时，只需对主域名（如`example.com`）进行备案。备案完成后，其下的所有子域名（如`www.example.com`）无需额外备案即可正常使用。
## **附录：域名后缀列表**
### **国家顶级域名**
**点击查看详情**
  * **类别二级域名** ：  
...
## **常见问题**
  ...
  * [备案提示域名与主体信息不一致](https://help.aliyun.com/zh/dws/support/faq-about-domain-name-real-name-authentication#a49bcab74d8oo)
  * **谷歌购买的.org域名能否在阿里云备案？**
不能。.org域名后缀未获得工信部批复，不在可备案域名后缀列表中，因此无法在阿里云进行ICP备案。请更换为已获工信部批复的域名后缀，或将网站部署在中国内地以外的服务器上。
...
### 联系我们
4008013260[售前咨询](https://smartservice.console.aliyun.com/pre-sale/chat?entrance=201&referrer=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Ficp-filing%2Fbasic-icp-service%2Fuser-guide%2Fprepare-and-check-the-domain-name)[售后在线](https://smartservice.console.aliyun.com/service/robot-chat?entrance=201&referrer=https%3A%2F%2Fhelp.aliyun.com%2Fzh%2Ficp-filing%2Fbasic-icp-service%2Fuser-guide%2Fprepare-and-check-the-domain-name)
### 其他服务
[我要建议](https://www.aliyun.com/connect/home)[我要投诉](https://www.aliyun.com/complaint)
登录以查看您的控制台资源
管理云资源
