...
[Tailscale without Tailscale. Meet tailcat. Read the blog →](https://tailscale.com/blog/tailcat)
### Documentation
Close navigation
# Tailscale Funnel
Last validated: Jan 20, 2026|Copy as Markdown|[View as Markdown](https://tailscale.com/docs/features/tailscale-funnel.md)
Tailscale Funnel is currently [in beta](https://tailscale.com/docs/reference/tailscale-release-stages#beta). To try it, follow the steps below to enable it for your network using Tailscale v1.38.3 or later.
Tailscale Funnel is available for [all plans](https://tailscale.com/pricing).
...
If you'd like to share local services only with other devices in your tailnet, use [Tailscale Serve](https://tailscale.com/docs/features/tailscale-serve) instead. Also check out [Funnel vs. sharing](https://tailscale.com/docs/reference/funnel-vs-sharing).
...
Explore the following topics to find more information about Tailscale Funnel:
  * [Get started with Funnel](https://tailscale.com/docs/features/tailscale-funnel#get-started-with-funnel)
  * [How Funnel works](https://tailscale.com/docs/features/tailscale-funnel#how-funnel-works)
  * [The `tailscale funnel` command](https://tailscale.com/docs/reference/tailscale-cli/funnel)
  * [Troubleshoot Funnel](https://tailscale.com/docs/features/tailscale-funnel#troubleshooting)
...
The process, from creating a Funnel to accessing a resource shared through it, involves:
  * [Creating and sharing the Funnel URL](https://tailscale.com/docs/features/tailscale-funnel#creating-and-sharing-the-funnel-url).
  * [Resolving the Funnel URL to an IP address](https://tailscale.com/docs/features/tailscale-funnel#resolving-the-funnel-url-to-an-ip-address).
  * [Establishing an encrypted proxy](https://tailscale.com/docs/features/tailscale-funnel#establishing-an-encrypted-proxy).
  * [Serving the shared resource through the proxy](https://tailscale.com/docs/features/tailscale-funnel#serving-the-shared-resource-through-the-proxy).
...
Funnel relay servers do not decrypt the traffic between public devices and your device. This ensures that Tailscale cannot access or read any content. It maintains end-to-end [encryption](https://tailscale.com/docs/concepts/tailscale-encryption) and privacy.
...
## [Get started with Funnel](https://tailscale.com/docs/features/tailscale-funnel#get-started-with-funnel)
Tailscale Funnel is disabled by default, but you can use the [Tailscale CLI](https://tailscale.com/docs/reference/tailscale-cli) to enable the Funnel service and create Funnels.
### [Requirements and limitations](https://tailscale.com/docs/features/tailscale-funnel#requirements-and-limitations)
Funnel requires the following to work:
  ...
  * [HTTPS](https://tailscale.com/docs/how-to/set-up-https-certificates) enabled and valid HTTPS certificates for your tailnet.
  * A funnel [node attribute](https://tailscale.com/docs/reference/syntax/policy-file#node-attributes) in your tailnet policy file. This attribute tells Tailscale which tailnet users can use Funnel.


Additionally, Funnel has the following limitations:
  * Funnel can only use DNS names in your [tailnet's domain](https://tailscale.com/docs/concepts/tailnet-name) (`tailnet-name.ts.net`).
  ...
  * To use Funnel on macOS, you must use one of the [open source variants of the Tailscale application for macOS](https://tailscale.com/docs/concepts/macos-variants).
...
When you enable Funnel using the Tailscale CLI, Tailscale automatically creates valid HTTPS certificates and updates your tailnet policy file. By default, it lets any users in the `autogroup:member` [autogroup](https://tailscale.com/docs/reference/syntax/policy-file#autogroups) to use Funnel. You can adjust this by manually updating your tailnet policy file.
...
### [Enable Funnel](https://tailscale.com/docs/features/tailscale-funnel#enable-funnel)
Use the [`tailscale funnel`](https://tailscale.com/docs/reference/tailscale-cli/funnel) command to enable Funnel. The command triggers a web interface that prompts you to approve enabling Funnel. After you approve it, Tailscale will create valid HTTPS certificates for your tailnet and add a funnel node attribute to your tailnet policy file.
### [Create a Funnel](https://tailscale.com/docs/features/tailscale-funnel#create-a-funnel)
To create a Funnel, use the `tailscale funnel` command and pass the target you want to share. You can share a service, a file, or a directory. You can also optionally pass any of the [available flags](https://tailscale.com/docs/reference/tailscale-cli/funnel#funnel-command-flags).
...
When you use Tailscale Funnel, the Funnel relay servers show up in your device's list of Tailscale peers. All peers using the command [`tailscale status --json`](https://tailscale.com/docs/reference/tailscale-cli#status) display.
Sharing files and directories with Funnel only works with the [open source variants](https://tailscale.com/docs/concepts/macos-variants) of the Tailscale client for macOS. Other variants have sandbox limitations.
...
## [Resources](https://tailscale.com/docs/features/tailscale-funnel#resources)
  * Explore the collection of [Funnel use case examples](https://tailscale.com/docs/reference/examples/funnel) for inspiration and ideas.
  * [Use the PROXY protocol](https://tailscale.com/docs/reference/tailscale-cli/funnel#use-the-proxy-protocol).
  * [Use the TCP forwarder](https://tailscale.com/docs/reference/tailscale-cli/funnel#use-a-tcp-forwarder).
  * [Reset the Funnel configuration](https://tailscale.com/docs/reference/tailscale-cli/funnel#reset-tailscale-funnel).


## [Troubleshooting](https://tailscale.com/docs/features/tailscale-funnel#troubleshooting)
Trouble with Funnels is usually related to one of the following issues:
  * [Funnel node attribute](https://tailscale.com/docs/features/tailscale-funnel#funnel-node-attribute)
  * [HTTPS certificates](https://tailscale.com/docs/features/tailscale-funnel#https-certificates)
  * [Access controls](https://tailscale.com/docs/features/tailscale-funnel#access-controls)
  * [DNS propagation](https://tailscale.com/docs/features/tailscale-funnel#dns-propagation)


### [Funnel node attribute](https://tailscale.com/docs/features/tailscale-funnel#funnel-node-attribute)
Tailscale Funnel requires a [node attribute](https://tailscale.com/docs/reference/syntax/policy-file#node-attributes) (`nodeAttrs`) of `funnel` in your tailnet policy file to tell Tailscale who can use Funnel. If you use the Tailscale CLI to enable Funnel, Tailscale ensures this requirement is met. Alternatively, you can manually add the node attribute for Funnel. You need to be an [Owner, Admin, or Network admin](https://tailscale.com/docs/reference/user-roles) of a tailnet to modify a tailnet policy file.
...
### [HTTPS certificates](https://tailscale.com/docs/features/tailscale-funnel#https-certificates)
Tailscale Funnel requires valid [HTTPS certificates](https://tailscale.com/docs/how-to/set-up-https-certificates) for your tailnet to automatically provision TLS certificates for your unique tailnet DNS name. If you use the Tailscale CLI to enable Funnel, Tailscale ensures this requirement is met.
### [Access controls](https://tailscale.com/docs/features/tailscale-funnel#access-controls)
If the funnel node attribute in your tailnet policy file doesn't permit you to use Funnel, you won't be able to. The default node attribute to enable Funnel includes all tailnet members by default, but [Owners, Admins, and Network admins](https://tailscale.com/docs/reference/user-roles) can modify this to further restrict access.
### [DNS propagation](https://tailscale.com/docs/features/tailscale-funnel#dns-propagation)
