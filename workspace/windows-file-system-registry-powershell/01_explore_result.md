...
## 用户决策结果（P1 检查点 · 2026-09-10 已确认）

| 决策项 | 选择 |
|--------|------|
| P2 投入规模 | **完整路线**：补齐 9 个缺口，处理约 20–25 篇来源 |
| 方向内取舍 | **三者均衡**，文件系统 / 注册表 / PowerShell 篇幅与深度相当 |
| 社区经验素材 | **引入**，但严格标注为经验型（C 级），不混入原理性论断 |
| 系统目录结构 | **接受降维改写**：基于官方 KNOWN_FOLDERID / 环境变量参考页改写，并标注原始来源 |

**对 P2 的约束**：
1. 按上表 9 个缺口逐项补齐，缺口 1（系统目录结构）与缺口 8（cmd/Bash 对照表）为改写类，需在 `02_deep_research.md` 中标注"本文整理，来源见 X"。
2. C 级社区来源仅用于"实际踩坑表现"，每条须带层级标记；原理性结论仍须回溯到 A 级来源。
3. 三个方向素材密度对齐，避免某一方向明显偏薄影响成章。
