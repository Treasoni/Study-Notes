...
| 3 | **`tcp_ecn` 取值表数值列在抓取中丢失** | 无法断言各数值语义 | 只写作用描述，**不写数值**。已回原文核对：`tcp_ecn` 的 `Default: 2`（`sources/03_docs_kernel_org.md` L356）；「Default: 3」属另一个键 `tcp_ecn_option_beacon`（L372），**两者不可混用**。数值语义仍待核对 |
