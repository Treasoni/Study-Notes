...
aliases:
  ...
  - Subagent Skill Pattern
---

# Claude Code Subagent 与 Skill 调度机制

> [!info] 背景
> 在使用 Claude Code 处理复杂项目时，==上下文窗口==是一个关键瓶颈。长时间的纠错、阅读冗长日志、反复执行测试会产生大量"垃圾信息"，导致主 Agent 变笨、遗忘目标或产生幻觉。
...
> [!info] 创建方式
> 在项目根目录 `.claude/agents/` 或全局 `~/.claude/agents/` 下编写 Markdown/YAML 配置文件
...
graph TB
    subgraph 团队架构
        PM[主 Agent<br/>项目经理]
        SOP[Skill<br/>SOP 标准作业程序]
        EMP1[固化 Subagent<br/>专职员工 A]
        EMP2[固化 Subagent<br/>专职员工 B]
        EMP3[临时 Subagent<br/>临时工]
    end
    ...
    class PM internal-link
    class SOP internal-link
    class EMP1,EMP2 internal-link
```
...
> [!warning] 常见问题
> 如果传递的信息不完整，Subagent 可能需要反复追问，浪费上下文窗口。
