...
[^c1-1]: 素材来源 S1：mihomo GitHub Releases，按 amd64/arm64 选 `.gz` 二进制 — <https://github.com/MetaCubeX/mihomo/releases>
[^c1-2]: 素材来源 S5：mihomo 官方 systemd 示例，`Restart=always` 崩溃自愈 — <https://wiki.metacubex.one/en/start/>
[^c1-3]: 素材来源 S4：mihomo 官方 inbound 文档，`mixed-port` 同时提供 HTTP 与 SOCKS5 — <https://wiki.metacubex.one/en/config/inbound/>
[^c1-4]: 素材来源 S3/S2：`proxy-providers` 订阅导入；sing-box 需转换 — <https://wiki.metacubex.one/en/config/proxy-providers/>
[^c1-5]: 素材来源 S12：既有笔记《docker进行代理》，容器用宿主机 IP 而非 127.0.0.1 — [[docker/docker进行代理]]
[^c1-6]: 素材来源 S4/S2：入站暴露公网需加 `authentication` — <https://wiki.metacubex.one/en/config/general/>
