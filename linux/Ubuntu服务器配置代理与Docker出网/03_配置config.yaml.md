...
> [[02_安装mihomo内核|← 上一章]] · [[README|🏠 首页]] · [[04_系统级代理接管|下一章 →]]

# 第 3 章：配置 config.yaml
...
[^c3-1]: 素材来源 S2：mihomo 官方文档 General config（allow-lan / bind-address / authentication / external-controller / mode / log-level）— <https://wiki.metacubex.one/en/config/general/>
[^c3-2]: 素材来源 S4：mihomo 官方文档 inbound（mixed-port / port / socks-port；`listen: 0.0.0.0` 明文代理安全警告）— <https://wiki.metacubex.one/en/config/inbound/>
[^c3-3]: 素材来源 S3：mihomo 官方文档 proxy-providers（type / url / path / interval / health-check；SAFE_PATHS；filter / exclude-filter / exclude-type）— <https://wiki.metacubex.one/en/config/proxy-providers/>
