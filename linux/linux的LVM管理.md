#linux 
# 1. Linux服务器的磁盘
我们在进行linux服务器搭建的时候，我们的硬盘经常是这样的情况：

```text
硬盘 sda        200G
├─ sda2         2G        → /boot
└─ sda3         198G      → LVM 物理卷（PV）
    └─ ubuntu-vg
        └─ ubuntu-lv     99G → /
```
关键点在这里 👇
>- sda3：198G
>- / 实际用的 ubuntu-lv：99G

👉 中间差的 ~99G：
>- 已经在 Volume Group（VG）里
>- 但 没有分配给 Logical Volume（LV）

## 1.1 为什么会这样？（不是你装错）

这是 **Ubuntu 安装器的“保守默认行为”**：

- 使用 LVM
- **只给 `/` 分一半空间**
- 留一半：
    - 方便以后加 `/home`
    - 或快照
    - 或扩容
👉 服务器上这是**好习惯**，但你现在想全用，那就扩。
# 2. 名词概念讲解
- **LVM 是一套“磁盘管理体系”**
- **VG 是 LVM 里的“空间仓库 / 池子”**

一句顺口溜版：

> **LVM 管钱，VG 存钱，LV 花钱**

记住这句，后面全通。
## 2.1 为什么会有 LVM？

在 **没有 LVM** 的年代：
>- 一块盘分好区（/、/home、/data）
>- `/` 分小了 👉 **完蛋**
>- 想扩容 👉 **重装 / 挪数据 / 高风险**

服务器一多，这种方式直接崩。

👉 **LVM 就是为了解决“以后还能随便改大小”这个问题**

## 2.2 LVM 是什么？（从“人话”讲）

**官方名：**

**LVM = Logical Volume Manager（逻辑卷管理器）**

 **人话版：**

> **LVM 是一个“把硬盘空间池化、可随时切割和扩容”的系统**

它让你做到：

>- 不一次性决定分区大小
>- 空间用完了可以随时加
>- 不用重装系统

## 2.3 LVM 的 3 个核心角色（重点）

LVM 就 3 个东西，你全搞懂就毕业了：

`PV → VG → LV`

我们一个一个拆。

### 2.3.1  PV 是什么？（Physical Volume）

**官方：**

**物理卷**

**人话：**

> **被 LVM 接管的“原材料硬盘空间”**

比如你现在：

`/dev/sda3  →  PV`

意思是：

- `/dev/sda3` 不再直接用
- 我把它交给 LVM 管

👉 就像：

> 把现金存进银行

### 2.3.2  VG 是什么？（Volume Group）🔥重点🔥

**官方：**

**卷组**

**人话（非常重要）：**

> **一个“统一的空间仓库 / 池子”**

你的机器里有：

`VG 名字：ubuntu-vg 总容量：≈ 198G`

VG 的本质是：

- 把多个 PV 合并
- 形成一个“可分配的空间池”

👉 类比：

`硬盘 / 分区 → 存进仓库（VG）`

**VG 有啥用？**

>- 不直接存文件
>- 不挂载
>- 只负责 **“给谁分多少空间”**

### 2.3.3 LV 是什么？（Logical Volume）

**官方：**

**逻辑卷**

**人话：**

> **真正能格式化、挂载、被系统用的“假分区”**

你现在有：

`LV：ubuntu-lv 大小：99G 挂载点：/`

👉 对系统来说：

- LV ≈ 分区
- `/dev/ubuntu-vg/ubuntu-lv` ≈ `/dev/sdaX`
## 2.4 再用一个超好记的类比（强烈推荐）

**银行类比（秒懂版）**

|LVM 组件|类比|
|---|---|
|硬盘 / 分区|现金|
|**PV**|存进银行|
|**VG**|银行账户|
|**LV**|银行卡|
|`/`|你刷的那张卡|

👉 你现在是：

- 账户里有 198G
- 卡里只有 99G
- 剩下的钱还没转到卡上
你现在真实结构是：
```text
硬盘 sda（200G）
 └─ sda3（198G）
     ↓
   PV（物理卷）
     ↓
   VG：ubuntu-vg（≈198G）
     ├─ LV：ubuntu-lv（99G） → /
     └─ 剩余空间（≈100G）   ← 闲着

```

这就是那句：

> **100G 在 LVM 里，但没分配给 /**
# 3. 理论讲解
## 3.1 从你的 `lsblk` 结果开始逐层拆

你现在的盘是这样的 👇

```text
sda        200G   ← 整块硬盘
├─sda1       1M
├─sda2       2G   → /boot
└─sda3     198G   → LVM 物理卷（PV）
    └─ubuntu-vg-ubuntu-lv  99G  → /

```

我们按 **“从外到内”** 的顺序来。

### 3.1.1 第 1 层：物理硬盘（Physical Disk）

`sda = 200G`

这是**实打实的 200G 硬盘**，没问题。

### 3.1.2 第 2 层：分区（Partition）

`sda3 = 198G`

这里非常关键的一点：

> **你的 200G 硬盘，已经几乎全部给了 LVM（sda3）**

所以👇  
❌ **问题不在“没分区”**  
❌ **也不在“PVE / Ubuntu 只认 100G”**

### 3.1.3 第 3 层：LVM 的第一步 —— PV（物理卷）

`sda3` 并不是直接挂载的，而是：

`sda3 → LVM Physical Volume (PV)`

这一步的含义是：

> **我把一整块分区，交给 LVM 统一管理**

就像：

>- 把 198G 现金
>- 全部存进银行（LVM）
>- 不再直接揣在兜里用


### 3.1.4 第 4 层：VG（卷组）—— LVM 的“仓库”

在 LVM 里：

`PV(sda3 198G)   ↓ VG(ubuntu-vg)`

**VG = Volume Group（卷组）**

你可以把它理解为：

> **一个“总仓库”，里面有大约 198G 可用空间**

⚠️ 注意：

>- **VG ≠ 文件系统**
>- **VG 不能直接用**
>- 它只是“空间池”

### 3.1.5 第 5 层：LV（逻辑卷）—— 真正能用的“分区”

`ubuntu-vg └─ ubuntu-lv 99G → /`

这一步发生了什么？

> **你从 198G 的仓库里，只拿了 99G，做成了一个 LV，并挂载为 `/`**

于是就出现了你看到的现象：

- 仓库里：≈ 198G
- `/` 实际用的：99G
- **剩下 ≈ 100G：还在仓库里，没人用**

# 4. 新增的硬盘如何操作？

> **在使用 LVM 的 Linux 里，新增硬盘 = 把新硬盘加入 VG，然后再分配给某个 LV**

口诀版（非常重要）👇：

`新硬盘 → PV → 加入 VG → 扩 LV → 扩文件系统`

你以后只要记住这 5 步，就不会慌。
## 4.1 第一步：确认系统认到新硬盘

```bash
lsblk
```

你会看到类似：

`sda   200G sdb   200G   ← 新盘`

👉 看不到？

>- 虚拟机：没添加
>- 实体机：BIOS / 热插拔问题

##  4.2 第 2 步：给新硬盘分区（一次性给 LVM）
具体教学可以看[添加一个硬盘的流程](linux磁盘相关的知识.md#添加一个硬盘的流程)

```bash
sudo fdisk /dev/sdb
```

**标准操作：**

```bash
n   （新建分区）
p   （主分区）
回车 回车 回车（用整块盘）
t   （修改类型）
8e  （Linux LVM）
w   （写入）

```

然后刷新：

```bash
sudo partprobe
```

现在你会得到：

`/dev/sdb1`

## 4.3 第 3 步：把新分区变成 PV（交给 LVM）

```bash
sudo pvcreate /dev/sdb1
```

验证：

```bash
pvs
```
你会看到：

```bash
/dev/sda3   ubuntu-vg
/dev/sdb1   ubuntu-vg  （还没加入）
```

## 4.4 第 4 步：把新 PV 加入 VG（关键一步）

```bash
sudo vgextend ubuntu-vg /dev/sdb1
```

查看：

```bash
vgs
```

此时：

```bash
VG        VSize   VFree
ubuntu-vg 398G    200G
```

👉 **VG 就变大了**

## 4.5 第 5 步：把空间分配给某个 LV

### 情况 1️⃣：全部扩给根目录 `/`

```bash
sudo lvextend -l +100%FREE /dev/ubuntu-vg/ubuntu-lv

```
### 情况 2️⃣：只加 100G

```bash
sudo lvextend -L +100G /dev/ubuntu-vg/ubuntu-lv
```
### 推荐的一步到位写法（强烈建议）

```bash
sudo lvextend -L +100G -r /dev/ubuntu-vg/ubuntu-lv
```

👉 `-r` = resize filesystem  
👉 **逻辑卷 + 文件系统一起扩**
可以省了第6步。

## 4.6 第 6 步：扩展文件系统（非常重要）

### 如果是 ext4（Ubuntu 默认）

```bash
sudo resize2fs /dev/ubuntu-vg/ubuntu-lv

```
### 如果是 xfs（常见于服务器）

```bash
sudo xfs_growfs /
```

## 4.7 第 7 步：确认成功

```bash
df -h
```

你会看到：

```bash
/   398G
```

🎉 **完成，全程不停机**

# 5. LVM 的优缺点

## 5.1 优点

| 优点 | 说明 |
|------|------|
| **在线扩容/缩容** | 无需停机即可调整分区大小 |
| **多磁盘合并** | 多块硬盘可以合并成一个大池子使用 |
| **快照功能** | 可以创建瞬间快照，用于备份或回滚 |
| **灵活分区** | 不需要预先精确规划分区大小 |
| **动态分配** | 从 VG 中按需分配空间给 LV |

## 5.2 缺点

| 缺点 | 说明 |
|------|------|
| **性能轻微损耗** | 增加一层抽象，性能略有下降（通常可忽略） |
| **复杂度增加** | 多一层管理结构，出问题排查难度增加 |
| **恢复复杂** | 系统崩溃时，LVM 恢复比传统分区复杂 |
| **引导限制** | BIOS 系统下，/boot 分区通常不能在 LVM 上 |

# 6. 创建新的逻辑卷

之前的流程是扩容现有 LV，如果你想**创建全新的 LV**（比如 `/home`、`/data`）：

## 6.1 流程总览

`从 VG 分配空间 → 创建 LV → 格式化 → 挂载 → 设置自动挂载`

## 6.2 具体步骤



### 步骤 1：创建新 LV

从 `ubuntu-vg` 中分配 50G 给新 LV：

```bash
# 方法 1：指定大小
sudo lvcreate -L 50G -n data-lv ubuntu-vg

# 方法 2：使用剩余所有空间
sudo lvcreate -l 100%FREE -n data-lv ubuntu-vg
```

参数说明：
- `-L 50G`：指定大小为 50G
- `-n data-lv`：指定 LV 名字
- `-l 100%FREE`：使用 VG 中所有剩余空间

### 步骤 2：验证 LV 创建成功

```bash
sudo lvs
```

输出示例：

```bash
  LV         VG         Attr       LSize   Pool Origin Data%  Meta%  Move Log Cpy%Sync Convert
  ubuntu-lv  ubuntu-vg  -wi-ao----  398.00g
  data-lv    ubuntu-vg  -wi-a-----   50.00g
```

### 步骤 3：格式化文件系统

```bash
# ext4（推荐）
sudo mkfs.ext4 /dev/ubuntu-vg/data-lv

# 或者 xfs
sudo mkfs.xfs /dev/ubuntu-vg/data-lv
```

### 步骤 4：创建挂载点并挂载

```bash
# 创建挂载点目录
sudo mkdir /data

# 挂载
sudo mount /dev/ubuntu-vg/data-lv /data
```

### 步骤 5：设置开机自动挂载

获取 UUID：

```bash
sudo blkid /dev/ubuntu-vg/data-lv
```

编辑 `/etc/fstab`：

```bash
sudo nano /etc/fstab
```

添加：

```
UUID=你的UUID /data ext4 defaults 0 2
```

### 步骤 6：验证

```bash
df -h
```

# 7. LVM 快照（Snapshot）

## 7.1 什么是快照？

快照是某个 LV 在某一时刻的**只读副本**，用于：

- 瞬间备份（无需停机）
- 测试前的状态保存
- 数据回滚

## 7.2 创建快照

```bash
# 创建 ubuntu-lv 的快照，命名 ubuntu-lv-snap，大小 10G
sudo lvcreate -L 10G -s -n ubuntu-lv-snap /dev/ubuntu-vg/ubuntu-lv
```

参数说明：
- `-L 10G`：快照大小（取决于数据变化量）
- `-s`：表示创建快照
- `-n ubuntu-lv-snap`：快照名称

## 7.3 查看快照

```bash
sudo lvs
```

快照会显示 `[ubuntu-lv]` 表示它是哪个 LV 的快照。

## 7.4 挂载快照查看内容

```bash
sudo mkdir /mnt/snapshot
sudo mount /dev/ubuntu-vg/ubuntu-lv-snap /mnt/snapshot
ls /mnt/snapshot
```

## 7.5 删除快照

```bash
sudo lvremove /dev/ubuntu-vg/ubuntu-lv-snap
```

## 7.6 从快照恢复（谨慎！）

```bash
# 1. 卸载原 LV
sudo umount /

# 2. 从快照创建新 LV
sudo lvcreate -s -n ubuntu-lv-new /dev/ubuntu-vg/ubuntu-lv-snap

# 3. 重命名（如果需要）
sudo lvrename /dev/ubuntu-vg/ubuntu-lv /dev/ubuntu-vg/ubuntu-lv-old
sudo lvrename /dev/ubuntu-vg/ubuntu-lv-new /dev/ubuntu-vg/ubuntu-lv

# 4. 挂载
sudo mount /dev/ubuntu-vg/ubuntu-lv /
```

# 8. 缩小 LV（危险操作）

⚠️ **重要限制**：

- **xfs 文件系统不能缩小！**只能扩容
- ext4 可以缩小，但必须先缩小文件系统，再缩小 LV

## 8.1 缩小 ext4 LV 的步骤

```bash
# 1. 卸载 LV
sudo umount /data

# 2. 检查文件系统（必须）
sudo e2fsck -f /dev/ubuntu-vg/data-lv

# 3. 先缩小文件系统到 30G
sudo resize2fs /dev/ubuntu-vg/data-lv 30G

# 4. 再缩小 LV 到 30G
sudo lvreduce -L 30G /dev/ubuntu-vg/data-lv

# 5. 重新挂载
sudo mount /dev/ubuntu-vg/data-lv /data
```

# 9. 删除 LVM 组件

删除顺序与创建顺序**相反**：

`LV → VG → PV`

## 9.1 删除 LV

```bash
# 先卸载
sudo umount /data

# 删除 LV
sudo lvremove /dev/ubuntu-vg/data-lv
```

## 9.2 从 VG 中移除 PV

```bash
# 如果 PV 中还有数据在用，需要先迁移数据
sudo pvmove /dev/sdb1

# 从 VG 中移除 PV
sudo vgreduce ubuntu-vg /dev/sdb1
```

## 9.3 删除 PV

```bash
sudo pvremove /dev/sdb1
```

# 10. 常用命令速查表

## 10.1 查看命令

| 命令 | 作用 |
|------|------|
| `pvs` / `pvdisplay` | 查看所有物理卷 |
| `vgs` / `vgdisplay` | 查看所有卷组 |
| `lvs` / `lvdisplay` | 查看所有逻辑卷 |
| `lsblk` | 查看整体磁盘结构 |

## 10.2 管理命令

| 命令 | 作用 |
|------|------|
| `pvcreate /dev/sdb1` | 创建物理卷 |
| `vgcreate myvg /dev/sdb1` | 创建卷组 |
| `vgextend myvg /dev/sdb2` | 扩展卷组（添加 PV）|
| `lvcreate -L 50G -n mylv myvg` | 创建逻辑卷 |
| `lvextend -L +100G /dev/myvg/mylv` | 扩展逻辑卷 |
| `lvreduce -L 50G /dev/myvg/mylv` | 缩小逻辑卷 |
| `lvremove /dev/myvg/mylv` | 删除逻辑卷 |

## 10.3 文件系统命令

| 命令 | 作用 |
|------|------|
| `resize2fs /dev/vg/lv` | 扩展/缩小 ext4 文件系统 |
| `xfs_growfs /` | 扩展 xfs 文件系统（只能扩） |
| `mkfs.ext4 /dev/vg/lv` | 格式化为 ext4 |
| `mkfs.xfs /dev/vg/lv` | 格式化为 xfs |

# 11. 与传统分区的对比

| 对比项 | 传统分区 | LVM |
|--------|----------|-----|
| **扩容** | 需要停机，操作复杂 | 在线操作，简单 |
| **大小调整** | 受分区表限制，难以调整 | 灵活调整 |
| **多磁盘使用** | 需要挂载到不同目录 | 可合并成一个 VG |
| **快照** | 不支持 | 支持 |
| **性能** | 无损耗 | 轻微损耗 |
| **复杂度** | 简单 | 较复杂 |
| **适用场景** | 简单桌面/嵌入式 | 服务器/需要灵活管理的系统 |

# 12. LVM 限制与注意事项

## 12.1 引导分区不能在 LVM 上

- **BIOS 系统**：/boot 分区必须在传统分区上
- **UEFI 系统**：EFI System Partition (ESP) 必须是 FAT32 分区

## 12.2 文件系统限制

| 文件系统 | 是否支持缩容 | 是否支持扩容 |
|----------|--------------|--------------|
| ext4 | ✅ | ✅ |
| xfs | ❌ | ✅ |
| btrfs | ✅ | ✅ |
| fat32 | ❌ | ❌ |
| ntfs | ⚠️ (需工具) | ⚠️ (需工具) |

## 12.3 建议

1. **根分区建议使用 LVM**（方便扩容）
2. **/boot 分区不要用 LVM**（兼容性问题）
3. **大容量数据分区建议用 LVM**（灵活管理）
4. **定期备份 LVM 元数据**：

```bash
# 备份 VG 元数据
sudo vgcfgbackup -f /backup/vg-backup.conf ubuntu-vg
```
