...
上面讲的是"权限位怎么读"，那 Owner 和 Group 这两栏本身又是怎么来的？创建文件时系统按固定规则自动决定，不接受随意指定：

| 字段 | 默认来源 |
|------|----------|
| **所有者（Owner / UID）** | 谁创建就归谁——你以哪个用户身份创建文件，文件的 UID 就是该用户 |
| **所属组（Group / GID）** | 创建进程的**有效 GID**，通常就是该用户的**主组（Primary / Effective Group）**，而**不是**他所属的任意一个附加组 |

注意这里的关键区别：Linux 用户可以有 1 个主组 + 若干附加组，但**新建文件只认有效主组**。你同时挂名在 `developers`、`docker`、`sudo` 里，新文件也不会自动盖 `docker` 的章。

> [!tip] 大白话
> 办入职时工牌上印谁的名字，Owner 就是谁——**谁办的，写谁**。
> 工牌盖哪个部门的章，看你**当前的部门归属**（主组），而不是你挂名的所有部门（附加组）。

**例外：父目录的 SGID**

如果父目录设置了 SGID（`chmod g+s 目录`），在该目录下新建的文件**不再使用创建者的有效主组**，而是直接继承**父目录的所属组**：
...
# alice 的主组是 alice，但在此目录下创建的文件属于 developers
$ touch /shared/project/test.txt
$ ls -l /shared/project/test.txt
-rw-r--r-- 1 alice developers 0 Sep 14 12:00 test.txt
#            ^^^^^ ^^^^^^^^^^
#            创建者  继承自父目录（父目录的组，不是 alice 的主组）
```

> [!note] 补充：有效 GID 可以被临时改变
> 进程的有效 GID 与用户的主组配置并不总是相等。用 `newgrp <组名>` 或 `sg <组名> -c '命令'` 可以临时切换当前 shell / 进程的有效 GID，此后新建文件的组会跟随切换后的值。但这只影响当前会话，不会改动 `/etc/passwd` 里的主组配置——用户与组的完整管理见 [[linux常用命令/Linux 用户管理|Linux 用户管理]]。

SGID 对目录的完整用法（含数字模式 `2755`）见下文 **6.4 特殊权限：SUID / SGID / Sticky Bit**。

### 权限判定的匹配顺序：首项匹配（First-Match Wins）

一个用户可能同时满足多个身份——既是文件的**所有者**，又属于文件的**所属组**。那系统到底按哪一档权限判定？

答案是**严格自上而下匹配，命中即停**。三档权限**不是叠加**，也**不是取交集**，而是三选一：

| 顺序 | 判断条件 | 命中后 |
|:----:|----------|--------|
| 第一步 | 当前用户是否是文件**所有者**（Owner/User）？ | 只应用 **User** 权限，**完全忽略** Group 和 Others，判定结束 |
| 第二步 | 不是所有者，当前用户是否属于文件**所属组**（Group）？ | 只应用 **Group** 权限，**完全忽略** Others，判定结束 |
| 第三步 | 前两者都不满足 | 应用 **Others** 权限 |

> [!tip] 大白话
> 像小区门禁刷卡：先看你是不是业主——是业主，就只按业主规则放行，**压根不会再去翻访客规则**；不是业主，才看是不是家属卡；都不是，才按访客处理。
> 关键在于**不叠加**：不会出现"业主规则不行，再拿家属规则试一次"这种兜底逻辑。

**实例 1：组只有只读，所有者依然能写**

```bash
$ ls -l file.txt
-rw-r--r-- 1 alice developers 100 Sep 14 12:00 file.txt
```

- 所有者 `alice`：`rw-`（可读可写）
- 所属组 `developers`：`r--`（只读，不可写）
- `alice` 恰好也在 `developers` 组里

`alice` 执行写入时，第一步就命中"所有者"，直接取用 User 权限 `rw-`。**所属组的"只读"限制被整个跳过**，`alice` 正常写入。

这里不存在"冲突"——权限不是"用户权限 ∩ 组权限"，只是 Group 那一档**根本没参与判定**。所谓"用户可读写、组不可写"的矛盾设置，在实际判定中根本不会碰面。
...
> ```bash
> $ ls -l file.txt
> ----rw-r-- 1 alice developers 100 Sep 14 12:00 file.txt
> ```
>
> `alice` 是所有者，但 User 权限是 `---`（空）。**即使 `alice` 属于 `developers` 组、而组权限是可读写**，系统在第一步命中所有者后就直接拒绝——`alice` 反而读不了也写不了这个文件。
>
> 这正是"首项匹配不叠加"的直接后果：**所有者位一旦留空，组权限救不了你**。所以 `chown` 之后一定要顺手检查 User 位是否给足，别指望组权限兜底。

> [!note] 补充：root 不受这套规则约束
> `root` 用户（以及持有相应 capability 的进程）**绕过权限位检查**：任何文件都能读、能写，无论权限位怎么设置。
>
> 唯一的例外是**执行**——root 执行一个文件时，该文件至少要有一个 `x` 位被置上，否则同样会得到 `Permission denied`。
...
> [!note] SGID 与「新建文件的默认组」是同一机制
> 6.1 提到的例外——"父目录设了 SGID，新文件的组就继承父目录"——正是这里说的 SGID 对目录的行为，两边是同一个开关，只是分别从"文件组的来源"和"特殊权限位"两个角度描述。
...
- **新建文件的 Owner 归创建者，Group 归创建者的主组**（不是任意附加组）；父目录设了 SGID 则继承父目录的组
- **权限判定是首项匹配（First-Match Wins）**：先看所有者 → 再看组 → 最后看其他人，命中即停，**不叠加、不取交集**——所有者位留空时，组权限救不了你
...
| 日期 | 变更 |
|------|------|
| 2026-09-14 | 新增 6.1 两节：**「新建文件的默认所有者与所属组」**（Owner 归创建者、Group 默认取进程有效主组而非任意附加组、父目录 SGID 例外、`newgrp`/`sg` 临时切换有效 GID）与**「权限判定的匹配顺序：首项匹配（First-Match Wins）」**（所有者 → 组 → 其他人三步命中即停、不叠加不取交集；组只读不影响所有者写入的实例；`----rw-r--` 反直觉陷阱；root 绕过权限位但执行仍需任一 `x` 位）。两节均补 `[!tip] 大白话` 类比；本章总结同步补充对应要点 |
