...
**是什么**：拥有**独立上下文**的专用执行单元，由主 Agent 派发任务并独立完成

**为什么需要**：
- 隔离复杂任务的中间过程，避免污染主上下文
- 并行处理多个独立子任务
- 为特定任务配置专属工具集（白名单/黑名单）

**与 Agent 核心区别**：

| 维度 | Agent | SubAgent |
|------|-------|----------|
| **上下文** | 共享主上下文 | 完全独立的上下文空间 |
| **执行方式** | 串行，在主推理流程中展开 | 可并行，独立推理循环 |
| **工具调用** | 通过主 Agent 间接调用 | 可配置专属工具集 |
| **适合任务** | 分析、审查、简单线性任务 | 开发、重构、代码探索等复杂多步骤任务 |
| **资源消耗** | 中等 | 较高（额外协调开销） |
| **Token 成本** | 低 | 较高（独立上下文需要额外 token） |

**选择原则**：> **能用 Agent 就不用 SubAgent，能用 SubAgent 就不用 Agent Teams**
