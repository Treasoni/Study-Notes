...
> [!summary] 导读
> [[DeepSeek-Harness 插件开发教程/02-配置体系-补丁树Profile与bundle|配置体系]] 的配套实操专册。上一篇讲了 preset「是什么」——会话级能力组合、4 个内置预设、目录结构；这篇专讲「怎么用」：**怎么选**（4 个预设的真实身份）、**怎么换**（切换 / 默认 / 重链）、**怎么造**（复制目录 / copy API / 真实示例 + 验证步骤）。
...
相关：[[DeepSeek-Harness 插件开发教程/02-配置体系-补丁树Profile与bundle|配置体系（preset 是什么）]] → 本册（preset 怎么造）→ [[DeepSeek-Harness 插件开发教程/01-插件开发核心-从apply到system-prompt|插件开发核心（装配的插件从哪来）]]。
