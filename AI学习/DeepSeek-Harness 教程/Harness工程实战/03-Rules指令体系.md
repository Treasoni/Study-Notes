---
title: "第三章 Rules/指令体系——AGENTS.md、CLAUDE.md 与 workspaceContext"
tags: [deepseek-harness, ai, agent, rules, AGENTS.md, 实战]
created: 2026-08-16
updated: 2026-08-16
status: new
...
dsh 官方源码里 `instructionFileCandidates` 的默认值就是 **`['AGENTS.md', 'CLAUDE.md']`**[^c3-b3][^c3-d1]——它从 session 工作目录向上找最近的含 `.git` 的祖先作为项目根，逐个目录加载这些文件。
...
`@deepseek-ai/dsh-agent-instructions` 插件（"User-facing workspace instruction loader configuration"）拥有 AGENTS.md/CLAUDE.md 加载能力。在 spine bundle 里字段是 `workspaceContext`[^c3-b3]：
...
**设 `false` 可整体关闭**：官方注释明确 "Workspace context instead requires an explicit byte budget or `false` because it changes model-visible input"——关闭后得到 hermetic prompts[^c3-b3]。
...
dsh 官方仓库 root/packages/examples 三处都用 `CLAUDE.md` symlink 到 `AGENTS.md`，**edit 真文件**（即 `AGENTS.md` 是 canonical source）[^c3-b1]。
...
[^c3-b1]: B1 · dsh 官方仓库 `AGENTS.md`，2026-08-16 抓取。
[^c3-b3]: B3 · dsh 官方 `docs/config-catalog.md`（workspaceContext / agent-instructions），2026-08-16 抓取。
[^c3-d1]: D1 · 你的 vault 笔记《03-配置实战-接入skills-hooks-mcp-rules》，2026-08-16。

---

> [!info] 导航
> [[02-先创建哪些文件|← 上一章：开始一个项目]] · [[README|返回首页]] · [[04-Skills放置与结构|下一章：Skills →]]
