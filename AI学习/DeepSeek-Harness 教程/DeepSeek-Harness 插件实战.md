...
> **前置要求**：建议先读 [[DeepSeek-Harness 安装与快速上手|第 2 章]]（源码环境就绪）、[[DeepSeek-Harness 插件开发核心|第 3 章]]（插件概念）与 [[DeepSeek-Harness 配置体系|配置体系专册]]（补丁树 / bundle 术语）；排查时随时查 [[DeepSeek-Harness 常见坑与速查|第 5 章]]。本分册不重复讲概念，未读也可照做。
