---
title: "第二章 开始一个项目——先创建哪些文件"
tags: [deepseek-harness, ai, agent, 脚手架, 实战]
created: 2026-08-16
updated: 2026-08-16
status: new
...
dsh 的 `instructionFileCandidates` 默认值是 **`['AGENTS.md', 'CLAUDE.md']`**——它把两个都当指令文件候选，从 session 工作目录向上找最近含 `.git` 的祖先作为项目根，逐目录加载[^c2-b3][^c2-d1]。
...
dsh 没有「一份完整配置文件」，是**三层文件 + 一个用户级 home** 的分布式结构[^c2-d1][^c2-d2]：
...
> `.dsh` 是真实存在的「harness home」，但它只覆盖四块能力里的两块：**rules 不是放 `.dsh` 里、hooks 和 mcp 也不是**。rules 是项目根黑板（`AGENTS.md`/`CLAUDE.md`）；hooks 和 MCP 要走 `cordis.yml` 的插件系统「申请」。所以 `.dsh/skills/` 是你唯一需要亲手建的项目级 dsh 目录[^c2-d1]。

## 2.3 开发 dsh：源码路径 + 最小插件 2 文件

如果你要写插件（自定义工具、hook 插件、subagent provider），必须走源码路径[^c2-d5]：
...
然后**最小插件 2 文件**（在仓库根目录下建插件目录）[^c2-d5]：
...
要点：`export const name` 是诊断名（四名分离的第一个）；`apply(ctx)` 是插件入口，`ctx` 是上下文句柄[^c2-d5]。
...
> patch 的 `name` 会被加载器**直接当文件系统路径**定位模块，不相对工作目录换算。写相对路径**不会报错、不会警告**——只是模块永远加载不上。这种「没报错的静默」是新手最容易被消耗时间的地方[^c2-d5]。
...
无论哪条路，第一次跑起来都要在 Web UI 做两步首配[^c2-d5]：
...
[^c2-b3]: B3 · dsh 官方 `docs/config-catalog.md`（workspaceContext 配置），2026-08-16 抓取。
[^c2-d1]: D1 · 你的 vault 笔记《03-配置实战-接入skills-hooks-mcp-rules》，2026-08-16。
[^c2-d2]: D2 · 你的 vault 笔记《02-配置体系-补丁树Profile与bundle》，2026-08-16。
[^c2-d5]: D5 · 你的 vault 笔记《DeepSeek-Harness 是什么 / 安装与快速上手 / 05-实战-起步-最小骨架与脚手架》，2026-08-16。

---

> [!info] 导航
> [[01-心智模型|← 上一章：心智模型]] · [[README|返回首页]] · [[03-Rules指令体系|下一章：Rules/指令体系 →]]
