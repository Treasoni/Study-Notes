...
> 适用对象：已经会用 Claude Code 基础功能，希望了解如何把"多 agent 编排"从一次性操作升级为可复用脚本的学习者。#claude-code #dynamic-workflows
...
```mermaid
flowchart LR
    A[传统 Subagent 编排] -->|Claude 上下文里调度| B[Turn-by-Turn 协调]
    B -->|规模瓶颈| C[力不从心]
    D[Dynamic Workflow] -->|脚本持有计划| E[后台运行时执行]
    E -->|几十~几百 agent| F[规模可扩展]
    C -.对比.-> F
```
...
> [!tip] 关键洞察
> 在前三种模式里，**Claude 是编排者**；而 Workflow 把编排权交给了**一段脚本**。Claude 的上下文里只剩下最终答案——所有循环、分支、中间状态都活在脚本里。[来源: R1]
...
```mermaid
flowchart TB
    subgraph "主对话 Session"
        U[用户] <--> C[Claude 主 agent]
    end
    subgraph "隔离运行时"
        S[Workflow 脚本]
        S --> V[脚本变量<br/>中间结果]
...
> [!warning] 成本警示
> 默认 fan-out 会继承父模型，缺少 per-agent 成本上限。在大型 run 启动前必须确认模型与阶段预算。
...
classDiagram
    ...
    User --> SlashCommand
    SlashCommand --> DynamicWorkflow
    DynamicWorkflow --> Subagent
    Skill ..> Subagent : 注入
    Hook ..> Subagent : 拦截
    CLAUDEmd ..> Subagent : 读取
```

> [!tip] 关键边界
> [[如何编写Skills|skills]] 跑在前台（在主上下文或 forked subagent 里）；workflow 跑在后台（独立运行时 + 隔离脚本）。**`/deep-research` 是个 workflow，不是 skill**——这是初学者最常踩的分类错误。[来源: R4][来源: R9]
...
> [!note] 历史兼容性
> 早期版本里 `args` 是 JSON 字符串，社区提供了一个 workaround [来源: R7]：
...
> [!tip] 速查提示
> 想快速翻查时看这一节。命令、参数、位置、快捷键都列在这里。#cheatsheet
