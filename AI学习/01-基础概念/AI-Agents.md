...
> [!info] 概述
> **一句话定义**：AI Agent 是能够自主感知环境、推理决策、执行任务并与环境交互的智能系统。
>
> **通俗比喻**：如果 LLM 是一个只会说话的顾问，那么 AI Agent 就是一个能帮你实际办事���助理——不仅会给出建议，还会拿起电话、打开电脑、操作软件，真正把事情做完。
...
**一句话总结**：LLM 会说话，AI Agent 会做事。

**价值主张**：传统 AI 让你在单个任务上更快，AI Agents 让你在整个项目上更快。
...
**A**: 核心区别在于**自主性**。普通 AI（如 LLM）是被动工具，需要用户明确指令；AI Agent 能自主规划、决策和执行，具有主动完成任务的能力。

### Q2: 什么时候应该用 Agent 而不是直接用 LLM？

**A**: 当任务满足以下条件时，考虑使用 Agent：
- 需要多步骤执行
- 需要调用外部工具或 API
- 需要根据中间结果动态调整策略
- 需要保持长期记忆
