...
#### 嵌入模型配置（Embedding Models）

> [!info] 这是什么？
> **嵌入模型**是 Smart Connections 的"大脑"，它负责：
> 1. **理解**你的笔记内容
> 2. **转换**成数字向量
> 3. **比较**找出语义相似的笔记
...
│  └─────────────────────────────────────────────┘       │
│                                                         │
│  Current: transformers - TaylorAI/bge-micro-v2         │
...
| 模型名称 | 推荐度 | 特点 | 适用场景 |
|---------|-------|------|---------|
| **Multilingual E5 Small** | ⭐⭐⭐⭐⭐ | 对中文支持最好，专门为多语言设计 | **中文笔记首选！**检索精度最稳 |
| **BGE-micro-v2** | ⭐⭐⭐ | 速度最快，模型体积极小 | 快速体验，对精度要求不高 |
| **Snowflake Arctic Embed XS** | ⭐⭐⭐ | 性能均衡 | 英文笔记或混合笔记 |
| **BAAI/bge-small-en-v1.5** | ⭐⭐⭐⭐ | 精度较高，本地运行 | 追求质量的进阶用户 |

> [!tip] 为什么推荐 Multilingual E5 Small？
> 1. **中文兼容性**：在中文环境下，BGE 和 Snowflake 的某些版本对词义的理解有时会产生偏差，E5 是多语言检索的标杆
> 2. **资源占用低**：虽然是 Small 版本，但对内存和 CPU 占用很低，Mac 跑起来完全无压力
> 3. **检索质量**：能更精准地通过"意思"而不是"关键词"找到关联笔记

##### 如何切换嵌入模型？

1. **进入设置**：设置 → Smart Connections → Smart Environment → Embedding models
2. **选择模型**：点击下拉框，选择你想要的模型
3. **重新索引**：切换模型后，**必须**重新索引整个笔记库

> [!warning] 重要提醒
> 更换嵌入模型后，**必须重新索引**！否则会出现新旧向量混用导致的结果不准确。
> 重新索引时电脑风扇可能会转一会儿，属于正常现象。

##### 本地 vs 云端模型

| 类型 | 优点 | 缺点 | 推荐场景 |
|------|------|------|---------|
| **本地模型** (transformers) | 免费、离线、隐私好 | 精度略低于大模型 | 日常使用、隐私优先 |
| **云端模型** (OpenAI等) | 更精准 | 需要 API Key、网络、付费 | 追求高质量、网络稳定 |

---

#### 其他 Smart Environment 设置
...
| 设置 | 改变什么 | 何时使用 |
|------|----------|----------|
| **嵌入模型选择** | 选择"大脑"模型 | 中文笔记推荐 **Multilingual E5 Small** |
