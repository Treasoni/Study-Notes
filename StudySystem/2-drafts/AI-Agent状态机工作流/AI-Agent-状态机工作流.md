...
## Part A: 核心概念 (Core Concepts)

### 1. 为什么需要状态机

> **为什么这节重要**：搞清楚"agent 为什么跑偏"是设计正确工程结构的前提，否则后面所有模式都只是空中楼阁。
...
> **实战要点**：能写成 workflow 的不要写成 agent。Agent 适合开放式问题（步骤数无法先验确定），Workflow 适合定义清晰的任务。

---

### 2. FSM 基础

> **为什么这节重要**：很多人用 LangGraph 但不知道它在底层就是 FSM + Actor Model。理解这一点能让你在选型、debug、调优时心里有底。
...
> **小结**：FSM 不是新事物。把 FSM 思想引入 agent 工程，核心目的是把 LLM 的"黑盒推理"显式化、可视化、可恢复。

---

### 3. Agent 范式对比：FSM vs ReAct vs Plan-and-Execute

> **为什么这节重要**：选错范式 = 后期无法收敛。先理解每个范式的边界条件。
...
> **实战要点**：ReAct 简单但 unbounded，生产环境必须有兜底机制。FSM 复杂但 predictable，永远要知道 agent 当前"在哪一格"。

---

### 4. 主流实现

> **为什么这节重要**：不要重新发明轮子。选型时理解每个框架的设计哲学能省下 3 个月试错。
...
> **实战要点**：LangGraph = 工程深度，CrewAI = 上手速度，AutoGen = 分布式 actor。需要"既能工作又能 debug"，首选 LangGraph。

---

### 5. 状态机核心机制

> **为什么这节重要**：这节是 LangGraph 的"ABC"。读懂这里，后面所有模式都只是这五个概念的组合。
...
> **小结**：State 是数据，Node 是函数，Edge 是控制流，Command 是"动作+转移"原子，Send 是动态 fan-out。记住这五个词就够了。

---

## Part B: 实战示例 (Practical Examples)

### 6. 基础工作流模式

> **为什么这节重要**：Lance Martin 的视频用 LangGraph 把 Anthropic 博客里的 5 个模式从零实现了一遍 [来源: video-02.md]。这一节是整个笔记最浓缩的精华。
...
> **实战要点**：选模式前先问"步骤数我能先验确定吗？"能 → chaining；能并行 → parallelization；分支多 → routing；分支数动态 → orchestrator-worker；需要质量迭代 → evaluator-optimizer。

---

### 7. 完整 LangGraph 代码示例

> **为什么这节重要**：概念看再多不如跑一遍。下面三个代码块覆盖了核心模式、Send API、evaluator loop——每一个都是生产可用的脚手架。
...
> **实战要点**：worker 返回的 `completed_sections` 用了 `operator.add` reducer，否则并行写入会互相覆盖 [来源: video-02.md]。
...
> **小结**：三个示例展示了从单步（示例 1）到并行 fan-out（示例 2）到反馈循环（示例 3）的演进。生产中你可能要把它们嵌套组合。

---

### 8. 多 Agent 协作：Subgraphs

> **为什么这节重要**：复杂业务几乎一定要拆成多个子图。理解父子图通信模式是构建可维护系统的关键。
...
> **实战要点**：能复用就拆 subgraph；subgraph 的 checkpointer 选 `None`（默认）还是 `True` 取决于是否需要跨调用记忆。
...
> **为什么这节重要**：这是本笔记用户最关心的部分。Terlson 演讲的核心论点是：状态机为 agent 提供 predictability / traceability / recoverability [来源: video-01.md]。
...
> **实战要点**：单层防御不够。生产级 agent 至少要 recursion_limit + 关键节点 interrupt + prompt CI 三件套。

---

### 10. 持久化与错误恢复

> **为什么这节重要**：LLM 调用是 flaky 的（超时、限流、幻觉），agent 又可能是长时任务（小时级）。没有持久化层，任何失败都是从头开始。
...
> **实战要点**：永远不要用 `InMemorySaver` 跑生产。它是测试用的。Postgres 是默认选择。云上想脱 Postgres 可以用 Lyft 的 `DynamoDBSaver` 模式自己实现 `BaseCheckpointSaver` 接口。

---

### 11. 可观测与调试

> **为什么这节重要**：生产 agent 失败时，**没有 trace = 没有调试**。LLM 调用链长、并行、条件分支，缺少 trace 几乎无法定位问题。
...
> **实战要点**：trace + 监控告警 + 测试，三件套缺一不可。Lyft 的 PagerDuty 告警阈值：错误率 >5% 或 p95 latency >10 秒持续 15 分钟 [来源: doc-14.md]。

---

## Part D: 选型速查 (Decision Framework)
...
Q1: 任务步骤能先验画图吗？
├─ 是 → Q2
│   ├─ 需要状态持久化？ → LangGraph（首选）
│   ├─ 业务人员配置？ → CrewAI
│   └─ 嵌入式/极致性能？ → 自研 FSM（XState）
└─ 否（开放式）→ Q3
    ├─ 工具数 < 10？ → LangGraph + ReAct 模板
    ├─ 工具数 > 10？ → LangGraph + 严格 routing
    └─ 分布式 actor 风格？ → AutoGen

无论选什么：
├─ 生产 → 必须有 checkpointer
├─ 风险操作 → 必须有 interrupt
└─ 任何规模 → 必须接 LangSmith 监控
```
...
> **实战要点**：上面 10 个坑在 Lyft、Anthropic、LangChain 官方都明确点过。**生产前先把这个清单过一遍**。

---

### 15. 未来趋势

> **为什么这节重要**：Terlson 在 2025 年 AI Engineer 大会上描绘了一个清晰的前进方向 [来源: video-01.md]。理解趋势能帮你选对长期生态。
...
> **实战要点**：短期学 LangGraph，中期关注 Chartering，长期用低层基础设施 + 多个高层 SDK（Deep Agents、CrewAI）组合。

---

## 思考题

1. **概念理解**：为什么说"workflow 是脚手架，agent 把脚手架撤掉"？试举例：同一个客服任务，写成 workflow 和 agent 在可预测性、灵活性上各有什么代价？

2. **应用场景**：你的团队要做一个"长篇研报自动生成"agent，应该选 LangGraph 的哪个模式（chaining / parallelization / orchestrator-worker / evaluator-optimizer）？为什么？如果要加入"质量不达标重写"的兜底，应该怎么改？

3. **边界情况思考**：当 `recursion_limit=25` 真的触发了，应该如何**优雅降级**？是把已完成的中间结果回给用户，还是抛错让人介入？两种策略各适合什么业务？

4. **工程取舍**：HITL 的 `interrupt()` 加在哪里最有效？太少 → 风险大；太多 → 体验差。你会用什么原则决定"哪些节点必须人工审批"？

5. **架构决策**：为什么 Lyft 选择 LangGraph 而不是 CrewAI？他们的核心考量（prompt 质量、debug 友好、状态可见）对你的项目选型有什么启示？
