...
tags: [AI/编码助手, 配置文件管理, ClaudeCode, Codex, CodeBuddy, 最佳实践]
description: 在多 AI 编码助手（Claude Code、Codex、CodeBuddy 等）之间共享 skills、rules、MCP、subagent 配置的完整方案
created: 2026-07-11
updated: 2026-07-11
sources: [R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12]
concepts: [CLAUDE.md, AGENTS.md, MCP, symlink, skill-sharing]
---
...
这是 **重复配置** 的典型困境：每个 Agent 使用不同的目录名、不同的文件格式、不同的发现路径，但核心内容（Skills、Rules、Subagent 定义）基本一致。 [来源: R4]
...
```
1. Base settings        ← Git 跟踪的源配置
       |
2. Profile overrides   ← 活跃 Profile 的覆盖
       |
3. User overrides       ← ~/.ai-agent-rules-config.yaml
       |
4. Preserved fields     ← 从缓存合并，保护 Agent 管理的字段
       |
5. Cached              ← 存储在 ~/.ai-agent-rules/cache/<agent>/
       |
6. Symlinked           ← 最终结果 symlink 到 Agent 配置目录
```
...
```
你使用多少个 Agent？
├── 1 个 → 不折腾，直接用 Agent 自带的配置
└── 2-4 个 →
    ├── 你是个人开发者吗？
    │   ├── 是 → 方案一：符号链接（ai-agent-config / AgentSync）
    │   │      建议：优先尝试 AgentSync，配置更清晰
    │   └── 否 →
    │       ├── 团队协作 → 方案二：安装管理器（ai-config）
    │       └── 跨多台机器 → 方案三：合并管线（ai-agent-rules）
    └── 5+ 个 →
        └── 方案二：安装管理器（swarmskills）
            建议：swarmskills 是当前支持 Agent 最多的方案
```
...
注意：保持 `name` 与父目录名一致，使用小写+连字符，1-64 字符。 [来源: R12]
...
> **坑点 1：Claude Code 的技能列表不显示 symlink**
>
...
通过 Agent 的钩子系统（hooks），在每次工具调用前动态注入或修改 MCP 配置。Claude Code 和 Codex 都支持 PreToolUse 和 PostToolUse 钩子。 [来源: R1, R2]
